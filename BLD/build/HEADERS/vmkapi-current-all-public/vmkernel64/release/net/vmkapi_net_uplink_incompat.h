...
/*
 ***********************************************************************
 * vmk_UplinkStartPanicTx --                                      */ /**
 *
 * \brief Handler used by vmkernel to send packet through the device
 *        associated to an uplink, during PANIC.
 *
 * \param[in] clientData         Internal module structure for the device
 *                               associated to the uplink. This structure
 *                               is the one passed during uplink connection
...
typedef VMK_ReturnStatus (*vmk_UplinkStartPanicTx)(void *clientData,
					           vmk_PktList pktList);
 ...
 * \param[in]  traceSrcID      Identifier for the uplink device
 *                             generating trace events. Stored locally
 *                             and used in RecordEvent/Interrupt vmkapi
 *                             calls.
 *
 * \retval    VMK_OK       If the driver information is properly stored
 * \retval    VMK_FAILURE  Otherwise
...
typedef VMK_ReturnStatus (*vmk_UplinkSetPktTrace)(void *clientData, 
                                                  vmk_Bool enable,
...
typedef struct {
   ...
   vmk_UplinkSetPktTrace        setPktTrace;
...
typedef struct {
   ...
   /** Handler used in Panic to Tx a packet immediately through the device */
   vmk_UplinkStartPanicTx           startPanicTxImmediate;
...
typedef struct vmk_UplinkConnectInfo {
   ...
   /** Modes supported by the uplink */
   vmk_UplinkSupportedMode *supportedModes;

   /** Number of supported modes stored in the array */
 ...
 *
 * \param[in]  vectorNum  Interrupt vector number generating the trace.
...
VMK_ReturnStatus vmk_UplinkRecordInterrupt(vmk_uint32 vectorNum,
                                           vmk_uint64 traceSrcID);

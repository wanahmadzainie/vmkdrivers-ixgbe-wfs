 ...
 * destination packet might be modified as a result of this call.
 *
 * \note  Once this function return VMK_OK, the source packet can not be
 *        used any more since its ownership has been transferred to the
 *        stack.
 *
 ...
 *
 * \ingroup PktIncompatEncapFrameLayout
 * \brief   Set the PKT_ENCAP bit to indicate that the packet has an 
 *          encapsulated frame along with setting up and associating
 *          the encap frame layout for the packet.
 *
 * \param[in]   pkt         handle needed for encap frame layout.
 *
 * \retval      VMK_OK      The encap frame bit is set, and the Encap
 *                          Frame Layout handle is linked to the pkt. 
 * \retval      VMK_FAILURE No encap frame layout has been registered
 *                          or set up for this pktHandle.
 *
 ***********************************************************************
 */
 ...
 *
 * \ingroup PktIncompatEncapFrameLayout
 * \brief   Check if the pkt has an encapsulated frame layout associated
 *          with it. 
 *
 * \param[in]   pkt       handle needed to check for encap frame layout.
 *
 ...
 *
 * \ingroup PktIncompat
 * \brief   Check whether both the outer and the inner IP and L4 hdr 
...
vmk_Bool 
vmk_PktGetEncapCsumVfd(const vmk_PktHandle *vmkPkt);
 ...
 *
 * \ingroup PktIncompat
 * \brief   Mark the given packet as "IP/L4 checksum verified" for both
...
VMK_ReturnStatus 
vmk_PktSetEncapCsumVfd(vmk_PktHandle *vmkPkt);

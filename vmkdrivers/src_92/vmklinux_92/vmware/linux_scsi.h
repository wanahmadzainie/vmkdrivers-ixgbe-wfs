...
VMK_ReturnStatus SCSILinuxDumpCommand(void *clientData, 
                                             vmk_ScsiCommand *vmkCmdPtr, 
					     void *deviceData);
void SCSILinuxDummyDumpBH(void *clientData);
...
int SCSILinuxTLSSetVector(struct scsiLinuxTLS *tls, vmk_uint32 vector);
...
vmklnx_ScsiAdapterVectorInfo * SCSILinuxGetVectorInfo(
   struct vmklnx_ScsiAdapter *adp, vmk_uint32 vec);
void SCSILinuxRemoveAllVectorTracker(struct vmklnx_ScsiAdapter *adp);

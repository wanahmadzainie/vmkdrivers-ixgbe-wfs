...
vmklnx_setup_adapter_tls(struct vmklnx_ScsiAdapter *vmklnx26ScsiAdapter)
{
   ...
   vInfo = vmklnx_kzmalloc(vmklnxLowHeap, numCompObjects * sizeof(*vInfo), GFP_KERNEL);
   if (vInfo == NULL) {
      goto tls_create_error;
   }
   memset(vInfo, 0, numCompObjects * sizeof *vInfo);
   vmklnx26ScsiAdapter->vector_info = vInfo;
   vmklnx26ScsiAdapter->numVec = 0;
...
tls_create_error:
   ...
   if (vInfo != NULL) {
      vmklnx_kfree(vmklnxLowHeap, vInfo);
      vmklnx26ScsiAdapter->vector_info = NULL;
   }
...
scsi_add_host(struct Scsi_Host *sh, struct device *pDev)
{
   ...
   vmkAdapter->dumpBHHandler = SCSILinuxDummyDumpBH;
                                       ...
                                       void *dev_id)
{
   ...
   result = vmk_ScsiRegisterIRQ(vmkAdapter, irq, Linux_IRQHandler, idr);
...
vmklnx_destroy_adapter_tls(struct vmklnx_ScsiAdapter *vmklnx26ScsiAdapter)
{
   ...
   if (vmklnx26ScsiAdapter->vector_info != NULL) {
      /*
       * Remove all the Interrupt Vector Trackers, using
       * SCSILinuxRemoveAllVectorTrackers.
       */
      SCSILinuxRemoveAllVectorTracker(vmklnx26ScsiAdapter);
      vmklnx_kfree(vmklnxLowHeap, vmklnx26ScsiAdapter->vector_info);
   }
                     ...
                     struct vmklnx_scsi_ioqueue_info q_info[])
{
   ...
   if (vmklnx26ScsiAdapter->numVec > 1) {
      return VMK_EXISTS;
   }

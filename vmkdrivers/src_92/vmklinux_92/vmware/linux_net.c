/* ****************************************************************
 * Portions Copyright 2005, 2009-2012 VMware, Inc.
 *
               ...
               struct sk_buff **pskb)
{
   ...
   vmk_Bool any_encap=VMK_FALSE, vdl2_encap=VMK_FALSE, pkt_w_layout=VMK_FALSE;
...
#ifdef VMX86_DEBUG
 {
   ...
   any_encap = ((dev->features & NETIF_F_ENCAP)   ||
                (dev->features & NETIF_F_OFFLOAD_8OFFSET) ||
                (dev->features & NETIF_F_OFFLOAD_16OFFSET));
   ...
   if (any_encap) {
      ...
      if (status == VMK_OK) {
          ...
          */
         pkt_w_layout = VMK_TRUE;
         vdl2_encap = ((dev->features & NETIF_F_ENCAP) &&
                        vmk_PktIsEncapsulatedFrame(pkt));
      }
   }

   if (pkt_w_layout && !vdl2_encap) {
      ...
      struct ethhdr *eh;
      vmk_uint16 IpHdrOffset = 0;
      vmk_uint8 l4proto = 0;

      eh = (struct ethhdr *) skb->head;
      IpHdrOffset = eth_header_len(eh);
      skb->nh.raw = skb->mac.raw + IpHdrOffset;
      skb->protocol = eth_header_frame_type(eh);
      sgInspected = 1;

      if (eth_header_is_ipv4(eh)) {
         if (headLen < IpHdrOffset + sizeof(*skb->nh.iph)) {
            ret = VMK_FAILURE;
            goto done;
         }
         skb->h.raw = skb->nh.raw + skb->nh.iph->ihl*4;
         pkt_ipv4 = VMK_TRUE;
         l4proto = skb->nh.iph->protocol;
      } else {
         if (vdl2_encap) { // Outer VDL2 hdr IPv6 is not supported
            ret = VMK_FAILURE;
            goto done;
         } else {
            pkt_ipv4 = VMK_FALSE;
            l4proto = 0xff;  // unused value.
            if (skb->protocol == ETH_P_IPV6_NBO) {
               ipv6_set_hraw(skb, &l4proto);
               VMKLNX_DEBUG(3, "ipv6 %ld offset %ld %d",
                           skb->h.raw - (vmk_uint8 *)(skb->data),
                           skb->h.raw - (vmk_uint8 *)(skb->nh.ipv6h), l4proto);
            }
         }
      }

      if (vdl2_encap) {
         skb_encap_layout_t *encap_layout;

         /* Make inner offset available to the driver via skb CB */
         encap_layout = VMKLNX_SKB_ENCAP_CB(skb);
         encap_layout->magic=SKB_ENCAP_MAGIC;
         encap_layout->innerIpHdrOff = ehLen;
         encap_layout->innerL4Off = ehLen+ipHdrLength;
         encap_layout->innerL4Proto = protocol;

         VMKLNX_DEBUG(3, "VDLCHK Successfully got the outer and inner hdr "
                         "outerIP=0x%p outerL4=0x%p outerL4Proto=%d "
                         "innerIP=0x%x innerL4=0x%x innerL4proto=%d ",
                          skb->nh.raw, skb->h.raw, ip_hdr(skb)->protocol, 
                          ehLen, (ehLen + ipHdrLength), protocol); 
      } 
      else { 
         // ehLen and protocol must be set properly in the non-vdl2 case!
         ehLen = IpHdrOffset;
         protocol = l4proto;
      }
   } 
   ...
   if (must_csum || must_tso) {
      ...
      case IPPROTO_TCP:
         skb->csum = 16;
         skb->ip_summed = (vdl2_encap?CHECKSUM_HW_ENCAP:CHECKSUM_HW);
         break;
...
{
   VMK_ReturnStatus ret = VMK_OK;
   ret = netdev_tx_internal(dev, pktList, vmkqid, VMK_TRUE);
   return ret;
}
...
{
   VMK_ReturnStatus ret = VMK_OK;
   ret = netdev_tx_internal(dev, pktList, vmkqid, VMK_FALSE);
   return ret;
}
...
netdev_query_capabilities(struct net_device *dev)
{
    ...
    *    Note: Both the 8OFFSET and 16OFFSET bits also need to be set
    *          so that CSO/TSO is not done in software for encapsulated
    *          packets.
    */
 ...
 */
static VMK_ReturnStatus
StartPanicTxImmediate(void *clientData, vmk_PktList pktList)
{
   struct net_device *dev = (struct net_device *)clientData;
   vmk_PktHandle *pkt = vmk_PktListGetFirstPkt(pktList);
   vmk_NetqueueQueueID vmkqid;
 ...
 */
static VMK_ReturnStatus
NICSetLinkStatus(void *clientData, vmk_LinkStatus *linkInfo)
{
   struct net_device *dev = (struct net_device *)clientData;
   struct ethtool_cmd cmd;
   uint32_t result;
                     ...
                     vmk_UplinkConnectInfo *connectInfo)
{
   ...
   if (mask & SUPPORTED_Autoneg) {
      connectInfo->supportedModes[i].speed = VMK_LINK_SPEED_AUTO;
      connectInfo->supportedModes[i].duplex = VMK_LINK_DUPLEX_AUTO;
      i++;
   }
   if (mask & SUPPORTED_10baseT_Half) {
      connectInfo->supportedModes[i].speed = 10;
      connectInfo->supportedModes[i].duplex = VMK_LINK_DUPLEX_HALF;
      i++;
   }
   if (mask & SUPPORTED_10baseT_Full) {
      connectInfo->supportedModes[i].speed = 10;
      connectInfo->supportedModes[i].duplex = VMK_LINK_DUPLEX_FULL;
      i++;
   }
   if (mask & SUPPORTED_100baseT_Half) {
      connectInfo->supportedModes[i].speed = 100;
      connectInfo->supportedModes[i].duplex = VMK_LINK_DUPLEX_HALF;
      i++;
   }
   ...
   if (mask & SUPPORTED_1000baseT_Half) {
      connectInfo->supportedModes[i].speed = 1000;
      connectInfo->supportedModes[i].duplex = VMK_LINK_DUPLEX_HALF;
      i++;
   }
   ...
   if (mask & SUPPORTED_10000baseT_Full) {
      connectInfo->supportedModes[i].speed = 10000;
      connectInfo->supportedModes[i].duplex = VMK_LINK_DUPLEX_FULL;
      i++;
   }
    ...
    */
   if (i == 0) {
      VMKLNX_WARN("No supported speed/duplex found in %s, mask %x", dev->name, mask);
      return VMK_FAILURE;
   }

   connectInfo->supportedModesArraySz = i;
...
{                                                \
   ...
   startPanicTxImmediate: StartPanicTxImmediate, \
   open:                  OpenNetDev,            \
   close:                 CloseNetDev,           \
   panicPoll:             PanicPoll,             \
...
LinNet_ConnectUplink(struct net_device *dev, struct pci_dev *pdev)
{
   ...
   connectInfo.supportedModes = supportedModes;
   if (GetNICSupportedModes(dev, &connectInfo) != VMK_OK) {
      goto fail;
   }
   VMK_ASSERT(connectInfo.supportedModesArraySz <= 16);
...
{ 
   ...
   list_for_each_entry_safe(eventCB_task, eventCB_temp, &temp_list, head) {
      ...
      eventCB->devCB((uint8_t *)macAddr, eventCB_task->event, 
                                         eventCB->devData);
cb_cleanup:

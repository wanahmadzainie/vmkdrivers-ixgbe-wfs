...
/* Checksums values for non-encap packets */
#define CHECKSUM_NONE              0  // Used in Tx/Rx paths, no csum is 
                                      // requested/done on hdrs.
#define CHECKSUM_HW                1  // Used in Tx path, HW cscum is 
                                      // requested on the hdrs.
#define CHECKSUM_UNNECESSARY       2  // Used in Rx path, IP/L4  hdr csums 
                                      // validated by hw.

/* Checksums values for encap packets */
#define CHECKSUM_NONE_ENCAP        3  // Used in Tx/Rx paths, no csum is
                                      // requested/done of outer/inner hdrs.
#define CHECKSUM_HW_ENCAP          4  // Used in Tx path, HW csum requested 
                                      // both of inner/outer hdrs.
#define CHECKSUM_UNNECESSARY_ENCAP 5  // Used in Rx path, both outer and
                                      // inner hdr csums validated by hw.
 ...
 *      @queue_mapping: Queue mapping of this skb
...
{
        return vmklnx_net_alloc_skb(THIS_MODULE->skb_cache, size, NULL, priority);
}
#else /* !defined(__VMKLNX__) */
 ...
 *
 *  ESX Deviation Notes:
 *  Callers must not assume that the returned sk_buff's 'cb'
...
/* _VMKLNX_CODECHECK_: alloc_skb_fclone */
 ...
 */
/* _VMKLNX_CODECHECK_: skb_queue_empty */
...
/*
 * vmklinux doesn't support properly skb cloning and data reference counter.
 ...
 */
/* _VMKLNX_CODECHECK_: skb_queue_head_init */
 ...
 *	A buffer cannot be placed on two lists at the same time.
 */
/* _VMKLNX_CODECHECK_: __skb_queue_head */
 ...
 *	A buffer cannot be placed on two lists at the same time.
 */
/* _VMKLNX_CODECHECK_: __skb_queue_tail */
 ...
 * the list known..
 */
/* _VMKLNX_CODECHECK_: __skb_unlink */
...
/* _VMKLNX_CODECHECK_: skb_is_nonlinear */
...
/**
 *	__skb_push - add data to the start of a buffer
 *	@skb: buffer to use
 *	@len: amount of data to add
 *
...
/**                                          
 *  pskb_may_pull - adjust the header in a skb
 *  @skb: buffer to adjust
 *  @len: the desired header length in bytes
 *                                           
 *  If the header length is less then @len, data
 *  will be pull in from the fragments to expand
 *  the header to @len bytes in length. @skb will
 *  be updated to reflect this data movement.
 *
 *  If the header length is already equal to or
 *  greater than @len, then @skb is not changed.
 *
 *  RETURN VALUE:
 *  Non-zero if header length is @len or longer;
 *  otherwise 0.
 */                                          
...
/* _VMKLNX_CODECHECK_: skb_transport_header */
...
/**
 *  skb_set_transport_header - Set transport header data based on skb offset data
 *  @skb: Socket buffer
 *  @offset: Offset from buffer data
 *
 *  Set skb transport header based on data from socket buffer data at offset.
 *
 *  RETURN VALUE:
 *  NONE
 *
 */
 /* _VMKLNX_CODECHECK_: skb_set_transport_header */
...
static inline unsigned char *skb_network_header(const struct sk_buff *skb)
{
 ...
 /* _VMKLNX_CODECHECK_: skb_reset_network_header */
...
/**
 *  skb_set_network_header - Set network header data based on skb offset data
 *  @skb: Socket buffer
 *  @offset: Offset from buffer data
 *
 *  Set skb network header based on data from socket buffer data at offset.
 *
 *  RETURN VALUE:
 *  NONE
 *
 */
 /* _VMKLNX_CODECHECK_: skb_set_network_header */
...
/* _VMKLNX_CODECHECK_: skb_reset_mac_header */
...
/**
 *  skb_transport_offset - Return the offset of transport header in protocol data area
 *  @skb: sk_buff
 *
 *  Returns the offset of transport header in protocol data area
 *  of @skb
 *
 *  RETURN VALUE:
 *  The offset of transport header in protocol data area of @skb
 *
 */
 /* _VMKLNX_CODECHECK_: skb_transport_offset */
...
/**
 * __pskb_trim - trim the skb to the length given
 * @skb: buffer to alter
 * @len: new length
 * 
 * Cut the length of a buffer down by removing data from the tail. If
 ...
 * 
 * RETURN VALUE:
 * Returns 0 if it was able to sucessfully trim the buffer.
 * Returns non zero in case of error.
 */
/* _VMKLNX_CODECHECK_: __pskb_trim */
...
/**
 * pskb_trim - trim the skb to the length given
 * @skb: buffer to alter
 * @len: new length
 * 
 * Cut the length of a buffer to @len by removing data from the tail. If
 * the buffer is already under the length specified, it is not modified.
 * 
 * RETURN VALUE:
...
/**
 *  skb_get_queue_mapping - Get index of queue the skb is placed in
 *  @skb: Pointer to socket buffer
 *
 *  Get index of queue @skb is placed in
 *
 *  RETURN VALUE:
 *  Queue index
 */
/* _VMKLNX_CODECHECK_: skb_get_queue_mapping */

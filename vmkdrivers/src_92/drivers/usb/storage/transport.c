		...
		unsigned int length, int result, unsigned int partial)
{
	...
	case -EPIPE:
		...
		_VMKLNX_USB_STOR_MSG("clearing endpoint halt for pipe 0x%x\n", us->srb, pipe);
	...
	case -EREMOTEIO:
		_VMKLNX_USB_STOR_MSG("-- short read transfer\n", us->srb);
	...
	void *buf, unsigned int length, unsigned int *act_len)
{
	...
	if (!in_interrupt() &&
		...
		dev_info(&us->pusb_intf->dev,
			"Sleeping for 2 seconds to delay USB SCSI transfer");
		ssleep(2);
...
#if defined(__VMKLNX__)
/*
 * Transfer a scatter-gather list via series of bulk transfers
 *
 * This function iterates supplied scatter-gather list and calls
 * usb_stor_bulk_transfer_buf() on each segment and so is suitable
 * for transferring data when we dumping core to USB disk.
 */
static int usb_stor_bulk_transfer_sglist_dump(struct us_data *us,
		unsigned int pipe, struct scatterlist *sglist, int num_sg,
		unsigned int length, unsigned int *act_len)
{
	struct scatterlist *sg;
	unsigned int buf_act_len;
	...
	for_each_sg(sglist, sg, num_sg, i) {
...
#if defined(__VMKLNX__)
	if (unlikely(scsi_dump_active(us->srb))) {
		return usb_stor_bulk_transfer_sglist_dump(us, pipe, sg, num_sg,
							  length, act_len);
	}
#endif
		      ...
		      struct scsi_cmnd* srb)
{
	...
	if (srb->use_sg) {
		...
		result = usb_stor_bulk_transfer_buf(us, pipe, srb->request_buffer, 
				      scsi_bufflen(srb), &partial);
	}
#else
...
Retry_Sense:
	...
	if ((srb->result == SAM_STAT_GOOD || srb->sense_buffer[2] == 0) &&
		...
		if (unlikely(srb->cmnd[0] == MODE_SENSE || srb->cmnd[0] == MODE_SENSE_10)) {
			memcpy(srb->sense_buffer,
				usb_stor_sense_invalidCDB,
				sizeof(usb_stor_sense_invalidCDB));
...
int usb_stor_Bulk_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	if (!in_interrupt() &&
		...
		// XXX make sleep duration a random value?
		dev_info(&us->pusb_intf->dev,
			"Sleeping for 5 seconds to delay USB SCSI data phase");
		ssleep(5);
	...
	_VMKLNX_USB_STOR_WARN("ERROR: fell thru switch in %s", us->srb, __func__);
	return USB_STOR_TRANSPORT_ERROR;
}

...
static kmem_cache_t *scsi_pkt_cachep;
...
#if defined(__VMKLNX__)
static inline int vmklnx_fc_remote_port_chkready(struct fc_rport *rport);
...
#if defined(__VMKLNX__)
unsigned int rec_tov = 2 * HZ;
module_param_named(rec_tov, rec_tov, int, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(rec_tov, "REC timeout value");
#endif /* __VMKLNX__ */
...
#if defined(__VMKLNX__)
#define FC_SCSI_REC_TOV		((rec_tov > 2 * HZ) ? rec_tov : (2 * HZ))
#else /* __VMKLNX__ */
...
#if defined(__VMKLNX__)
/* _VMKLNX_CODECHECK_: libfc_lun_qdepth */
short libfc_lun_qdepth = FC_FCP_DFLT_QUEUE_DEPTH;
EXPORT_SYMBOL(libfc_lun_qdepth);
#endif /* __VMKLNX__ */
...
#if defined(__VMKLNX__)
	/*
	 * Our intention is to let scsi ml to retry the cmd.
	 * In ESX DID_ERROR is for non-retriable error and DID_BUS_BUSY is
	 * for retriable error. Hence return FC_CMD_RECOVERY here so the cmd
	 * will be returned as BUSY and be retried.
	 */
...
void fc_fcp_ddp_setup(struct fc_fcp_pkt *fsp, u16 xid)
{
	...
	if ((fsp->req_flags & FC_SRB_READ) &&
		...
		sc = fsp->cmd;
		if (!sc) {
			printk("fc_fcp_ddp_setup(): fsp->cmd is NULL\n");
			return;
		}
	
		if (sc->use_sg) {
			sgl = scsi_sglist(sc);
			sgc = scsi_sg_count(sc);
		} else if (sc->request_buffer && sc->request_bufflen) {
			sgl = &(fsp->sg);

			/*
			* Confirm that fsp's SG members have been
			* initialized via fc_queuecommand.
			*/
			VMK_ASSERT((sgl->vmksgel == sgl->cursgel) &&
			           (sgl->cursgel == &(fsp->vmksgel)) &&
			           (sgl->cursgel->length == sc->request_bufflen));

			sgc = 1;
		} else {
			printk("fc_fcp_ddp_setup(): invalid request buffer 0x%lx, len = %d\n",
			       (unsigned long) sc->request_buffer, sc->request_bufflen);
			return;
		}
		if (lport->tt.ddp_setup(lport, xid, sgl, sgc))
				fsp->xfer_ddp = xid;
#endif /* !defined(__VMKLNX__) */
	}
}
			    ...
			    size_t offset, size_t seq_blen)
{
	...
	if (sc->use_sg) {
		 ...
		 * initialized via fc_queuecommand.
		 */
		VMK_ASSERT((sg->vmksgel == sg->cursgel) &&
			   (sg->cursgel == &(fsp->vmksgel)) &&
			   (sg->cursgel->length == sc->request_bufflen));
	}
#endif /* !defined(__VMKLNX__) */
	...
	while (remaining > 0 && sg) {
		...
		if (!fp) {
			...
			if (!fp) {
#if defined(__VMKLNX__)
				FC_FCP_DBG(fsp, "%s: fc_frame_alloc() failed, "
					   "tlen = %lu, using_sg = %d\n",
					   __FUNCTION__, tlen, using_sg )
				sg_reset(sg);
#endif /* defined(__VMKLNX__) */
				return -ENOMEM;
			}
		...
		sg_bytes = min(tlen, sg_dma_len(sg) - offset);
		sg_bytes = min(sg_bytes,
			       (size_t) (PAGE_SIZE - (off & ~PAGE_MASK)));

		page = nth_page(sg_page(sg), (off >> PAGE_SHIFT));
#else /* !defined(__VMKLNX__) */
	...
	if (unlikely((flags & ~FCP_CONF_REQ) || fc_rp->fr_status)) {
		...
		if (flags & (FCP_RESID_UNDER | FCP_RESID_OVER)) {
			...
			if (flags & FCP_RESID_UNDER) {
				...
				/* OVERRUN is permanent error in ESX */
				fsp->status_code = FC_ERROR;
			}
		}
	}
	...
	if (fsp->state & FC_SRB_ABORTED) {
		...
		if (fsp->xfer_len < fsp->data_len && !fsp->io_status &&
		    (!(fsp->scsi_comp_flags & FCP_RESID_UNDER) ||
		     fsp->xfer_len < fsp->data_len - fsp->scsi_resid)) {
			FC_FCP_DBG(fsp, "fsp->xfer_len=%d, data_len=%d, scsi_resid=%d\n",  
				(int)fsp->xfer_len, fsp->data_len, fsp->scsi_resid );
...
#if defined(__VMKLNX__)
	/*
	 * Return FC_CMD_RECOVERY here so the cmd will be
	 * returned as BUSY and be retried by ESX.
	 */
	fc_fcp_cleanup_each_cmd(lport, -1, -1, FC_CMD_RECOVERY);
#else /* defined(__VMKLNX__) */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_queuecommand */
...
#if defined(__VMKLNX__)
	rval = vmklnx_fc_remote_port_chkready(rport);
#else /* !defined(__VMKLNX__) */
...
#if defined(__VMKLNX__)
	/*
	 * Setup SG when use_sg == 0.
	 */
	if (!sc_cmd->use_sg && sc_cmd->request_buffer && sc_cmd->request_bufflen) {
		struct scatterlist *sgl;
		sgl = &(fsp->sg);
		VMKLNX_INIT_VMK_SG(sgl, &(fsp->vmksgel));
		sg_init_one(sgl, sc_cmd->request_buffer, sc_cmd->request_bufflen);
	}	
#endif /* defined(__VMKLNX__) */
...
static void fc_io_compl(struct fc_fcp_pkt *fsp)
{
...
#if defined(__VMKLNX__)
	/* dequeue fsp here because the upstream locking is broken */
	spin_lock_irqsave(&si->scsi_queue_lock, flags);
	if (fsp->list.next != LIST_POISON1) {
		list_del(&fsp->list);
		if (!fsp->cmd || !fsp->cmd->SCp.ptr) {
			printk(KERN_WARNING "%s: Null fsp->cmd or "
			       "fsp->cmd->SCp.ptr\n", __FUNCTION__);
			spin_unlock_irqrestore(&si->scsi_queue_lock, flags);
			return;
		} else {
		...
		printk(KERN_DEBUG "%s: fsp %p is not in the queue "
	...
	case FC_COMPLETE:
		if (fsp->cdb_status == 0) {
			...
			if (fsp->scsi_resid) {
				CMD_RESID_LEN(sc_cmd) = fsp->scsi_resid;
				scsi_set_resid(sc_cmd, fsp->scsi_resid);
			}
#endif /* defined(__VMKLNX__) */
		} else {
			/*
	...
	case FC_ERROR:
#if defined(__VMKLNX__)
		printk(KERN_ERR "libfc: cmd x%x failed FC_ERROR\n",
			sc_cmd->cmnd[0]);
#endif /* defined(__VMKLNX__) */
	...
	case FC_DATA_UNDRUN:
		...
		/* underflow is considered as PERM error in ESX */
		if ((sc_cmd->request_bufflen - sc_cmd->resid) <
		     sc_cmd->underflow) {
			printk(KERN_ERR "libfc: cmd x%x error underflow x%x x%x x%x\n",
				sc_cmd->cmnd[0], sc_cmd->request_bufflen,
				sc_cmd->resid, sc_cmd->underflow);
			sc_cmd->result = (DID_ERROR << 16) | fsp->cdb_status;
		} else if (fsp->cdb_status == 0) {
	...
	case FC_DATA_OVRRUN:
		 ...
		 */
#if defined(__VMKLNX__)
		printk(KERN_ERR "libfc: cmd x%x error overrun x%x x%x\n",
			sc_cmd->cmnd[0], sc_cmd->request_bufflen, sc_cmd->resid);
#endif /* defined(__VMKLNX__) */
		sc_cmd->result = (DID_ERROR << 16) | fsp->cdb_status;
		break;
	case FC_CMD_ABORTED:
	...
	default:
#if defined(__VMKLNX__)
		printk(KERN_ERR "libfc: cmd x%x failed with status code x%x\n",
			sc_cmd->cmnd[0], fsp->status_code);
#endif /* defined(__VMKLNX__) */
		sc_cmd->result = (DID_ERROR << 16);
		break;
	}
	...
	FC_FCP_DBG(fsp, "SCSI cmd=%x, result=%x\n", sc_cmd->cmnd[0], sc_cmd->result );
 ...
 */
/* _VMKLNX_CODECHECK_: fc_eh_abort */
...
#if defined(__VMKLNX__)
        /* hold scsi_queue_lock to protect the fsp->cmd in same lock */
	si = fc_get_scsi_internal(lport);
	spin_lock_irqsave(&si->scsi_queue_lock, flags);
	fsp = CMD_SP(sc_cmd);
	if (!fsp) {
...
/**
 * vmklnx_fc_remote_port_chkready - called to validate the remote port state
 *                                  prior to initiating I/O to the port.
 *
 * @rport: remote ported to be validated.
 *
 * This is a substitute of the function fc_remote_port_chkready(). Wherever
 * the function fc_remote_port_chkready() is called, it should be replaced
 * by the function vmklnx_fc_remote_port_chkready.
 *
 * The reason having this substitute is because when a rport is removed and
 * being created again due to link state change, there is a chance that rport
 * init has finished at transport layer, but the rport private data has not
 * been initialized yet. If IO comes in , calling fc_remote_chkready() cannot
 * guarentee the rport is really ready even it says ready, thus we need this
 * substitute.
 *
 * Note that the upstream may take this workaround in future. If so, we will
 * delete this function.
 *
 * RETURN VALUE:
 * Returns 0 if rport is ready, otherwise a DID_xxx error.
 *
 */
static inline int vmklnx_fc_remote_port_chkready(struct fc_rport *rport)
{
...
struct vmklnx_device_reset {
        struct scsi_device *sdev;
        struct list_head   list;
};

#define LIBFC_LUN_RESET 	0x10
#define LIBFC_TARGET_RESET 	0x20
#define LIBFC_BUS_RESET 	0x40

/**
 * vmklnx_fc_eh_lun_reset() - Reset a single LUN
 * @sc_cmd: The SCSI command which identifies the device whose
 *	    LUN is to be reset
 *
 * Set from SCSI host template.
 */
static int vmklnx_fc_eh_lun_reset(struct scsi_cmnd *sc_cmd)
{
	struct fc_lport *lport;
	struct fc_fcp_pkt *fsp;
	struct fc_rport *rport = starget_to_rport(scsi_target(sc_cmd->device));
...
/**
 * vmklnx_fc_eh_multilun_reset() - Reset multiple LUNs
 * 
 * @sc_cmd: The SCSI command which identifies the device whose
 *	    TARGET(s) is to be reset
 * @type:   The type of reset 
 */
static int vmklnx_fc_eh_multilun_reset(struct scsi_cmnd *sc_cmd, u8 type)
{
        ...
        struct vmklnx_device_reset *sdev_reset, *temp_reset;

	BUG_ON(type != LIBFC_TARGET_RESET && type != LIBFC_BUS_RESET);
        ...
        /*
         * Traverse through the list and obtain list of devices
         * that are to be reset. Add them to the device list while
         * taking a reference count
         */
	spin_lock_irqsave(shost->host_lock, flags);
        shost_for_each_device(sdev, shost) {
                if ((sdev->channel == sc_cmd->device->channel) &&
                    (!scsi_device_get(sdev))) {
			if(type == LIBFC_TARGET_RESET &&
			   (sdev->id != sc_cmd->device->id))
				continue;

                        sdev_reset = (struct vmklnx_device_reset *)
                                kzalloc(sizeof(*sdev_reset), GFP_ATOMIC);       
                        if (sdev_reset) {
                                INIT_LIST_HEAD(&sdev_reset->list);
                                /*
                                 * Since we have reference we should
                                 * be good
                                 */
                                sdev_reset->sdev = sdev;
                                list_add_tail(&sdev_reset->list,
                                                &device_list);
         ...
         */
        if (rc != FAILED) {
                list_for_each_entry(sdev_reset, &device_list, list) {
                        sc_cmd->device = sdev_reset->sdev;
                        if (vmklnx_fc_eh_lun_reset(sc_cmd) == FAILED) {
                                rc = FAILED;
                        }
                }
        }
         ...
         * Free up the memory
         */
        list_for_each_entry_safe(sdev_reset, temp_reset, &device_list, list) {
                scsi_device_put(sdev_reset->sdev);
                list_del(&sdev_reset->list);
                kfree(sdev_reset);
        }
...
/**
 * fc_eh_device_reset() - Reset a single LUN or a single TARGET
 * @sc_cmd: The SCSI command which identifies the device whose
 *	    LUN/TARGET is to be reset
 *
 * Set from SCSI host template.
 *
 * ESX Deviation Notes:
 * Both TARGET reset and LUN reset are supported in ESX.
 */
/* _VMKLNX_CODECHECK_: fc_eh_device_reset */
int fc_eh_device_reset(struct scsi_cmnd *sc_cmd)
{
	int rc;
	 ...
	 * flush outstanding commands
	 */
	if (sc_cmd->vmkflags & VMK_FLAGS_USE_LUNRESET) {
		rc = vmklnx_fc_eh_lun_reset(sc_cmd);
	} else {
		rc = vmklnx_fc_eh_multilun_reset(sc_cmd, LIBFC_TARGET_RESET);
	}
...
/**
 * vmklnx_fc_eh_bus_reset: Reset all luns on a given bus
 * @sc_cmd:     scsi command
 * 
 * Set from scsi host template to send tm cmd to the target
 * In case of FCoE individual LUN resets will be sent
 *
 * ESX Deviation Notes:
 * This API is not present in Linux.
 */
/* _VMKLNX_CODECHECK_: vmklnx_fc_eh_bus_reset */
int vmklnx_fc_eh_bus_reset(struct scsi_cmnd *sc_cmd)
{
        return vmklnx_fc_eh_multilun_reset(sc_cmd, LIBFC_BUS_RESET);
}

EXPORT_SYMBOL(vmklnx_fc_eh_bus_reset);
#endif /* defined(__VMKLNX__) */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_eh_host_reset */
...
#if defined(__VMKLNX__)
/**
 * fc_slave_configure() - Configure the queue depth of a Scsi_Host
 * @sdev: The SCSI device that identifies the SCSI host
 *
 * Configures queue depth based on device type (tagged or not).
 *
 * ESX Deviation Notes:
 * This API is not present in Linux.
 */
/* _VMKLNX_CODECHECK_: fc_slave_configure */
int fc_slave_configure(struct scsi_device *sdev)
{
	struct fc_rport *rport = starget_to_rport(scsi_target(sdev));

	if (!rport || vmklnx_fc_remote_port_chkready(rport))
		return -ENXIO;

	if (sdev->tagged_supported)
		scsi_activate_tcq(sdev, libfc_lun_qdepth);
	else
 ...
 */
/* _VMKLNX_CODECHECK_: fc_slave_alloc */
 ...
 *
 * ESX Deviation Notes:
 * Parameter reason is not supported in ESX. When called, it adjusts queue
 * dpeth regardless of the reason.
 */
/* _VMKLNX_CODECHECK_: fc_change_queue_depth */
#if defined(__VMKLNX__)
 ...
 */
/* _VMKLNX_CODECHECK_: fc_change_queue_type */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_fcp_destroy */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_fcp_init */
...
#if defined(__VMKLNX__)
	si->scsi_pkt_pool = mempool_create_slab_pool(2, (struct kmem_cache *) scsi_pkt_cachep);
#else /* !defined(__VMKLNX__) */

...
static int min_exch_pool_elem = 2048;
module_param_named(min_exch_pool_elem, min_exch_pool_elem, int, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(min_exch_pool_elem, "Miminum number of elements guaranteed to be allocated for exchange pool.");
#endif /* __VMKLNX__ */

/* _VMKLNX_CODECHECK_: fc_cpu_mask */
u16	fc_cpu_mask;		/* cpu mask for possible cpus */
EXPORT_SYMBOL(fc_cpu_mask);
static u16	fc_cpu_order;	/* 2's power to represent total possible cpus */
#if defined(__VMKLNX__)
static struct kmem_cache_s *fc_em_cachep;       /* cache for exchanges */
					...
					struct fc_exch_mgr *mp)
{
	...
	cpu = get_cpu();
#if defined(__VMKLNX__)
	pool = FCOE_PER_CPU_PTR(mp->pool, cpu, FCOE_EXCH_POOL_SIZE(mp));
#else
					    ...
					    struct fc_frame *fp)
{
	...
	struct fc_exch *exchange = NULL;

	list_for_each_entry(ema, &lport->ema_list, ema_list)
		if (!ema->match || ema->match(fp))
		{
			exchange = fc_exch_em_alloc(lport, ema->mp);
			if(exchange)
				return exchange;
		}
...
static struct fc_exch *fc_exch_find(struct fc_exch_mgr *mp, u16 xid)
{
	...
	if ((xid >= mp->min_xid) && (xid <= mp->max_xid)) {
		pool = FCOE_PER_CPU_PTR(mp->pool, xid & fc_cpu_mask, FCOE_EXCH_POOL_SIZE(mp));
#if defined(__VMKLNX__)
		/*
		 * Make sure the pool is valid since the CPU ID is derived
		 * from the OX_ID.
		 */
		if (!pool) {
			return NULL;
 ...
 */
/* _VMKLNX_CODECHECK_: fc_exch_mgr_reset */
	...
	list_for_each_entry(ema, &lport->ema_list, ema_list) {
		for_each_possible_cpu(cpu)
			fc_exch_pool_reset(lport,
					   FCOE_PER_CPU_PTR(ema->mp->pool, cpu, FCOE_EXCH_POOL_SIZE(ema->mp)),
					   sid, did);
	}
}
 ...
 */
/* _VMKLNX_CODECHECK_: fc_exch_mgr_add */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_exch_mgr_del */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_exch_mgr_list_clone */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_exch_mgr_alloc */
				      ...
				      bool (*match)(struct fc_frame *))
{
...
#if defined(__VMKLNX__)
	mp->ep_pool = mempool_create_slab_pool(min(min_exch_pool_elem, max_xid - min_xid + 1),
					       (struct kmem_cache *) fc_em_cachep);
#else /* !defined(__VMKLNX__) */
	...
	mp->pool = fcoe_alloc_percpu(pool_size, __alignof__(struct fc_exch_pool));

	if (!mp->pool)
		goto free_mempool;
	for_each_possible_cpu(cpu) {
		pool = FCOE_PER_CPU_PTR(mp->pool, cpu, pool_size);
 ...
 */
/* _VMKLNX_CODECHECK_: fc_exch_mgr_free */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_exch_recv */
 ...
 */
/* _VMKLNX_CODECHECK_: fc_exch_init */

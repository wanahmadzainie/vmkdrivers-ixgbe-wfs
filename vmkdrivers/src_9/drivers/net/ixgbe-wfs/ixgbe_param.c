 ...
 */
IXGBE_PARAM(CNA, "Number of FCoE Queues: 0 = disable, "
	    "1-8 = enable this many Qs");

#ifdef __VMKLNX__
/* Enable/disable ESX Large Receive Offload
 *
 * Valid Values: 0(off), 1(on)
 *
 * Default Value: 0
 */
IXGBE_PARAM(LRO, "Large Receive Offload (ESX5.1 or later): (0,1), default 0 = off");
#else
...
void __devinit ixgbe_check_options(struct ixgbe_adapter *adapter)
{
...
#ifdef __VMKLNX__
		int ret = ixgbe_compare_esx_product_version(ESX51_PRODUCT_VER);
		if (ret < 0) {
			feature[RING_F_RSS].limit = 1;
			DPRINTK(PROBE, ERR,
				"This kernel version does not support RSS\n");
		}
		if (feature[RING_F_RSS].limit == IXGBE_ESX_RSS_QUEUES) {
			...
			case ixgbe_mac_X540:
				...
				/* Explicitly enable UDP RSS flags since
				 * they are turn off by default.
				 */
				adapter->flags2 |=
					IXGBE_FLAG2_RSS_FIELD_IPV4_UDP |
					IXGBE_FLAG2_RSS_FIELD_IPV6_UDP;

				/* do not use Flow Director
				 * when RSS is enabled
				 */
	...
	}
	{
		...
		if (adapter->hw.mac.type != ixgbe_mac_82598EB) {
			...
			if (feature[RING_F_RSS].limit == IXGBE_ESX_RSS_QUEUES) {
				cna_qs = 0;
				DPRINTK(PROBE, INFO, "CNA turned off when RSS "
				" is enabled thru mod_param\n");
			}
			if (cna_qs)
				adapter->flags2 |= IXGBE_FLAG2_CNA_ENABLED;

			feature[RING_F_FCOE].limit = cna_qs;
	...
	{ /* Virtual Machine Device Queues (VMDQ) */
		...
		case ixgbe_mac_X540:
			/*
			 * VMware ESX performance tests show that optimal
			 * perf is achieved if num_queues is less than 1/4
			 * the number of CPUs, but no less than 4.
			 */
			if (num_online_cpus() < 32)
				opt.def = 4;
			break;
#endif /* __VMKLNX__ */
		default:
...
#endif
#ifdef __VMKLNX__
		if (ixgbe_compare_esx_product_version(ESX51_PRODUCT_VER) < 0) {
			adapter->num_vfs = 0;
			*aflags &= ~IXGBE_FLAG_SRIOV_ENABLED;
			DPRINTK(PROBE, ERR,
			"SR-IOV is only supported in ESX5.1 or later. Disabling SR-IOV\n");
		}
#endif /* __VMKLNX__ */
...
#ifdef __VMKLNX__
			/*
			 * ESX5.1: Uplink is not usable by the hypervisor, so
			 * disable VMDQ, RSS, RSC, and DCB, CNA, FCOE
			 */
				adapter->flags2 &= ~IXGBE_FLAG2_CNA_ENABLED;
				adapter->flags &= ~IXGBE_FLAG_FCOE_ENABLED;
				feature[RING_F_FCOE].limit = 0;
				DPRINTK(PROBE, INFO,
					"CNA turned off when SR-IOV is enabled.\n");
			feature[RING_F_VMDQ].limit = 0;
			*aflags &= ~IXGBE_FLAG_VMDQ_ENABLED;
			feature[RING_F_RSS].limit = 1;
	...
	{ /* LRO - Enable Large Receive Offload */
		...
		if (ret < 0) {
			...
			adapter->flags2 &= ~IXGBE_FLAG2_RSC_CAPABLE;
			DPRINTK(PROBE, ERR,
				"This kernel version does not support LRO\n");
		} 
		else if (vmdq_default_override == true ||
				feature[RING_F_RSS].limit > 1) {
			netdev->features &= ~NETIF_F_LRO;
		}
		...
		if (netdev->features & NETIF_F_LRO) {
			/*
			 * Allocate 8 queues to have enough resreved
			 * for HW LRO and legacy queues.
			 */
			feature[RING_F_VMDQ].limit = 8;
		}
#endif /* __VMKLNX__ */
	}
#ifndef __VMKLNX__

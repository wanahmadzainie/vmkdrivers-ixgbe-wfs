						...
						void *pargs)
{
	...
	case VMK_NETPTOP_VF_SET_MAC:
	{
		...
		if (is_zero_ether_addr(args->mac)) {
			...
			e_info(probe, "Setting ndo MAC addr"
					" %02x:%02x:%02x:%02x:%02x:%02x"
					" for vf %u\n",
					args->mac[0], args->mac[1],
					args->mac[2], args->mac[3],
					args->mac[4], args->mac[5], args->vf);
...
static int ixgbe_set_vf_lpe(struct ixgbe_adapter *adapter, u32 max_frame, u32 vf)
{
	...
	if (adapter->hw.mac.type == ixgbe_mac_82599EB) {
#ifdef __VMKLNX__
		if (ixgbe_compare_esx_product_version(ESX55_PRODUCT_VER) < 0) {
			e_err(drv, "This kernel version does not support VF MTU\n");
			return -EINVAL;
		} 
#endif /* __VMKLNX__ */

...
#ifdef bool
#undef bool /* bool is defined for scsi layer */
#endif
...
#else
	ddp_pool = ESX_PER_CPU_PTR(fcoe->ddp_pool, get_cpu(),
		   ...
		   struct sk_buff *skb)
{
	...
	/* on ESX, skb->data points to the beginning of MAC header. So add
	 * that offset.
	 */
...
#else
	fcoe->ddp_pool = ESX_ALLOC_PERCPU(struct ixgbe_fcoe_ddp_pool);
#endif

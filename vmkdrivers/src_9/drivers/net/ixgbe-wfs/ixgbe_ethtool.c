...
static struct ixgbe_stats ixgbe_gstrings_stats[] = {
	...
	IXGBE_STAT("rx_header_split", rx_hdr_split),
...
#ifdef __VMKLNX__
extern void ixgbe_print_regs(struct ixgbe_adapter *adapter);
#endif
...
#ifdef __VMKLNX__
	/* rx ring is restricted to avoid heap issues in Jumbo and MQ case */
	if ((adapter->num_rx_queues > 1) &&
	    (adapter->netdev->mtu > ETH_DATA_LEN))
		ring->rx_max_pending = min(IXGBE_MAX_RXD /
...
#ifdef __VMKLNX__
	/* Adjust the rx ring size to avoid heap issues in Jumbo mode only */
	if ((adapter->netdev->mtu > ETH_DATA_LEN) &&
		(adapter->rx_ring && adapter->num_rx_queues > 1)) {
		int count = ixgbe_calculate_rx_ring_size(adapter);
...
#if defined(ESX41) && defined(__VMKLNX__)
		if (if_running) {
			dev_open(netdev);
			/* Invalidate netqueue state as filters have been
			 * lost after reinit */
			vmknetddi_queueops_invalidate_state(netdev);
			/* PR 451198: While the device is opened, the link
			 * state is set to BLOCKED. Clear the bit here so
			 * the stack knows that device is up.
			 */
			VMK_ASSERT(test_bit(__LINK_STATE_BLOCKED,
					    &netdev->state));
			clear_bit(__LINK_STATE_BLOCKED, &netdev->state);
		}
#else
		if (if_running)
			dev_open(netdev);
...
static bool is_cim_supported(struct ixgbe_adapter *adapter)
{
	/* The CIM funcionality is only supported on HP devices */
	if (adapter->hw.subsystem_vendor_id == 0x103C)
		return true;
	else
		return false;
}

static bool validate_smbios_entry(const struct smbios_structure_table *tbl,
					struct ixgbe_adapter *adapter)
{
	int size = tbl->EntryPointLength;
	int offset;
	u8 *byte;
	u8 csum;
	...
	byte = (unsigned char *)tbl;
	csum = 0;
	for (offset = 0; offset < size; ++offset)
		csum += byte[offset];

	if (csum != 0) {
		DPRINTK(DRV, ERR, "SMBIOS Checksum failed\n");
		return false;
	}
...
static struct smbios_structure_table *
find_smbios_entry(struct smbios_structure_table *stbl, struct ixgbe_adapter *adapter)
{
	void *vbase;
	u32 offset;
	u32 size = SM_ADDR_HIGH - SM_ADDR_LOW + 1;
	struct smbios_structure_table *tbl = 0;

	vbase = ioremap(SM_ADDR_LOW, size);
	if (!vbase) {
		DPRINTK(DRV, ERR, "Failed to map SMBIOS memory region\n");
		return 0;
	}

	for (offset = 0; offset < size; offset += 0x10) {
		tbl = (struct smbios_structure_table *)(vbase + offset);
		if (!memcmp(tbl->AnchorString, sm_anchor, sizeof(sm_anchor)) &&
			validate_smbios_entry(tbl, adapter))
			break;
		tbl = 0;
	}

	if (!tbl || !stbl || !tbl->TableAddress || !tbl->TableLength) {
		adapter->SmbTblLen = 0;
		adapter->SmbTblAddr = 0;
		return 0;
	} else {
		adapter->SmbTblLen = tbl->TableLength;
		adapter->SmbTblAddr = tbl->TableAddress;
		memcpy(stbl, tbl, sizeof(*stbl));
	}
...
static bool get_smbios_tables(void *smbios, struct ixgbe_adapter *adapter)
{
	void *vbase;
	...
	vbase = ioremap(adapter->SmbTblAddr, (u32)adapter->SmbTblLen);
	if (!vbase) {
		DPRINTK(DRV, ERR, "Failed to map SMBIOS memory\n");
		return false;
	}

	memcpy(smbios, vbase, adapter->SmbTblLen);
...
static int find_ext_cap(struct pci_dev *pdev, uint16_t cap)
{
	int res;
	uint16_t cap_off;
	uint32_t dword;

	for (cap_off = 0x100; cap_off; cap_off = PCI_EXT_CAP_NEXT(dword)) {
		res = pci_read_config_dword(pdev, cap_off & ~3, &dword);
		if (res < 0)
		        return res;

		if (PCI_EXT_CAP_ID(dword) == cap)
			return cap_off;
	}
	return -ENODEV;
}

static int get_pcie_error_info(u32 *pcie_errors, u32 num_regs,
				struct ixgbe_adapter *adapter)

{
	...
	if (num_regs == 0) {
		DPRINTK(DRV, ERR, "num_regs is zero\n");
		return -EINVAL;
	}

	cap = find_ext_cap(adapter->pdev, PCI_EXT_CAP_ID_ERR);
	if (cap <= 0) {
		DPRINTK(DRV, ERR, "PCIe err cap not found\n");
		res = -ENODEV;
		return res;
	}

	for (ix = 0; ix < num_regs; ++ix) {
		res = pci_read_config_dword(adapter->pdev,
				cap + PCI_ERR_HEADER_LOG + ix * sizeof(u32),
				&pcie_errors[ix]);
		if (res < 0)
			return res;
	}
...
static int get_link_status(u16 *link_status, struct ixgbe_adapter *adapter)
{
	int cap;
	int res;

	cap = pci_find_capability(adapter->pdev, PCI_CAP_ID_EXP);
	if (cap <= 0) {
		DPRINTK(DRV, ERR, "PCIe cap not found, cap=%d\n", cap);
		res = -ENODEV;
		return res;
	}

	res = pci_read_config_word(adapter->pdev,
				cap + PCI_EXP_LNKSTA, link_status);
	if (res < 0) {
		DPRINTK(DRV, ERR, "pci_read_config_word failed with %d\n", res);
		return res;
	}
	return 0;
}
...
int ixgbe_intelcim_ioctl(struct net_device *netdev, struct ifreq *ifr)
{
	...
	useraddr += sizeof(req.cmd);

	switch (req.cmd) {
	case INTELCIM_ENUMDIAGS:
		{
			struct ethtool_gstrings gstrings;
			u8 *data;

			DPRINTK(DRV, DEBUG, "INTELCIM_ENUMDIAGS ioctl called\n");

			gstrings = req.cmd_req.gstrings;
			gstrings.len = ixgbe_diag_test_count(netdev);

			data = kmalloc(gstrings.len * ETH_GSTRING_LEN,
					GFP_USER);
			if (!data) {
				DPRINTK(DRV, ERR, "Cannot allocate memory\n");
				return -ENOMEM;
			}

			ixgbe_get_strings(netdev, gstrings.string_set, data);
			if (copy_to_user(useraddr, &gstrings, sizeof(gstrings)))
			{
				DPRINTK(DRV, ERR,
					"Cannot copy to user memory\n");
				kfree(data);
				return -EFAULT;
			}
		...
		break;
	case INTELCIM_RUNDIAG:
		{
			struct ethtool_test test;
			u64 *data;

			DPRINTK(DRV, DEBUG, "INTELCIM_RUNDIAG ioctl called\n");

			test = req.cmd_req.test;
			test.len = ixgbe_diag_test_count(netdev);
			...
			ixgbe_diag_test(netdev, &test, data);
		...
		break;
	case INTELCIM_FNDSMB:
		{
			struct smbios_structure_table smbtbl;

			DPRINTK(DRV, DEBUG, "INTELCIM_FNDSMB ioctl called\n");

			smbtbl = req.cmd_req.tbl;

			if (!is_cim_supported(adapter))
				return -EPERM;

			if (!find_smbios_entry(&smbtbl, adapter)) {
				DPRINTK(DRV, ERR, "SMBIOS table not found\n");
				return -EFAULT;
			} else {
				if (copy_to_user(useraddr,
						&smbtbl, sizeof(smbtbl)))
				{
					DPRINTK(DRV, ERR,
						"Cannot copy to user memory\n");
                                	return -EFAULT;
		...
		break;
	case INTELCIM_GETSMBTBL:
		{
			void *smbios;

			DPRINTK(DRV, DEBUG, "INTELCIM_GETSMBTBL ioctl called\n");

			if (!is_cim_supported(adapter))
				return -EPERM;

			smbios = kmalloc(adapter->SmbTblLen * sizeof(u8),
						GFP_USER);
			if (!smbios) {
				DPRINTK(DRV, ERR, "Cannot allocate memory\n");
				return -ENOMEM;
			...
			if (!get_smbios_tables(smbios, adapter)) {
				DPRINTK(DRV, ERR, "Cannot get SMBIOS tables\n");
				kfree(smbios);
				return -EFAULT;
			}

			if (copy_to_user(useraddr, smbios,
				adapter->SmbTblLen * sizeof(u8))) {
				DPRINTK(DRV, ERR,
					"Cannot copy to user memory\n");
				kfree(smbios);
				return -EFAULT;
		...
		break;
	case INTELCIM_WRITEMEM:
		{
			struct intelcim_mem_buf buf;
			void *data;
			void *vbase;

			DPRINTK(DRV, DEBUG, "INTELCIM_WRITEMEM ioctl called\n");
			...
			data = kmalloc(buf.len * sizeof(u8), GFP_USER);
			if (!data) {
				DPRINTK(DRV, ERR, "Cannot allocate memory\n");
				return -ENOMEM;
			}

			useraddr += sizeof(buf.addr) + sizeof(buf.len);
			if (copy_from_user(data, useraddr,
						buf.len * sizeof(u8))) {
				DPRINTK(DRV, ERR,
					"Cannot copy memory from user space\n");
				kfree(data);
				return -EFAULT;
			}

			vbase = ioremap(buf.addr, buf.len);
			if (!vbase) {
				DPRINTK(DRV, ERR,
					"Failed to map buffer memory\n");
				return -EFAULT;
			}
			memcpy(vbase, data, buf.len);
			iounmap(vbase);
			kfree(data);
		}
		break;
	case INTELCIM_READMEM:
		{
			struct intelcim_mem_buf buf;
			void *data;
			void *vbase;

			DPRINTK(DRV, DEBUG, "INTELCIM_READMEM ioctl called\n");
			...
			memcpy(data, vbase, buf.len);

			useraddr += sizeof(buf.addr) + sizeof(buf.len);
			if (copy_to_user(useraddr, data,
						buf.len * sizeof(u8))) {
				DPRINTK(DRV, ERR,
				"Cannot copy mem contents to user space\n");
				kfree(data);
				return -EFAULT;
			}
		...
		break;
	case INTELCIM_GET_PCIE_ERROR_INFO:
		{
			struct intelcim_pcie_error_info info;
			void *pcie_errors;

			DPRINTK(DRV, DEBUG,
				"INTELCIM_GET_PCIE_ERROR_INFO ioctl called\n");

			info = req.cmd_req.info;

			if (!is_cim_supported(adapter))
				return -EPERM;

			pcie_errors = kmalloc(info.num_regs * sizeof(u32),
						GFP_USER);
			if (!pcie_errors) {
				DPRINTK(DRV, ERR, "Cannot allocate memory\n");
				return -ENOMEM;
			...
			if((get_pcie_error_info((u32*)pcie_errors,
						(u32)info.num_regs,
						adapter)) < 0) {
				DPRINTK(DRV, ERR,
					"Cannot get pcie error info\n");
				kfree(pcie_errors);
				return -EFAULT;
			}

			useraddr += sizeof(info.num_regs);
			if (copy_to_user(useraddr, pcie_errors,
					info.num_regs * sizeof(u32))) {
				DPRINTK(DRV, ERR,
				"Cannot copy mem contents to user space\n");
				kfree(pcie_errors);
				return -EFAULT;
		...
		break;
	case INTELCIM_GET_PCI_LINK_STATUS:
		{
			u16 link_status;

			DPRINTK(DRV, DEBUG,
				"INTELCIM_GET_PCI_LINK_STATUS ioctl called\n");

			link_status = req.cmd_req.link_status;

			if (!is_cim_supported(adapter))
				return -EPERM;

			if ((get_link_status(&link_status, adapter)) < 0) {
				DPRINTK(DRV, ERR,
		...
		break;
#ifdef __VMKLNX__
	case INTELDIAG_ESX_READ_REG:
		{
			struct inteldiag_esx_reg_access era;
			struct ixgbe_adapter *adapter = netdev_priv(netdev);
			struct ixgbe_hw *hw = &adapter->hw; 
			u32 value;
			era = req.cmd_req.reg_info;			
			value = IXGBE_READ_REG(hw, era.reg_offset);
			if (copy_to_user(useraddr+sizeof(era.reg_offset),
			 &value, sizeof(era.data))) {
				DPRINTK(DRV, ERR, 
				"Cannot copy to user memory\n");
				return -EFAULT;
			}
		...
		break;
	case INTELDIAG_ESX_WRITE_REG:
		{
			struct inteldiag_esx_reg_access era;
			struct ixgbe_adapter *adapter = netdev_priv(netdev);
			struct ixgbe_hw *hw = &adapter->hw;
			DPRINTK(DRV, DEBUG,
				"INTELDIAG_ESX_WRITE_REG ioctl called\n");
			
			era = req.cmd_req.reg_info;
			IXGBE_WRITE_REG(hw, era.reg_offset, era.data);			
		}
		break;
#endif	/* __VMKLNX__ */
	default:
		DPRINTK(DRV, ERR, "Unknown ioctl\n");
	}
	return ret;
}

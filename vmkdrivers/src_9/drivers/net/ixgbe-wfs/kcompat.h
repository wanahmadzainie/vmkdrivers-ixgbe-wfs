...
#ifdef __VMKLNX__
/* this has to be defined for ESX since it is stripped from their skbuff.h */
#ifndef __skb_queue_purge
#define __skb_queue_purge(_l) __kc_skb_queue_purge(_l)
static inline void __kc_skb_queue_purge(struct sk_buff_head *list)
{
	struct sk_buff *skb;
...
#else
#define pci_disable_link_state(p, s) do {} while (0)
#endif /* __VMKLNX__ */
#else /* < 2.6.26 */
...
#ifdef __VMKLNX__
	/* VMKLNX already takes care of allocating the NET_SKB_PAD for
	 * each netdev skb allocation. It does not however include the
	 * 2 bytes for IP alignment. So, changing the call below to more
	 * accurately reflect what is done in the ESX 5.0 inbox driver
	 */

...
static bool ixgbe_cache_ring_dcb_vmdq(struct ixgbe_adapter *adapter)
{
	...
	/* ESX supports 2 TCs but only 1 TC per pool */
	tcs = 1;
...
static bool ixgbe_cache_ring_vmdq(struct ixgbe_adapter *adapter)
{
	...
	if(rss->limit > 1) {
		...
		/* Configure TX queues */
		for (i = 0; i < adapter->num_tx_queues; i++)
			adapter->tx_ring[i]->reg_idx = VMDQ_P(i) *
				IXGBE_ESX_HW_QUEUES_PER_POOL;
	}
...
static bool ixgbe_set_dcb_vmdq_queues(struct ixgbe_adapter *adapter)
{
	 ...
	 */
	if ((adapter->netdev->mtu > ETH_DATA_LEN) && (vmdq_i > 4))
		vmdq_i = 4;
	...
	if (adapter->flags & IXGBE_FLAG_FCOE_ENABLED) {
		...
		if (fcoe_i) {
			...
			/* add additional pools for CNA */
			adapter->num_rx_pools += fcoe_i;
		} else if (tcs > 1) {
...
static bool ixgbe_set_vmdq_queues(struct ixgbe_adapter *adapter)
{
	...
	if (adapter->ring_feature[RING_F_RSS].limit > 1) {
		...
		adapter->ring_feature[RING_F_RSS].indices = rss_i;
		adapter->ring_feature[RING_F_RSS].mask = IXGBE_RSS_4Q_MASK;
				       ...
				       int vectors)
{
	...
	if(!(adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)) {
		/*
		 * MIN_MSIX_COUNT includes 2 vectors for FCoE, 
		 * but we won't need them if CNA is disabled
		 */
		vector_threshold = MIN_MSIX_COUNT - MIN_MSIX_Q_FCOE_VECTORS;
	} else
#endif /* __VMKLNX__ */
...
int ixgbe_calculate_rx_ring_size(struct ixgbe_adapter *adapter)
{
	...
	rx_count = max_t(u32, rx_count, IXGBE_MIN_RXD);
				...
				unsigned int rxr_count, unsigned int rxr_idx)
{
	...
	while (txr_count) {
		...
		if (adapter->flags & IXGBE_FLAG_FCOE_ENABLED) {
			...
			if ((txr_idx >= f->offset) &&
			    (txr_idx < f->offset + f->indices))
				set_ring_type_cna(ring);
		}
...
static int ixgbe_alloc_q_vectors(struct ixgbe_adapter *adapter)
{
	...
	if ((q_vectors < (rxr_remaining + txr_remaining)) &&
		...
		int fcoe_rxq, fcoe_txq, fcoe_vidx;
		int q_split, i;
		...
		for (i = 0; i < q_split; i++, fcoe_vidx++) {
			int fqpv = DIV_ROUND_UP(fcoe_i, q_split - i);
			err = ixgbe_alloc_q_vector(adapter, q_split, fcoe_vidx,
						   0, 0, fqpv, fcoe_rxq);
			if (err)
				goto err_out;

			err = ixgbe_alloc_q_vector(adapter, q_split,
						   fcoe_vidx + q_split,
						   fqpv, fcoe_txq, 0, 0);
			if (err)
				goto err_out;
...
static void ixgbe_set_interrupt_capability(struct ixgbe_adapter *adapter)
{
	...
	v_budget = max(adapter->num_rx_queues, adapter->num_tx_queues);
	if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)
		v_budget += adapter->ring_feature[RING_F_FCOE].indices;
#endif /* __VMKLNX__ */

...
#else /* defined(__VMKLNX__) */
	/* PR 555248: for more details on reverting to older behavior of driver 
         * in handling the return value of e1000_clean_tx_irq() */
				 ...
				 const struct pci_device_id *ent)
{
   ...
   //triggering the ASSERT in vmkapi_mapping.c
...
#if defined(__VMKLNX__)
	if ( ((pci_resource_len(pdev, 1)) < MIN_IOBASE_LEN) ||
		((pci_resource_start(pdev, 1)) == 0)) {
		e_err("invalid ioremap size : %Ld, BAR 1 not enabled.\n",
                  pci_resource_len(pdev, 0));
		goto err_ioremap;
	}
#endif
	...
	strncpy(netdev->name, pci_name(pdev), sizeof(netdev->name) - 1);
        ...
        /* 
         * Following ifdef was present in inbox driver but not in

...
#if defined (__VMKLNX__)
static inline int nv_is_dev_in_special_handling(struct net_device *dev)
{
	struct fe_priv *np = netdev_priv(dev);
	u8 __iomem *base = get_hwbase(dev);
	...
	if (unlikely(test_bit(NV_DO_SPECIAL_DEV_HANDLING, &np->special_dev_handling))) {
		...
		// is done.
		spin_lock_irqsave(&np->lock, flags);
		if (likely(test_bit(NV_DO_SPECIAL_DEV_HANDLING, &np->special_dev_handling))) {
			u32 mask;
...
static inline int nv_need_dev_special_handling(struct net_device *dev, u32 events)
{
	struct fe_priv *np = netdev_priv(dev);
	u8 __iomem *base = get_hwbase(dev);

	if (unlikely(events & (NVREG_IRQ_FATAL_ERROR|NVREG_IRQ_RECOVER_ERROR))) {
		unsigned long flags;
...
#if defined (__VMKLNX__)
	if (nv_is_dev_in_special_handling (dev))
		return IRQ_RETVAL(0);
#endif /* (__VMKLNX__) */
...
#if defined (__VMKLNX__)
		if (nv_need_dev_special_handling(dev, events)) 
			break;
#endif /* defined (__VMKLNX__) */

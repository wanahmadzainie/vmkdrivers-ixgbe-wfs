                               ...
                               struct ethtool_ringparam *ring)
{
	...
	if (adapter->rx_ring && adapter->num_rx_queues > 1) {
...
#ifdef __VMKLNX__
			temp_tx_ring[i].active = adapter->tx_ring[i]->active;
#endif
		}
		need_update = true;
	}
...
#ifdef __VMKLNX__
			temp_rx_ring[i].active = adapter->rx_ring[i]->active;
#endif
		}
		need_update = true;
	}
					...
					struct ethtool_rxnfc *cmd)
{
	...
	if (fsp->location >= ((1024 << adapter->fdir_pballoc) - 2)) {
		DPRINTK(DRV, ERR, "Location out of range\n");
		return -EINVAL;
	}
	...
	if (!ixgbe_flowspec_to_flow_type(fsp,
		...
		DPRINTK(DRV, ERR, "Unrecognized flow type\n");
		goto err_out;
	}
	...
	if (hlist_empty(&adapter->fdir_filter_list)) {
		...
		if (err) {
			DPRINTK(DRV, ERR, "Error writing mask\n");
			goto err_out_w_lock;
		}
	} else if (memcmp(&adapter->fdir_mask, &mask, sizeof(mask))) {
		DPRINTK(DRV, ERR, "Only one mask supported per port\n");
		goto err_out_w_lock;
	}

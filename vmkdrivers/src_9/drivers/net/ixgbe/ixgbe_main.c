...
#define DRV_VERSION __stringify(MAJ) "." __stringify(MIN) "." \
	__stringify(BUILD) DRIVERIOV DRIVERNAPI \
	DRV_HW_PERF FPGA VMDQ_TAG
				 ...
				 union ixgbe_adv_rx_desc *rx_desc)
{
	...
	if (hdr_info & cpu_to_le16(IXGBE_RXDADV_SPH))
...
#ifdef __VMKLNX__
static void ixgbe_set_rsc_gso_size(struct net_device *netdev,
				   struct sk_buff *skb)
{
...
#ifdef __VMKLNX__
			ixgbe_set_rsc_gso_size(netdev_ring(rx_ring), skb);
...
#ifdef __VMKLNX__
	/* update the net device associated with the vector */
	if (ring_type_is_cna(rx_ring)) {
		q_vector->napi.dev = a->cnadev;
...
static void ixgbe_map_rings_to_vectors(struct ixgbe_adapter *adapter)
{
	...
	if ((q_vectors < (rxr_remaining + txr_remaining)) &&
		...
		int fcoe_i = adapter->ring_feature[RING_F_FCOE].indices;
		int fcoe_rxq, fcoe_txq;
		int fcoe_vector;
		int q_split;
		int i;

		if (q_vectors > MIN_MSIX_COUNT) {
			q_split = fcoe_i;
			q_vectors -= (fcoe_i * 2);
		} else {
			q_split = MIN_MSIX_Q_FCOE_VECTORS / 2;
			q_vectors -= MIN_MSIX_Q_FCOE_VECTORS;
		}

		/* remove FCOE vectors from regular Rx/Tx queue counts */
		fcoe_rxq = rxr_remaining = adapter->num_rx_queues - fcoe_i;
		fcoe_txq = txr_remaining = adapter->num_tx_queues - fcoe_i;

		/* FCOE vectors start at the end of the Rx/Tx q_vectors */
		fcoe_vector = q_vectors;

		/* map fcoe queues to vectors at end */
		for (i = 0; i < q_split && fcoe_i; i++, fcoe_vector++) {
			int fqpv = DIV_ROUND_UP(fcoe_i, q_split - i);
			for (; fqpv; fqpv--, fcoe_rxq++, fcoe_txq++, fcoe_i--) {
				map_vector_to_rxq(adapter,
						  fcoe_vector, fcoe_rxq);
				map_vector_to_txq(adapter,
						  fcoe_vector + q_split,
						  fcoe_txq);
			}
		}
	 ...
	 */
#ifdef __VMKLNX__
	int q_split = max(q_vectors / 2, 1);
	/* In case of RSS, there are more rx queues then usual, therefore
	 * q_split should be modified
	 */
	if ((adapter->flags & IXGBE_FLAG_RSS_ENABLED) &&
	    (q_vectors >= adapter->num_rx_queues + adapter->num_tx_queues))
		q_split = adapter->num_rx_queues;
#endif
	for (; v_start < q_vectors && rxr_remaining; v_start++) {
#ifdef __VMKLNX__
		int rqpv = DIV_ROUND_UP(rxr_remaining, q_split - v_start);
#else
...
#ifdef __VMKLNX__
	/*
	 * If Rx consumed all of the vectors then we must group Rx/Tx.
	 * This should only happen if the number of q_vectors is 1.
	 */
...
static int ixgbe_request_msix_irqs(struct ixgbe_adapter *adapter)
{
	...
	err = request_irq(adapter->msix_entries[vector].vector,
			  ...
			  adapter);
	if (err) {
		e_err(probe, "request_irq for msix_tcp_timer failed: %d\n",
		      err);
...
#ifdef CONFIG_IXGBE_NAPI
#ifdef IXGBE_NO_TX_NAPI
	/* We must clean the TX in the interrupt */
...
#ifdef __VMKLNX__
		/* In RSS mode, we should work in 4 queues per pool mode */
		if (adapter->flags & IXGBE_FLAG_RSS_ENABLED) {
			IXGBE_WRITE_REG(hw, IXGBE_MTQC,
				(IXGBE_MTQC_VT_ENA | IXGBE_MTQC_32VF));
			break;
		}
			    ...
			    struct ixgbe_ring *ring)
{
	...
	if (ring_is_ps_enabled(ring)) {
		...
		if (rx_buf_len < IXGBE_RXBUFFER_4K)
			...
			rscctrl |= IXGBE_RSCCTL_MAXDESC_16;
#else
			//In ESX, restricting max aggregated to 24K gives better perf compared 48K
			rscctrl |= IXGBE_RSCCTL_MAXDESC_8;
#endif
...
#ifndef __VMKLNX__
		//RSC enablement is done on demand in ESX.
		if (adapter->flags2 & IXGBE_FLAG2_RSC_ENABLED) {
...
int ixgbe_add_mac_filter(struct ixgbe_adapter *adapter, u8 *addr, u16 queue)
{
	...
	if (is_zero_ether_addr(addr)) {
		DPRINTK(PROBE, ERR, "MAC address is zero\n");
		return -EINVAL;
	}
		...
		if (adapter->mac_table[i].state & IXGBE_MAC_STATE_IN_USE) {
			...
			if (!compare_ether_addr
				...
				if (adapter->mac_table[i].queue == queue) {
					...
					DPRINTK(PROBE, ERR, 
					"Duplicate MAC address not allowed\n");
					return -EINVAL;
				}
			} else {
				continue;
...
static inline bool ixgbe_set_vmdq_queues(struct ixgbe_adapter *adapter)
{
...
#ifdef __VMKLNX__
	/* VMware ESX perf. tests show that optimal perf is achieved
	 * if num_queues = MIN(8, num_online_cpus/4).
	 */
	if ((adapter->hw.mac.type == ixgbe_mac_82599EB || adapter->hw.mac.type == ixgbe_mac_X540) &&
		!(adapter->flags2 & IXGBE_FLAG2_VMDQ_DEFAULT_OVERRIDE)) {
		vmdq_i = 1 << (fls(min(vmdq_i, ((int)num_online_cpus()/4))) - 1);
		vmdq_i = max(4, vmdq_i);
	}

	if ((adapter->flags2 & IXGBE_FLAG2_RSC_ENABLED) && !(adapter->flags & IXGBE_FLAG_RSS_ENABLED) &&
		!(adapter->flags2 & IXGBE_FLAG2_VMDQ_DEFAULT_OVERRIDE)) {
		//Allocate 8 queues to have enough resreved HW LRO and legacy queues.
		vmdq_i = 8;
	}
#endif //__VMKLNX__
	...
	/*
	 * See VMware PR 295512: For Jumbo Frame, we limit number of netqueues
	 * to 4 to support multiple ports with limited netPktHeap
	 */
	if ((adapter->netdev->mtu > ETH_DATA_LEN) && (adapter->num_rx_queues > 4)) {
		adapter->num_rx_queues = min(4, adapter->num_rx_queues);
		if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)
			adapter->num_rx_queues += fcoe_i;
		vmdq_i = adapter->num_rx_queues;
               /*
                * Do the same for tx queues as well.
                */
               adapter->num_tx_queues = min(4, adapter->num_tx_queues);
               if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)
                       adapter->num_tx_queues += fcoe_i;
	}
	if (adapter->flags & IXGBE_FLAG_RSS_ENABLED) {
		adapter->num_rx_queues = min(MAX_RX_QUEUES, vmdq_i + rss_i);
		adapter->num_tx_queues = min(MAX_TX_QUEUES, vmdq_i);
		vmdq_i++; /* Extra pool for RSS queues */
	}
	if (adapter->flags & IXGBE_FLAG_VMDQ_ENABLED) {
		...
		adapter->num_rx_queues_per_pool = adapter->num_rx_queues /
		                                  vmdq_i;
		if (adapter->flags & IXGBE_FLAG_RSS_ENABLED) {
			/* we use 1 queue per pool. This cannot be calculated
			   by regular division */
	...
	/*
	 * Make sure we've got the queues to give to CNA without
	 * giving away the default queue.
	 */
	if ((adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED) &&
		(adapter->num_rx_queues > fcoe_i)) {
		adapter->ring_feature[RING_F_FCOE].mask =
			adapter->num_rx_queues - fcoe_i;
	} else {

		if (adapter->flags2 & IXGBE_FLAG2_CNA_ENABLED)
			DPRINTK(PROBE, INFO, "Disabling CNA: "
				"not enough VMDq queues available\n");
		adapter->flags &= ~IXGBE_FLAG_FCOE_ENABLED;
		adapter->flags2 &= ~IXGBE_FLAG2_CNA_ENABLED;
		adapter->ring_feature[RING_F_FCOE].indices = 0;
		adapter->ring_feature[RING_F_FCOE].mask = adapter->num_rx_queues;
	}
vmdq_queues_out:
	return ret;
}
...
#ifdef __VMKLNX__
	if(adapter->flags & IXGBE_FLAG_SRIOV_ENABLED) {
		/* MIN_MSIX_COUNT includes 2 vectors for FCoE, but we won't need them */
		vector_threshold = 2 /* RX+TX */ + NON_Q_VECTORS;
	} else
#endif /* __VMKLNX__ */
...
static inline bool ixgbe_cache_ring_vmdq(struct ixgbe_adapter *adapter)
{
	...
	case (IXGBE_FLAG_RSS_ENABLED | IXGBE_FLAG_VMDQ_ENABLED):
		...
		case ixgbe_mac_82599EB:
		case ixgbe_mac_X540:
			/* Configure non RSS queues */
			for (i = 0; i < adapter->num_rx_pools - 1; i++)
				adapter->rx_ring[i]->reg_idx = i *
			...
			for (i = 0; i < IXGBE_ESX_RSS_QUEUES ; i++)
				...
				reg_idx = ((adapter->num_rx_pools - 1) *
...
static inline bool ixgbe_cache_ring_fcoe(struct ixgbe_adapter *adapter)
{
	...
	if (adapter->flags & IXGBE_FLAG_DCB_ENABLED) {
		 ...
		 */
		if ((f->indices == IXGBE_FCRETA_SIZE) && (tc > 3))
			fcoe_tx_i--;
...
static int ixgbe_alloc_queues(struct ixgbe_adapter *adapter)
{
	...
	DPRINTK(PROBE, ERR, "using rx_count = %d \n", rx_count);
#else
...
static int ixgbe_set_interrupt_capability(struct ixgbe_adapter *adapter)
{
	 ...
	 * rx and tx queue, even if that's more than num_online_cpu
	 */
	v_budget = adapter->num_rx_queues + adapter->num_tx_queues + NON_Q_VECTORS;
#endif
...
void ixgbe_clear_interrupt_scheme(struct ixgbe_adapter *adapter)
{
	...
	/* reinit the dev->napi_list */
	INIT_LIST_HEAD(&adapter->netdev->napi_list);
}
...
static int __devinit ixgbe_sw_init(struct ixgbe_adapter *adapter)
{
...
#ifdef __VMKLNX__
	/* Disable explicitly the packet split for ESX */
	adapter->flags &= ~IXGBE_FLAG_RX_PS_ENABLED;
	...
	if (adapter->flags & IXGBE_FLAG_DCB_CAPABLE) {
		...
		/* ESX CNA intitializes to DCB and four traffic classes */
		adapter->tc = 4;
		dcb_i = 4;
...
static int ixgbe_setup_all_tx_resources(struct ixgbe_adapter *adapter)
{
	...
	int net_tx_queues = adapter->num_tx_queues -
			    adapter->ring_feature[RING_F_FCOE].indices;
	for (i = 0; i < adapter->num_tx_queues; i++) {
		if (i < net_tx_queues) {
			adapter->tx_ring[i]->netdev = adapter->netdev;
			clear_ring_type_cna(adapter->tx_ring[i]);
		} else {
			adapter->tx_ring[i]->netdev = adapter->cnadev;
			set_ring_type_cna(adapter->tx_ring[i]);
		}
#ifdef HAVE_DEVICE_NUMA_NODE
...
{
	int i, err = 0;
	int net_rx_queues = adapter->num_rx_queues -
			    adapter->ring_feature[RING_F_FCOE].indices;

	for (i = 0; i < adapter->num_rx_queues; i++) {
		if (i < net_rx_queues) {
			adapter->rx_ring[i]->netdev = adapter->netdev;
			clear_ring_type_cna(adapter->rx_ring[i]);
		} else {
			adapter->rx_ring[i]->netdev = adapter->cnadev;
			set_ring_type_cna(adapter->rx_ring[i]);
		}
#ifdef HAVE_DEVICE_NUMA_NODE
			...
			for_each_possible_cpu(cpu) {
				...
				fcoe_noddp_counts_sum +=
					*ESX_PER_CPU_PTR(fcoe->pcpu_noddp,
								cpu, u64);
				fcoe_noddp_ext_buff_counts_sum +=
					*ESX_PER_CPU_PTR(fcoe->
						pcpu_noddp_ext_buff, cpu,u64);
#endif
			}
		}
...
#if defined(CONFIG_PCI_IOV) && defined(HAVE_IPLINK_VF_CONFIG) && defined(__VMKLNX__)
static VMK_ReturnStatus
ixgbe_passthru_ops(struct net_device *netdev, vmk_NetPTOP op, void *pargs)
{
	...
	case VMK_NETPTOP_VF_SET_MAC:
	{
		...
		if (is_zero_ether_addr(args->mac)) {
                	...
                	ixgbe_del_mac_filter(adapter,
                                adapter->vfinfo[args->vf].vf_mac_addresses, args->vf);
                	memset(adapter->vfinfo[args->vf].vf_mac_addresses, 0, ETH_ALEN);
                	adapter->vfinfo[args->vf].pf_set_mac = false;
			ret = VMK_OK;
        	} else {
			if (ixgbe_ndo_set_vf_mac(netdev, args->vf, args->mac) < 0)
				ret = VMK_FAILURE;
			else
				ret = VMK_OK;
		}
...
#endif
#else
	indices = IXGBE_MAX_VMDQ_INDICES;
#endif
...
static void __devexit ixgbe_remove(struct pci_dev *pdev)
{
	...
	if (netdev->reg_state == NETREG_UNINITIALIZED ||
		...
		dev_err(pci_dev_to_dev(pdev), "ixgbe_remove: netdev uninitialized"
		        ", reg_state=0x%x, state=0x%x\n",

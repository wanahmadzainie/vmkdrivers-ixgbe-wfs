...
#ifdef __VMKLNX__
/*
 * PR 683953: Performance study shows that having 1024 tx ring entries
 * will avoid frequent queue stop on ESX under heavy tx workloads.
 * Besides, esx40 and earlier releases use 1024 tx descriptors by
 * default already.
 */
...
struct ixgbe_adapter {
	...
	struct vlan_group *vlgrp;		/* must be first, see
						   ixgbe_receive_skb */
#else
...
#endif
#define IXGBE_FLAG_RX_1BUF_CAPABLE              (u32)(1 << 5)
#define IXGBE_FLAG_RX_PS_CAPABLE                (u32)(1 << 6)
#define IXGBE_FLAG_RX_PS_ENABLED                (u32)(1 << 7)
...
#define IXGBE_FLAG_VMDQ_CAPABLE                 (u32)(1 << 17)
...
#define IXGBE_FLAG2_CNA_ENABLED                  (u32)(1 << 3)
...
extern int ixgbe_add_mac_filter(struct ixgbe_adapter *adapter,
				u8 *addr, u16 queue);
#ifdef __VMKLNX__
extern void ixgbe_flush_sw_mac_table(struct ixgbe_adapter *adapter);
extern void ixgbe_del_mac_filter_by_index(struct ixgbe_adapter *adapter,
					  int index);
#endif

...
/*
 *  Currently, on ESX, the wmb() instruction is defined to only a compiler barrier.
 *  The macro wmb() need to be overrode to properly synchronize memory.
 *
 *  See PR 791836. This workaround is needed until PR 819237 is fixed in vmklinux.
 */
#if defined(__VMKLNX__)
#undef wmb
...
#define DP_SHMEM_LINE(_offset)				        \
	printk(KERN_ERR PFX "%s: %08x: %08x %08x %08x %08x\n",  \
	       bp->dev->name,				        \
	       _offset,                                         \
	       bnx2_shmem_rd(bp, _offset),                      \
	       bnx2_shmem_rd(bp, _offset + 4),                  \
	       bnx2_shmem_rd(bp, _offset + 8),                  \
	       bnx2_shmem_rd(bp, _offset + 12))

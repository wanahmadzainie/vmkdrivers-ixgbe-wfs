...
static void cnic_kwq_completion(struct cnic_dev *dev, u32 count)
{
	...
	info.cmd = DRV_CTL_RET_L5_SPQ_CREDIT_CMD;
...
static int cnic_bnx2x_destroy_ramrod(struct cnic_dev *dev, u32 l5_cid)
{
	...
	if (ret == 0) {
		wait_event_timeout(ctx->waitq, ctx->wait_cond, (HZ / 4));
...
static void cnic_bnx2x_delete_wait(struct cnic_dev *dev, u32 start_cid)
{
	...
	for (i = start_cid; i < MAX_ISCSI_TBL_SZ; i++) {
...
#if (LINUX_VERSION_CODE >= 0x20613) || (defined(VMWARE_ESX_DDK_VERSION) && \
...
static void cnic_service_bnx2x_bh(unsigned long data)
{
	...
	status_idx = *cp->kcq_status_idx_ptr;
	hw_prod = *cp->kcq_hw_prod_idx_ptr;
	...
	while (sw_prod != hw_prod) {
		...
		if (kcq_diff > MAX_KCQ_IDX)
			printk(KERN_WARNING PFX
				...
				" > MAX_KCQ_IDX(%lu)\n",
				dev->netdev->name,
				hw_prod, sw_prod, (unsigned long) MAX_KCQ_IDX);
		...
		barrier();
		if (status_idx == *cp->kcq_status_idx_ptr)
			break;
...
#endif
		cnic_ack_bnx2x_int(dev, cp->bnx2x_igu_sb_id, storm_id,
				   status_idx, IGU_INT_ENABLE, 1);
...
#if defined(__VMKLNX__)
static void cnic_stop_irq_task(struct cnic_dev *dev)
{
	struct cnic_local *cp = dev->cnic_priv;
	struct cnic_eth_dev *ethdev = cp->ethdev;
...
static void cnic_free_irq(struct cnic_dev *dev)
{
	...
	if (ethdev->drv_state & CNIC_DRV_STATE_USING_MSIX) {
		cp->disable_int_sync(dev);
		tasklet_kill(&cp->cnic_irq_task);
		if (cp->cnic_local_flags & CNIC_LCL_FL_IRQ_REQD)
			free_irq(ethdev->irq_arr[0].vector, dev);
		cp->cnic_local_flags &= ~CNIC_LCL_FL_IRQ_REQD;
	}
}
...
static void cnic_get_bnx2x_iscsi_info(struct cnic_dev *dev)
{
	...
	if (!(val & SHR_MEM_VALIDITY_LIC_NO_KEY_IN_EFFECT)) {
		...
		val16 = (CNIC_RD(dev, addr) & BNX2X_MAX_ISCSI_INIT_CONN_MASK) >> BNX2X_MAX_ISCSI_INIT_CONN_SHIFT;
...
static int cnic_start_bnx2x_hw(struct cnic_dev *dev)
{
...
#if (NEW_BNX2X_HSI == 60)
	switch (BNX2X_CHIP_NUM(cp->chip_id)) {
		case BNX2X_CHIP_NUM_57710:
			cp->iro_arr = e1_iro_arr;
			break;
		case BNX2X_CHIP_NUM_57711:
		case BNX2X_CHIP_NUM_57711E:
			cp->iro_arr = e1h_iro_arr;
			break;
		case BNX2X_CHIP_NUM_57712:
		case BNX2X_CHIP_NUM_57712E:
	...
	if (BNX2X_CHIP_IS_E2(cp->chip_id)) {
		...
		cp->kcq_hw_prod_idx_ptr =
			&sb->sb.index_values[HC_INDEX_ISCSI_EQ_CONS];
		cp->kcq_status_idx_ptr =
...
static struct cnic_dev *init_bnx2_cnic(struct net_device *dev)
{
	...
	probe = cnic_register_get_callback("bnx2");
#else
	probe = vmk_ModuleGetSymbol("bnx2_cnic_probe2");
#endif /* (VMWARE_ESX_DDK_VERSION >= 50000) */
	if (probe) {
		ethdev = (*probe)(dev);
	}
...
static struct cnic_dev *init_bnx2x_cnic(struct net_device *dev)
{
	...
	probe = cnic_register_get_callback("bnx2x");
#else
	probe = vmk_ModuleGetSymbol("bnx2x_cnic_probe2");
#endif /* (VMWARE_ESX_DDK_VERSION >= 50000) */
	if (probe) {
		ethdev = (*probe)(dev);
	}

...
lpfc_config_msi(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb)
{
	...
	/* Sanity check */
	if (phba->cfg_use_msi != LPFC_USE_MSIX) {
...
lpfc_config_pcb_setup(struct lpfc_hba * phba)
{
	...
	uint32_t iocbs_offset;
	uint32_t iocbCnt = 0;
	int i;

	iocbs_offset = offsetof(struct lpfc_sli2_slim, iocb_buffer);
	...
	for (i = 0; i < psli->num_rings; i++) {
		...
		pring->sizeCiocb =
			phba->sli_rev == LPFC_SLI_REV3 ? SLI3_IOCB_CMD_SIZE:
							 SLI2_IOCB_CMD_SIZE;
		pring->sizeRiocb =
			phba->sli_rev == LPFC_SLI_REV3 ? SLI3_IOCB_RSP_SIZE:
							 SLI2_IOCB_RSP_SIZE;
		...
		/* Command ring setup.  The byte_index is used to partition the
		 * rings because an IOCB_t has one size for all SLI revisions
		 * and using it creates a sparse SLIM config in host memory
		 * and the potential for memory overruns resulting from the
		 * sizeof(IOCB_t) vs the sizeCiob or sizeRiocb.
		 */
		pring->cmdringaddr = (void *)
			(&phba->slim2p->iocb_buffer[byte_index]);
		pdma_addr = phba->slim2p_mapping + iocbs_offset + byte_index;
		...
		/*
		 * Response ring setup.  For each pass, accumulate the distance
		 * in bytes for this ring's command iocbs and its response
		 * iocbs.  This distance prepares the next ring setup.
		 */
		byte_index += (pring->numCiocb * pring->sizeCiocb);
		pring->rspringaddr =
			(void *) (&phba->slim2p->iocb_buffer[byte_index]);
		pdma_addr = phba->slim2p_mapping + iocbs_offset + byte_index;
		...
		byte_index += (pring->numRiocb * pring->sizeRiocb);

		lpfc_printf_log(phba, KERN_INFO, LOG_INIT,
				"0407 PCB Setup: Ring %d:\n"
				"Ciocb #: %d, size:%d,\n"
				"Riocb #: %d, size:%d,\n"
				"cAddr_High:0x%x,cAddrLow:0x%x,\n"
				"rAddrHigh:0x%x,rAddrLow:0x%x,\n"
				"IOCB Cnt:0x%x IOCB Bytes 0x%x\n\n",
				i,
				pcbp->rdsc[i].cmdEntries, pring->sizeCiocb,
				pcbp->rdsc[i].rspEntries, pring->sizeRiocb,
				pcbp->rdsc[i].cmdAddrHigh,
				pcbp->rdsc[i].cmdAddrLow,
				pcbp->rdsc[i].rspAddrHigh,
				pcbp->rdsc[i].rspAddrLow,
				iocbCnt, byte_index);
	}
}
...
lpfc_config_port(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb)
{
	...
	if (phba->cfg_hostmem_hgp && phba->sli_rev != LPFC_SLI_REV3) {
...
/**
 * lpfc_mbox_cmd_check - Check the validity of a mailbox command
 * @phba: pointer to lpfc hba data structure.

			 ...
			 struct lpfc_nodelist *ndlp)
{
	...
	int  put_node;
	int  put_rport;
	uint16_t keep_prev_state = 0, keep_nlp_state = 0;
	uint32_t keep_nlp_flag = 0;
	...
	keep_prev_state = ndlp->nlp_prev_state;
	keep_nlp_state = ndlp->nlp_state;
	...
	if (!new_ndlp) {
		...
		/*
		 * Just grab the DID.  The ndlp states are not required because
		 * the ndlp is not an active node. Also, clear the ndlp's
		 * portname.  The ndlp is getting replaced by an inactive
		 * new_ndlp so there is nothing further pending on the ndlp.
		 */
		...
		/*
		 * The new_ndlp and the ndlp are both non-NULL and actual
		 * active nodes.  Rather than swap ndlps, just update the
		 * wwpn for the caller's ndlp and mark the new_ndlp as
		 * NPR state and no wwpn.  The logic will catch up later.
		 */
		keepDID = new_ndlp->nlp_DID;
		if (phba->sli_rev == LPFC_SLI_REV4)
			memcpy(&rrq.xri_bitmap,
				&new_ndlp->active_rrqs.xri_bitmap,
				sizeof(new_ndlp->active_rrqs.xri_bitmap));
		keep_prev_state = new_ndlp->nlp_prev_state;
	...
	if (memcmp(&ndlp->nlp_portname, name, sizeof(struct lpfc_name)) == 0) {
		 ...
		 * to put ndlp on UNUSED list and try to free it.  We also
		 * need to fix the rport bindings.
		 */
		rport =  ndlp->rport;
		if (rport) {
			rdata = rport->dd_data;
		...
		/* Two ndlps should not have the same did on the nodelist */
		...
		/*
		 * Complete the ndlp swap.  This path is executed when the
		 * an active new_ndlp is geting swapped with an active ndlp.
		 */
		lpfc_unreg_rpi(vport, ndlp);
		ndlp->nlp_DID = keepDID;
		ndlp->nlp_prev_state = keep_prev_state;
		ndlp->nlp_state = keep_nlp_state;
		ndlp->nlp_flag |= keep_nlp_flag;
		if (ndlp->nlp_state < NLP_STE_PLOGI_ISSUE)
			memset(&ndlp->nlp_portname, 0,
			       sizeof(struct lpfc_name));
		if (phba->sli_rev == LPFC_SLI_REV4)
			memcpy(&ndlp->active_rrqs.xri_bitmap,
				&rrq.xri_bitmap,
		...
		struct lpfc_iocbq *cmdiocb)
{
	...
	lpfc_ioctl_rscn_event(vport->phba, rscn_event_data,
		  ...
		  struct lpfc_nodelist *ndlp)
{
	...
	/* The first affected N_Port page */
	vmklnx_iodm_event(shost, IODM_RSCN, NULL, be32_to_cpu(*lp));
#endif
			...
			struct lpfc_iocbq *rspiocbp)
{
	...
	if (rspiocbp->iocb.ulpStatus == IOSTAT_LS_RJT) {
		...
		memcpy(sd_event->vport_name, &vport->fc_portname,
		       sizeof(sd_event->vport_name));
		lsrjt_event = (void *)&sd_event->event_payload;
		...
		evt_type.cat = FC_REG_ELS_EVENT;
		evt_type.sub = LPFC_EVENT_LSRJT_RCV;
	...
	if ((rspiocbp->iocb.ulpStatus == IOSTAT_NPORT_BSY) ||
		...
		memcpy(sd_event->vport_name, &vport->fc_portname,
		       sizeof(sd_event->vport_name));
		fabric_event = (void *)&sd_event->event_payload;
		...
		evt_type.mask = FC_REG_SANDIAGS_EVENT;
		evt_type.cat = LPFC_EVENT_PORT_BUSY;
		evt_type.sub = LPFC_EVENT_FABRIC_BUSY;
		lpfc_send_event(vport->phba, sd_event_size, sd_event, evt_type);
		return;
	}
}
		    ...
		    uint32_t *payload)
{
	...
	memcpy(sd_event->vport_name, vport->fc_portname.u.wwn,
	       sizeof(sd_event->vport_name));

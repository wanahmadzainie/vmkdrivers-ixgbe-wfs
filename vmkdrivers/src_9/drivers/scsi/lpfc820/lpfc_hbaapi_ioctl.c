				 ...
				 struct lpfcCmdInput * cip, void *dataout)
{
	...
	snprintf(ha->FirmwareVersion, FW_REV_STR_SIZE, "%s", fwrev);
			      ...
			      struct lpfcCmdInput * cip, void *dataout)
{
	...
	hp->PortSpeed = lpfc_port_link_speed(phba);

	hp->PortSupportedSpeed = 0;
	if (phba->lmt & LMT_16Gb)
		hp->PortSupportedSpeed |= HBA_PORTSPEED_16GBIT;

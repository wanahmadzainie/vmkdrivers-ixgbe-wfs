...
lpfc_start_discovery(struct lpfc_vport *vport)
{
	 ...
	 * New flag is added to handle the case for logo on physical port.
	 */
		...
		struct lpfc_work_evt *evtp)
{
	...
	/* Dont Q the fast_evt_data
	 * Alloc memory for the event and Q it
	 */

	evt_data_size += sizeof(struct lpfc_sd_event);

	sd_event = kzalloc(evt_data_size, GFP_KERNEL);
	...
	memcpy(sd_event->vport_name,
		fast_evt_data->vport->fc_portname.u.wwn,
		sizeof(sd_event->vport_name));
	memcpy(&sd_event->event_payload, evt_data, evt_data_size);

	evt_type.mask = FC_REG_SANDIAGS_EVENT;
	evt_type.cat = evt_category;
	evt_type.sub = evt_sub_category;
	lpfc_send_event(phba, evt_data_size, sd_event, evt_type);
...
#if defined(__VMKLNX__)
	/* Pass Link Down events on a per physical port basis. */
...
#if defined(__VMKLNX__)
	/* Pass Link Up events on a physical port basis. */
	vmklnx_iodm_event(lpfc_shost_from_vport(phba->pport), IODM_LINKUP,
			  NULL, 0);
#endif
...
lpfc_mbx_cmpl_reg_login(struct lpfc_hba *phba, LPFC_MBOXQ_t *pmb)
{
	...
	/* If vport is in the middle of deletion free the RPI */
	if (!(phba->pport->load_flag & FC_UNLOADING) &&
		(vport->load_flag & FC_UNLOADING)) {
		/* Free service param buffer */
		lpfc_mbuf_free(phba, mp->virt, mp->phys);
		kfree(mp);
		lpfc_nlp_put(ndlp);
		   ...
		   int state)
{
	...
	lpfc_printf_vlog(vport, KERN_INFO, LOG_NODE,
			 "0904 NPort state transition %p x%06x, %s -> %s\n",
			 ndlp,
			 ndlp->nlp_DID,
			 lpfc_nlp_state_name(name1, sizeof(name1), old_state),
			 lpfc_nlp_state_name(name2, sizeof(name2), state));

	lpfc_printf_vlog(vport, KERN_INFO, LOG_TRC_NODE, "3117 "
		 ...
		 int state)
{
	...
	if (state != NLP_STE_UNUSED_NODE)
		...
		lpfc_printf_vlog(vport, KERN_INFO, LOG_TRC_NODE, "3118 "
		"node enable:       did:x%x\n",
		ndlp->nlp_DID);
	return ndlp;
}
...
/**
 * lpfc_is_mapped_node_logged_in
 *
 * This routine checks if the mapped node if fully logged in.
 * A mapped node in this context is a Target Port.
 *
 * Inputs:
 ...
 *
 * Return codes
 * 0: is not a fully logged in mapped node
 * 1: is a fully logged in mapped node
 */
bool
lpfc_is_mapped_node_logged_in(struct lpfc_vport *vport, uint32_t did) {
	struct lpfc_nodelist *ndlp = lpfc_findnode_did(vport, did);

	/* Test executes on active nodes only. */
	if (!ndlp || !NLP_CHK_NODE_ACT(ndlp))
		return 0;

	/* This is the only successful test case. */
	else if ((ndlp->nlp_state == NLP_STE_MAPPED_NODE) &&
		 (ndlp->nlp_type & NLP_FCP_TARGET))
		return 1;

	else
	/* All other cases fail the test */
		return 0;
}


/**
 * lpfc_is_unmapped_node_logged_in
 *
 * This routine checks if the unmapped node if fully logged in.
 * An unmapped node in this context is either a Fabric or Initiator
 * node.
 *
 * Inputs:
 * vport: Vport to check
 ...
 *
 * Return codes
 * 0: is not a fully logged in unmapped node
 * 1: is a fully logged in unmapped node
 */
bool
lpfc_is_unmapped_node_logged_in(struct lpfc_vport *vport, uint32_t did) {
	struct lpfc_nodelist *ndlp = lpfc_findnode_did(vport, did);

	/* Test executes on valid nodes only. */
	if (!ndlp || !NLP_CHK_NODE_ACT(ndlp))
		return 0;

	/* This is the only successful test case. */
	else if ((ndlp->nlp_state == NLP_STE_UNMAPPED_NODE) &&
	    (ndlp->nlp_type & (NLP_FABRIC | NLP_FCP_INITIATOR)))
		return 1;
...
lpfc_issue_clear_la(struct lpfc_hba *phba, struct lpfc_vport *vport)
{
	...
	if ((phba->link_state >= LPFC_CLEAR_LA) ||
		...
		/* Clause to get SLI4 to ready state */
		if ((vport->port_type == LPFC_PHYSICAL_PORT) &&
		    (phba->sli_rev == LPFC_SLI_REV4) &&
		    (phba->link_state == LPFC_LINK_UP)) {
			phba->link_state = LPFC_HBA_READY;
	      ...
	      uint32_t did)
{
	...
	lpfc_printf_vlog(vport, KERN_INFO, LOG_TRC_NODE, "3119 "
		"node init:       did:x%x\n",
		ndlp->nlp_DID);
...
lpfc_nlp_get(struct lpfc_nodelist *ndlp)
{
	...
	if (ndlp) {
		lpfc_printf_vlog(ndlp->vport, KERN_INFO, LOG_TRC_NODE, "3120 "
...
lpfc_nlp_put(struct lpfc_nodelist *ndlp)
{
	...
	lpfc_printf_vlog(ndlp->vport, KERN_INFO, LOG_TRC_NODE, "3121 "
...
{
	lpfc_printf_vlog(ndlp->vport, KERN_INFO, LOG_TRC_NODE, "3122 "

...
#define LPFC_MAX_TGT_QDEPTH	0x2000
#define LPFC_MAX_SECTORS	0xffff

#define  LPFC_MAX_BUCKET_COUNT 20	/* Maximum no. of buckets for stat data
					   collection. */

/* Moved these definitions from lpfc_menlo.c to here because they are used
 * in lpfc_ioctl.c   -DrW
 */
...
/* Moved from lpfc_sli.h to here because it is used in lpfc_ioctl.c */
...
struct lpfc_mbox_ext_buf_ctx {
	uint32_t state;
#define LPFC_MGMT_MBOX_IDLE     0
#define LPFC_MGMT_MBOX_HOST     1
#define LPFC_MGMT_MBOX_PORT     2
#define LPFC_MGMT_MBOX_DONE     3
#define LPFC_MGMT_MBOX_ABTS     4
...
/* Provide symbolic defines for MSI management.*/
#define LPFC_USE_INTX 0
#define LPFC_USE_MSI  1
#define LPFC_USE_MSIX 2
enum intr_type_t {
	NONE = 0,
	INTx,
	MSI,
...
/* Provide symbolic defines for MultiQueue. */
#define LPFC_DISABLE_MQ	0
#define LPFC_ENABLE_MQ	1
...
#define HBA_DMA_RESTRICT	0x8000 /* HBA is DMA restrict capable */
	dma_addr_t slim2p_mapping;
	atomic_t fcp_ring_in_use; /* When polling test if intr-hndlr active*/
...
#define LPFC_FCF_PRIORITY 2    /* Priority fcf failover */
	...
	uint32_t cfg_log_verbose;
#if !defined(__VMKLNX__)
	/* 06/14/10 AER:  No support for AER in vmklinux right now. Needs
	 * testing once AER support is kernel-resident.
 	*/
...
#define VPD_MASK            0xf         /* mask for any vpd data */
	...
	spinlock_t hbalock	____cacheline_aligned;
	spinlock_t eh_waitlock	____cacheline_aligned;
...
/* Support HBAAPI parameters for RNID support. */
	...
	uint64_t last_fp_poll_time;
	struct rnd_state random_state;
	uint32_t iocb_cnt;
	uint32_t iocb_max;
	atomic_t sdev_cnt;

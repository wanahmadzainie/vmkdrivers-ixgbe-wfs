...
static DEFINE_IDR(lpfc_shost_index);
...
lpfc_config_port_prep(struct lpfc_hba *phba)
{
	...
	if (rc != MBX_SUCCESS) {
		lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
				...
				"READ_REV, rc x%x mbxStatus x%x\n",
				mb->mbxCommand, rc, mb->mbxStatus);
		mempool_free( pmb, phba->mbox_mem_pool);
		return -ERESTART;
	}
	...
	if (phba->sli_rev == LPFC_SLI_REV3 && !mb->un.varRdRev.v3rsp) {
		mempool_free(pmb, phba->mbox_mem_pool);
		return -EINVAL;
	}
	...
	memcpy(vp->rev.opFwName, (char *) mb->un.varRdRev.opFwName, 16);
...
{
	/* Ignore when mailbox doesn't complete, i.e. lpfc_sli_brdrestart */
	if (pmboxq->u.mb.mbxStatus == MBX_NOT_FINISHED)
		goto out;
	else if (pmboxq->u.mb.mbxStatus == MBX_SUCCESS)
...
/**
 * lpfc_set_hba_queue_depth - Set the hba queue depth for this SLI2/3 port.
 * @phba: pointer to lpfc hba data structure.
 * @mb: pointer to the completed read_config mailbox.
 *
 * This routine sets the @phba's cfg_hba_queue_depth based on the
 * ports read_config mailbox command reply.  This routine allows
 * a maximum hba_queue_depth of 2048 because in ESX, Emulex lpfc
 * drivers are held to a 34MB heap limit server-wide.  This routine
 * is only called for SLI2/3 HBAs.
 *
 * Return codes
 *   None
 **/
static void
lpfc_set_hba_queue_depth(struct lpfc_hba *phba, MAILBOX_t *mb)
{
	uint32_t port_queue_depth = mb->un.varRdConfig.max_xri+1;

	if (phba->cfg_hba_queue_depth > port_queue_depth) {
		if (port_queue_depth > 2048)
			port_queue_depth = 2048;
		phba->cfg_hba_queue_depth = port_queue_depth;
	}
}
...
lpfc_config_port_post(struct lpfc_hba *phba)
{
	...
	/* Set the HBA_Q_DEPTH based on the port's max. */
	lpfc_set_hba_queue_depth(phba, mb);
...
{
	struct lpfc_board_event_header *board_event = NULL;
	struct lpfc_sd_event *sd_event = NULL;
	int sd_event_size = 0;
	struct event_type evt_type;

	sd_event_size = sizeof(struct lpfc_sd_event)
		      + sizeof(struct lpfc_board_event_header);

	sd_event = kzalloc(sd_event_size, GFP_KERNEL);

	if (sd_event != NULL) {
		/* Write 0 for physical port */
		(void) memset(&sd_event->vport_name, 0,
				sizeof(sd_event->vport_name));
		board_event = (void *)&sd_event->event_payload;
		board_event->event_type  = FC_REG_BOARD_EVENT;
		board_event->subcategory = LPFC_EVENT_PORTINTERR;

		evt_type.mask = FC_REG_SANDIAGS_EVENT;
		evt_type.cat = FC_REG_BOARD_EVENT;
		evt_type.sub = LPFC_EVENT_PORTINTERR;
...
lpfc_get_hba_model_desc(struct lpfc_hba *phba, uint8_t *mdp, uint8_t *descp)
{
	...
	case PCI_DEVICE_ID_FALCON:
		m = (typeof(m)){"LPSe12002-ML1-E", "PCIe",
				"OneSecure Fibre Channel Adapter"};
		break;
		 ...
		 int type)
{
	...
	if (type & DIAG_VPORT)
		shost = lpfc_diag_shost_alloc();

	else if (dev != &phba->pcidev->dev)
...
#if defined(__VMKLNX__)
		/*
		 * vmware ignores the dma_boundary. To enforce max scatter
		 * gather element size, set max_Sectors.
		 * Some SLI4 hba can support larger max sector which can
		 * overflow the MAX SECTOR size supported by o/s.
		 */
		if (phba->sli4_hba.pc_sli4_params.sge_supp_len / 512
			< LPFC_MAX_SECTORS)
			shost->max_sectors =
			phba->sli4_hba.pc_sli4_params.sge_supp_len / 512;
#endif
	...
	if (!(vport->vport_flag & DIAG_VPORT)) {
...
/**
 * lpfc_get_shost_instance - Get a unique integer ID
 *
 * This routine allocates a unique integer ID from lpfc_shost_index pool. It
 * uses the kernel idr facility to perform the task.
 *
 * Return codes:
 *   instance - a unique integer ID allocated as the new instance.
 ...
 **/
int
lpfc_get_shost_instance(void)
{
	int instance = 0;

	/* Assign an unused number */
	if (!idr_pre_get(&lpfc_shost_index, GFP_KERNEL))
		return -1;
	if (idr_get_new(&lpfc_shost_index, NULL, &instance))
		return -1;
	return instance;
}
...
#if defined(__VMKLNX__)
		/*
		 * FCoE port issues a clear virtual link to the driver.
		 * Propagate the event to vmkernel.  Note - this event is
		 * designed to be on a per vport basis and the vport can
		 * be the physical or virtual port.  The driver handles the
		 * CVL on that sole basis as well.
		 */
		vmklnx_iodm_event(lpfc_shost_from_vport(vport),
				  IODM_FCOE_CVL, NULL, vport->fc_myDID);
#endif
...
lpfc_enable_pci_dev(struct lpfc_hba *phba)
{
	...
	if (pdev->device == PCI_DEVICE_ID_HORNET ||
	    pdev->device == PCI_DEVICE_ID_ZEPHYR_DCSP)
		phba->hba_flag |= HBA_FCOE_MODE;
#if !defined(__VMKLNX__)
	/* PCIe EEH revoery on powerpc platforms needs fundamental reset */
...
lpfc_sli_driver_resource_setup(struct lpfc_hba *phba)
{
	...
	/* SLI3 and earlier ports don't support MQ. */
	phba->cfg_use_mq = LPFC_DISABLE_MQ;
...
lpfc_setup_driver_resource_phase1(struct lpfc_hba *phba)
{
	...
	spin_lock_init(&phba->eh_waitlock);
...
lpfc_hba_alloc(struct pci_dev *pdev)
{
	...
	atomic_set(&phba->fcp_ring_in_use, 1);
...
lpfc_create_shost(struct lpfc_hba *phba)
{
	...
	vport = lpfc_create_port(phba, phba->brd_no, &phba->pcidev->dev, 0);
	if (!vport)
		return -ENODEV;
...
lpfc_post_init_setup(struct lpfc_hba *phba)
{
	...
	/*
	 * Disable the polling logic for now.  Needs to be test more
	 * thoroughly.
	 */
	phba->cfg_poll = 0;
	...
	if (sd_event != NULL) {
		...
		adapter_event = (void *)&sd_event->event_payload;
		adapter_event->event_type  = FC_REG_ADAPTER_EVENT;
		adapter_event->subcategory = LPFC_EVENT_ARRIVAL;

		/* Send board arrival event to upper layer */
		evt_type.mask = FC_REG_SANDIAGS_EVENT;
		evt_type.cat = FC_REG_ADAPTER_EVENT;
		evt_type.sub = LPFC_EVENT_ARRIVAL;
		lpfc_send_event(phba, sd_event_size, sd_event, evt_type);
	}
...
lpfc_sli_pci_mem_setup(struct lpfc_hba *phba)
{
	...
	phba->slim2p = dma_alloc_coherent(&phba->pcidev->dev,
					       sizeof(struct lpfc_sli2_slim),
					       &phba->slim2p_mapping,
					       GFP_KERNEL);
	if (!phba->slim2p)
		goto out_iounmap;

	memset(phba->slim2p, 0, sizeof(struct lpfc_sli2_slim));
	...
	phba->IOCBs = (struct _IOCB *)(phba->slim2p +
		       offsetof(struct lpfc_sli2_slim, iocb_buffer));
...
lpfc_sli4_queue_setup(struct lpfc_hba *phba)
{
	...
	struct vmklnx_scsi_ioqueue_info q_info[LPFC_FP_WQN_MAX];
	int vector_idx;
	...
	/* Set up fast-path FCP Work Queue.  The fp vectors start at index 1. */
	memset(&q_info, 0, (sizeof(struct vmklnx_scsi_ioqueue_info) *
			    LPFC_FP_WQN_MAX));
	vector_idx = 1;
	if (!phba->sli4_hba.fcp_wq) {
		lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
	...
	for (fcp_wqidx = 0; fcp_wqidx < phba->cfg_fcp_wq_count; fcp_wqidx++) {
		...
		if (phba->cfg_use_mq == LPFC_ENABLE_MQ) {
			q_info[fcp_wqidx].q_handle = (void *)
				(uint64_t)fcp_wqidx;
			q_info[fcp_wqidx].q_vector =
				phba->sli4_hba.msix_entries[vector_idx].vector;
			vector_idx++;
		}
	...
	/*
	 * All fast-path work-queues were successfully established.
	 * Call vmkernel to notify it of the available queues and MSI-X vectors.
	 */
	if (phba->cfg_use_mq == LPFC_ENABLE_MQ) {
		rc = vmklnx_scsi_register_ioqueue(shost, phba->mq_num_qs,
						  q_info);
		if (rc != 0) {
			lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
					"2816 Failed to register WQ->MSIX "
					"vectors, status x%x, disabling MQ "
					"and using INTX\n", rc);
			phba->cfg_use_mq = LPFC_DISABLE_MQ;
			phba->cfg_use_msi = LPFC_USE_INTX;
		}
	}
...
enable_msix_vectors:
	...
	for (index = 1; index < vectors; index++) {
		...
		atomic_set(&phba->sli4_hba.fcp_eq_hdl[index - 1].fcp_eq_in_use,
				1);
...
lpfc_pci_probe_one_s3(struct pci_dev *pdev, const struct pci_device_id *pid)
{
...
#if defined(__VMKLNX__)
	vmklnx_iodm_enable_events(pci_get_drvdata(pdev));
#endif
	...
	if (phba->pcidev->msix_enabled) {
		/*
		 * Register the correct SLI3 interrupt handler for coredump poll
		 * completions.
		 */
		vmklnx_scsi_register_poll_handler(shost,
						  phba->msix_entries[0].vector,
	...
	if (vports != NULL)
		for (i = 1; i <= phba->max_vports && vports[i] != NULL; i++) {
			if (vports[i]->port_type == LPFC_PHYSICAL_PORT)
				continue;
			__lpfc_vport_delete(lpfc_shost_from_vport(vports[i]),
				vports[i]);
		}
	lpfc_destroy_vport_work_array(phba, vports);
...
lpfc_pci_probe_one_s4(struct pci_dev *pdev, const struct pci_device_id *pid)
{
	...
	/*
	 * SLI4-capable ports should always attempt to configure for
	 * multiqueue support unless the admin has disabled it explicitly.
	 * Determine how many queues are wanted by vmkernel.  The driver
	 * wants to allocate only as many queues and MSI-X vectors as are
	 * needed.
	 */
	if (phba->cfg_use_mq == LPFC_ENABLE_MQ) {
		phba->mq_num_qs = vmklnx_scsi_get_num_ioqueue(
						      phba->cfg_fcp_eq_count);
		if (phba->mq_num_qs <= 1)
			phba->cfg_use_mq = LPFC_DISABLE_MQ;
		else if (phba->mq_num_qs < phba->cfg_fcp_wq_count) {
			phba->cfg_fcp_wq_count = phba->mq_num_qs;
			phba->cfg_fcp_eq_count = phba->mq_num_qs;
			phba->sli4_hba.cfg_eqn = phba->cfg_fcp_eq_count +
						 LPFC_SP_EQN_DEF;
		}

		lpfc_printf_log(phba, KERN_INFO, LOG_INIT,
				"2815 MultiQueue support: %d with "
				"%d fcp wqs, %d fcp eqs\n",
				phba->cfg_use_mq,
				phba->cfg_fcp_wq_count,
				phba->cfg_fcp_eq_count);
	...
	while (true) {
		...
		if (intr_mode == LPFC_INTR_ERROR) {
			lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
					"2705 Failed to enable interrupt.\n");
			error = -ENODEV;
			goto out_free_sysfs_attr;
		}
	...
	if (phba->pcidev->msix_enabled) {
		/*
		 * Register the correct SLI4 interrupt handler for coredump poll
		 * completions.
		 */
		vmklnx_scsi_register_poll_handler(shost,
				phba->sli4_hba.msix_entries[1].vector,
...
#if defined(__VMKLNX__)
	vmklnx_iodm_disable_events(shost);
#endif
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION(LPFC_MODULE_ANNOUNCE);

...
struct proc_page lpfc_selector[] = {
	{"0", PAGE_0,},
	{"1", PAGE_1,},
	{"2", PAGE_2,},
	{"3", PAGE_3,},
...
static uint32_t
lpfc_proc_rpl(struct lpfc_hba *, HBA_WWN *, HBA_WWN *,
		    ...
		    struct lpfc_scsi_buf *lpfc_cmd, struct lpfc_iocbq *rsp_iocb)
{
	...
	if (scsi_status && (pnode && NLP_CHK_NODE_ACT(pnode)))
		pnode->fcperr_cnt++;
	...
	if (resp_info & RESID_UNDER) {
		...
		lpfc_printf_log(phba, KERN_INFO, LOG_FCP_UNDER,
	 ...
	 */
		...
		case SAM_STAT_CHECK_CONDITION:
			...
			if (pnode && NLP_CHK_NODE_ACT(pnode)) {
				...
				pnode->frame_drop_cnt++;
#if defined(__VMKLNX__)
				vmklnx_iodm_event(lpfc_shost_from_vport(vport),
						IODM_FRAMEDROP, (void *)cmnd,
						be32_to_cpu(fcprsp->rspResId));
#endif
			}
			break;
...
/**
 * lpfc_proc_info - Proc Node  interface
 * @shost: pointer to Scsi_Host
 * @buffer: pointer to user buffer
 * offset: offset into buffer for writing
 * length: buffer length
 * func:  1=write, 0=read;
 *
 * Outputs:
 * @start: written with pointer to proc data
 *
 * Return: number of bytes written
 *
 **/
int
lpfc_proc_info(struct Scsi_Host *shost, char *buffer, char **start,
	       off_t offset, int length, int proc_write)
{
	struct lpfc_vport *vport = (struct lpfc_vport *) shost->hostdata;
	struct lpfc_hba *phba = vport->phba;
	int len = 0;
	...
	if (proc_write) {
		...
		if (!strncmp(log_cmd, "lpfc_dump_fcp",
			     sizeof("lpfc_dump_fcp"))) {
			lpfc_dump_fcp_cq(phba);
			msleep(1000);
			lpfc_dump_fcp_wq(phba);
			return length;
		}

		if (!strncmp(log_cmd, "lpfc_stop_chip",
			     sizeof("lpfc_stop_chip"))) {
			lpfc_sli4_stop_chip(phba);
			return length;
		}

		if (!strncmp(log_cmd, "lpfc_dump_mbxq",
			     sizeof("lpfc_dump_mbxq"))) {
			lpfc_dump_mbox_queue(phba);
			return length;
		}

		if (!strncmp(log_cmd, "lpfc_log_verbose",
				   sizeof("lpfc_log_verbose"))) {
			lpfc_printf_log(phba, KERN_INFO, LOG_INIT,
					"0701 Received %s value x%x\n",
					log_cmd, lpfc_log_setting);

			 /* Allow writes to lpfc_log_verbose.  */
			lpfc_log_verbose_set(phba->pport, lpfc_log_setting);
		} else if (!strncmp(log_cmd, "reset", sizeof("reset")))
			lpfc_resetcounters(phba);

		/* SLI4 IF 2 Ports */
		else if (strncmp(log_cmd, "dump", sizeof("dump") - 1) == 0)
			lpfc_sli4_pdev_reg_request(phba, LPFC_FW_DUMP);
		else if (strncmp(log_cmd, "fw_reset",
				 sizeof("fw_reset") - 1) == 0)
			lpfc_sli4_pdev_reg_request(phba, LPFC_FW_RESET);

		else
			/* Select a page for next proc read*/
			phba->proc_page = lpfc_proc_page_selector(buffer);
	...
	*start = buffer;
	...
	switch (phba->proc_page) {
	default:
	case PAGE_0:
		len = lpfc_proc_default(shost, info_buf, INFO_BUF);
		break;
	case PAGE_1:
		len = lpfc_proc_hbaattr(shost, info_buf, INFO_BUF);
		break;
	case PAGE_2:
		len = lpfc_proc_adapterportstats(shost, info_buf, INFO_BUF);
		break;
	case PAGE_3:
		len = lpfc_proc_ptxcnt(shost, info_buf, INFO_BUF);
		break;
	case PAGE_4:
		len = lpfc_proc_targetportstats(shost, info_buf, INFO_BUF);
		break;
	case PAGE_5:
		len = lpfc_proc_saninfo(shost, info_buf, INFO_BUF);
		break;
	case PAGE_6:
		len = lpfc_proc_vsaninfo(shost, info_buf, INFO_BUF);
		break;
	case PAGE_7:
		len = lpfc_proc_getcfg(shost, info_buf, INFO_BUF);
		break;
	case PAGE_COUNT:
		len = lpfc_proc_pagecount(shost, info_buf, INFO_BUF);
		break;
	}
...
/**
 * lpfc_proc_page_selector - Function to return proc node page
 * @buffer: pointer to buffer with user's text string
 * info_len: buffer length
 *
 * Note:  If unable to parse token or cmd, return page zero.
 *
 * Return: Proc Page.
 *
 **/
int
lpfc_proc_page_selector(char *buffer) {
	char token[32] = {0};
	char cmd[32] = {0};
	uint32_t line;
	int i, rc = 0;

	sscanf(buffer, "%31s%31s%d", token, cmd, &line);
	if (!strncmp(token, "count", sizeof("count")))
		return PAGE_COUNT;
	if (!strncmp(token, "page", sizeof("page")))
		for (i = 0; i < PAGE_COUNT; i++)
			if (!strcmp(cmd, lpfc_selector[i].page_string))
				rc = lpfc_selector[i].page_no;
	return rc;
}

/**
 * lpfc_proc_default - Proc Node default page interface
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 * Return: number of bytes written to info_buf
 *
 **/
int
lpfc_proc_default(struct Scsi_Host *shost, char *info_buf, int info_size)
{
	struct lpfc_vport *vport = (struct lpfc_vport *) shost->hostdata;
 ...
 port_data:
	...
	case LPFC_VPORT_READY:
		...
		if (phba->fc_linkspeed == LPFC_LINK_SPEED_16GHZ)
			len += snprintf(info_buf + len, INFO_BUF-len,
					"   Current speed 16G\n\n");
		else if (phba->fc_linkspeed == LPFC_LINK_SPEED_10GHZ)
			len += snprintf(info_buf + len, INFO_BUF-len,
					"   Current speed 10G\n\n");
		else if (phba->fc_linkspeed == LPFC_LINK_SPEED_8GHZ)
			len += snprintf(info_buf + len, INFO_BUF-len,
					"   Current speed 8G\n\n");
		else if (phba->fc_linkspeed == LPFC_LINK_SPEED_4GHZ)
			len += snprintf(info_buf + len, INFO_BUF-len,
					"   Current speed 4G\n\n");
		else if (phba->fc_linkspeed == LPFC_LINK_SPEED_2GHZ)
			len += snprintf(info_buf + len, INFO_BUF-len,
					"   Current speed 2G\n\n");
		else if (phba->fc_linkspeed == LPFC_LINK_SPEED_1GHZ)
			len += snprintf(info_buf + len, INFO_BUF-len,
					"   Current speed 1G\n\n");
		else
			len += snprintf(info_buf + len, INFO_BUF-len,
...
/**
 * lpfc_proc_vptxcnt - Proc node page for port transmit count
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 ...
 **/
int
lpfc_proc_ptxcnt(struct Scsi_Host *shost, char *info_buf, int info_size)
{
	struct lpfc_vport *vport = (struct lpfc_vport *) shost->hostdata;
	struct lpfc_hba *phba = vport->phba;
	uint8_t name[sizeof(struct lpfc_name)];
	...
	len += snprintf(info_buf + len, info_size - len,
			"\nlpfc%d AdapterPort Counters\n",
			phba->brd_no);
...
/**
 * lpfc_proc_getcfg - Proc Node for reading configuration parameters
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 ...
 **/
int
lpfc_proc_getcfg(struct Scsi_Host *shost, char *info_buf, int info_size)
{
	struct lpfc_vport *vport = (struct lpfc_vport *) shost->hostdata;
	struct lpfc_hba *phba = vport->phba;
	 ...
	 */
	len += snprintf(info_buf + len, info_size - len,
			"Configuration Parameters for lpfc%d\n\n"
			"Name                  Low        High    Dflt "
			"Current      Description\n",
			phba->brd_no);

	for (i = 0; i < phba->pport->CfgCnt; i++) {
		if (len > info_size - 132)
			break;
		/* First validate parameter */
		vce =  phba->pport->CfgTbl[i];
		if (!vce)
			return len;
		cp = vce->entry;
		if (!cp)
		...
		if (lpfc_is_sli4(phba) && (cp->a_flag & CFG_SLI4))
			...
			continue;
 keep1:
		/* cp has the config parameter to print */
		len += snprintf(info_buf + len, info_size - len,
				"%-20s %4x    %8x    %4x    %4x      %s\n",
				cp->a_string, cp->a_low, cp->a_hi,
				cp->a_default, cp->a_current, cp->a_help);
	}
	...
	for (i = 0; i < phba->CfgCnt; i++) {
		...
		/* Tailor link-speed help string */
		if (0 == (strncmp(cp->a_string, "link-speed",
				  sizeof("link-speed")))) {
			sprintf(cp->a_help,
				"Select link speed: "
				"[%s %s %s %s %s] or 0 for auto negotiate",
				(phba->lmt & LMT_1Gb)  ? "1"  : "",
				(phba->lmt & LMT_2Gb)  ? "2"  : "",
				(phba->lmt & LMT_4Gb)  ? "4"  : "",
				(phba->lmt & LMT_8Gb)  ? "8"  : "",
				(phba->lmt & LMT_16Gb) ? "16" : "");
		}
...
/**
 * lpfc_proc_hbaattr - Proc Node for hba attributes
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 ...
 **/
int
lpfc_proc_hbaattr(struct Scsi_Host *shost, char *info_buf, int info_size)
{
	...
	memset((char *)&cip, 0, sizeof(cip));
	memset((char *)&ha, 0, sizeof(ha));
	if (lpfc_ioctl_hba_adapterattributes(phba, &cip, &ha))
		return 0;

	len += snprintf(info_buf + len, info_size - len,
			"\nlpfc%d Adapter Attributes\n"
			"Manufacturer:\t%s\nSerialNum:\t%s\n",
			phba->brd_no,
			ha.Manufacturer, ha.SerialNumber);
	len += snprintf(info_buf + len, info_size - len,
			"Model:\t\t%s\nDescription:\t%s (rev %02d)\n",
			ha.Model, ha.ModelDescription, phba->pcidev->revision);
	len += snprintf(info_buf + len, info_size - len,
			"Driver Version:\t%s %s\nFW Version:\t%s\n",
			ha.DriverName, ha.DriverVersion, ha.FirmwareVersion);
	len += snprintf(info_buf + len, info_size - len,
			"Symbolic Name:\t%s\nHW Version:\t%s\n"
			"ROM Version:\t%s\n", ha.NodeSymbolicName,
			ha.HardwareVersion, ha.OptionROMVersion);
	len += snprintf(info_buf + len, info_size - len,
			"Vendor Id:\t0x%x\nPort Count:\t%d\n",
			ha.VendorSpecificID, ha.NumberOfPorts);
	memcpy(&name[0], &ha.NodeWWN, sizeof(struct lpfc_name));
	len += snprintf(info_buf + len, info_size - len,
...
/**
 * lpfc_proc_adapterportstats - Proc Node for adapter port stats
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 ...
 **/
int
lpfc_proc_adapterportstats(struct Scsi_Host *shost,
			   char *info_buf, int info_size)
{
	struct lpfcCmdInput cip;
	struct lpfc_vport *vport = (struct lpfc_vport *) shost->hostdata;
	struct lpfc_hba *phba = vport->phba;
	HBA_PORTSTATISTICS ps;
	int len = 0;

	memset((char *)&cip, 0, sizeof(cip));
	memset((char *)&ps, 0, sizeof(ps));

	if (lpfc_ioctl_hba_portstatistics(phba, &cip, &ps))
		return 0;

	len += snprintf(info_buf + len, info_size - len,
			"\nlpfc%d AdapterPort Statistics\n"
			"TxFrm        %lld\n"
			"TxWrd        %lld\n"
			"RxFrm        %lld\n"
			"RxWrd        %lld\n",
			phba->brd_no,
			ps.TxFrames, ps.TxWords,
			ps.RxFrames, ps.RxWords);
	len += snprintf(info_buf + len, info_size - len,
			"LIP          %lld\n"
			"NOS          %lld\n"
			"ErrFrames    %lld\n"
			"DumpedFrames %lld\n",
			ps.LIPCount, ps.NOSCount,
			ps.ErrorFrames, ps.DumpedFrames);
	len += snprintf(info_buf + len, info_size - len,
			"LnkFail      %lld\n"
			"LOSync       %lld\n"
			"LOSig        %lld\n"
			...
			ps.LinkFailureCount,
			ps.LossOfSyncCount,
			ps.LossOfSignalCount,
			ps.PrimitiveSeqProtocolErrCount);
	len += snprintf(info_buf + len, info_size - len,
			"InvTxWrd     %lld\n"
			"InvCRC       %lld\n",
			ps.InvalidTxWordCount,
			ps.InvalidCRCCount);
	return len;
}

/**
 * lpfc_proc_adapterportstats - Proc Node for target port stats
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 ...
 **/
int
lpfc_proc_targetportstats(struct Scsi_Host *shost,
			  char *info_buf, int info_size)
{
	struct lpfcCmdInput cip;
	struct lpfc_vport *vport = (struct lpfc_vport *) shost->hostdata;
	struct lpfc_hba *phba = vport->phba;
	struct DFC_GetNodeStat *stat;
	int vpi, len = 0, i;
	uint8_t name[sizeof(struct lpfc_name)];

#define STAT_BUF_SIZE 4096
	stat = kzalloc(STAT_BUF_SIZE, GFP_KERNEL);
	if (!stat)
		return 0;
	...
	len += snprintf(info_buf + len, info_size - len,
			"\nlpfc%d TargetPort Statistics\t\t      "
			"tx_cnt qdepth   qmax   qcur   busy tgtrst "
			"lunrst  abts    tmo no_res invrpi lclrjt  "
			"drops  over  under scsi_cc\n",
			phba->brd_no);

	stat->numberOfEntries =
		((STAT_BUF_SIZE - (sizeof(struct DFC_GetNodeStat)
				- sizeof(struct DFC_NodeStatEntry_V2)))
				/ sizeof(struct DFC_NodeStatEntry_V2));

	cip.lpfc_arg2 = (void *)(unsigned long)stat->numberOfEntries;


	for (vpi = 0; vpi < LPFC_INTR_VPI; vpi++) {
		if (lpfc_ioctl_node_stat(phba, &cip, stat, vpi))
			continue;
		for (i = 0; i < stat->numberOfEntries; i++) {
			if (len > info_size - 132)
				break;
			len += snprintf(info_buf + len, info_size - len,
					"%7s vport%d ",
					stat->nodeStat[i].vmhba, vpi);
			memcpy(&name[0], &stat->nodeStat[i].wwpn,
				sizeof(struct lpfc_name));
			len += snprintf(info_buf + len, info_size - len,
					"%02x:%02x:%02x:%02x:%02x:"
					"%02x:%02x:%02x",
					...
					name[4], name[5], name[6], name[7]);
			len += snprintf(info_buf + len, info_size - len,
					"\t%12lld %6d %6d %6d %6d %6d"
					"%6d %6d %6d %6d %6d %6d %6d"
					"%6d %6d %6d\n",
					stat->nodeStat[i].TargetIOCnt,
					stat->nodeStat[i].TargetQDepth,
					stat->nodeStat[i].TargetMaxCnt,
					stat->nodeStat[i].TargetActiveCnt,
					stat->nodeStat[i].TargetBusyCnt,
					stat->nodeStat[i].TargetResetCnt,
					stat->nodeStat[i].TargetLunResetCnt,
					stat->nodeStat[i].TargetAbtsCnt,
					stat->nodeStat[i].TargetTimeOutCnt,
					stat->nodeStat[i].TargetNoRsrcCnt,
					stat->nodeStat[i].TargetInvldRpiCnt,
					stat->nodeStat[i].TargetLclRjtCnt,
					stat->nodeStat[i].TargetFrameDropCnt,
					stat->nodeStat[i].TargetOverrunCnt,
					stat->nodeStat[i].TargetUnderrunCnt,
					stat->nodeStat[i].TargetFcpErrCnt);
		}
	}
	kfree(stat);
	return len;
...
/**
 * lpfc_proc_pagecount - Proc Node for returning the number of pages
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 ...
 **/
int
lpfc_proc_pagecount(struct Scsi_Host *shost, char *info_buf, int info_size)
{
	return snprintf(info_buf, info_size, "%d\n", PAGE_COUNT);
}


/**
 * lpfc_proc_resetcounters - Proc Node function for resetting ndlp counters
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 ...
 **/
void
lpfc_resetcounters(struct lpfc_hba *phba) {
	struct lpfc_vport **vports;
	struct lpfc_nodelist *ndlp;
	struct Scsi_Host  *shost;
	int i;
		...
		list_for_each_entry(ndlp, &vports[i]->fc_nodes, nlp_listp) {
			if (NLP_CHK_NODE_ACT(ndlp) && ndlp->rport &&
			   (ndlp->rport->roles & FC_RPORT_ROLE_FCP_TARGET)) {
				ndlp->scsi_busy_cnt = 0;
				ndlp->cmd_max = 0;
				ndlp->fcperr_cnt = 0;
				ndlp->abts_cnt = 0;
				ndlp->tmo_cnt = 0;
				ndlp->invld_rpi_cnt = 0;
				ndlp->no_rsrc_cnt = 0;
				ndlp->rjt_cnt = 0;
				ndlp->tgt_rst = 0;
				ndlp->lun_rst = 0;
				ndlp->frame_drop_cnt = 0;
				ndlp->underrun_cnt = 0;
				ndlp->overrun_cnt = 0;
				ndlp->tx_cnt = 0;
			}
...
/*! lpfc_proc_rpl - issue a SCSI Report LUN comand
 *
 * \pre
 * \post
 * \param  board
 * \param  PortWWN
 * \param  rspBuffer
 * \param  rspBufferCount
 * \param  senseBuffer
 * \param  senseBufferCount
 *
 * \return uint32_t      Return Code  0 = success
 *
 * \b Description:
 *    Requests report lun information
 */
static uint32_t
lpfc_proc_inq(struct lpfc_hba *phba, HBA_WWN *iwwpn, HBA_WWN *twwpn,
	      uint64_t lun, uint8_t evpd, uint32_t pagecode,
	      void *rspBuffer, uint32_t *rspBufferCount,
	      void *senseBuffer, uint32_t *senseBufferCount)
{
	struct lpfcCmdInput ci;
	IOargUn arg3;
	struct fcp_cmnd inqcmnd;
	uint32_t rc, count;

#define FCP_SCSI_INQUIRY 0x12

	memset(&inqcmnd, 0, sizeof(inqcmnd));
	memcpy((char *)&inqcmnd.fcp_lun, (char *)&lun, sizeof(lun));
	inqcmnd.fcpCntl3 = READ_DATA;
	inqcmnd.fcpCdb[0] = FCP_SCSI_INQUIRY;
	inqcmnd.fcpCdb[1] = evpd;
	if (evpd)
		inqcmnd.fcpCdb[2] = (uint8_t) pagecode;
	inqcmnd.fcpCdb[4] = (uint8_t)(*rspBufferCount);
	count = (uint32_t) (*rspBufferCount);
	inqcmnd.fcpDl = __swab32(count);

	memset(&ci, 0, sizeof(ci));
	memcpy(&arg3.Iarg.targ_wwpn, twwpn, sizeof(twwpn));
	memcpy(&arg3.Iarg.vport_wwpn, iwwpn, sizeof(iwwpn));

	ci.lpfc_brd = (short)phba->brd_no;
	ci.lpfc_ring = 1;       /* this means kernel buffers */
	ci.lpfc_arg1 = (void *)&inqcmnd;
	ci.lpfc_arg2 = senseBuffer;
	ci.lpfc_arg3 = (void *)&arg3;
	ci.lpfc_arg4 = sizeof(inqcmnd);
	ci.lpfc_flag = (*senseBufferCount);
	ci.lpfc_dataout = (char *)rspBuffer;
	ci.lpfc_outsz = (*rspBufferCount);
	ci.lpfc_cmd = LPFC_HBA_SEND_SCSI;

	rc = lpfc_ioctl_send_scsi_fcp(phba, &ci);
	if (!rc) {
		*senseBufferCount = arg3.Oarg.snscnt;
		*rspBufferCount = arg3.Oarg.rspcnt;
	}
	return rc;
}
 ...
 */
static uint32_t
lpfc_proc_rpl(struct lpfc_hba *phba, HBA_WWN *iwwpn, HBA_WWN *twwpn,
	      void *rspBuffer, uint32_t *rspBufferCount,
	      void *senseBuffer, uint32_t *senseBufferCount)
{
	struct lpfcCmdInput ci;
	IOargUn arg3;
	struct fcp_cmnd rplcmnd;
	uint32_t rc, count;

#define FCP_SCSI_REPORT_LUNS 0xa0
#define FC_LUN_SHIFT 8

	memset(&rplcmnd, 0, sizeof(rplcmnd));

	rplcmnd.fcpCntl3 = READ_DATA;
	rplcmnd.fcpCdb[0] = FCP_SCSI_REPORT_LUNS;
	count = (uint32_t) (*rspBufferCount);
	rplcmnd.fcpDl = __swab32(count);
	rplcmnd.fcpCdb[9] = (uint8_t) (count & 0xff);
	count = (count >> 8);
	rplcmnd.fcpCdb[8] = (uint8_t) (count & 0xff);
	count = (count >> 8);
	rplcmnd.fcpCdb[7] = (uint8_t) (count & 0xff);
	count = (count >> 8);
	rplcmnd.fcpCdb[6] = (uint8_t) (count & 0xff);
	...
	ci.lpfc_brd = (short)phba->brd_no;
	ci.lpfc_ring = 1;	/* this means kernel buffers */
	ci.lpfc_arg1 = (void *)&rplcmnd;
	ci.lpfc_arg2 = senseBuffer;
	ci.lpfc_arg3 = (void *)&arg3;
	ci.lpfc_arg4 = sizeof(rplcmnd);
	ci.lpfc_flag = (*senseBufferCount);
	ci.lpfc_dataout = (char *)rspBuffer;
	ci.lpfc_outsz = (*rspBufferCount);
	ci.lpfc_cmd = LPFC_HBA_SEND_SCSI;
...
/**
 * lpfc_proc_saninfo - Proc Node for san inventory
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 ...
 **/
int
lpfc_proc_saninfo(struct Scsi_Host *shost, char *info_buf, int info_size)
{
	struct lpfcCmdInput ci;
	struct lpfc_vport *vport = (struct lpfc_vport *) shost->hostdata;
	struct lpfc_hba *phba = vport->phba;
	struct lpfc_node_info *ni;
	HBA_PORTATTRIBUTES hp;
	uint8_t name[sizeof(struct lpfc_name)];
	int i = 0, j = 0, num = 0;
	int len = 0;
	uint8_t *rpl = 0, *inq = 0, *sense = 0, *lunp = 0;
	uint32_t rplcnt, snscnt, inqcnt, rc;
	uint32_t num_luns = 0, length = 0;
	uint8_t evpd;
	uint32_t pagecode;
	uint32_t fcpLun[2];
	uint32_t savelun;
...
#define NODE_CNT	64
#define RPL_BUFFER	1024
#define	INQ_BUFFER	128
#define SENSE_BUFFER	128

	len += snprintf(info_buf + len, info_size - len,
			"lpfc%d SAN Info\n\n", phba->brd_no);

	ni = kzalloc(sizeof(struct lpfc_node_info) * NODE_CNT, GFP_KERNEL);
	rpl = kzalloc(RPL_BUFFER, GFP_KERNEL);
	inq = kzalloc(INQ_BUFFER, GFP_KERNEL);
	sense = kzalloc(SENSE_BUFFER, GFP_KERNEL);
	if (!ni || !rpl || !inq || !sense)
		goto out;

	memset((char *)&ci, 0, sizeof(ci));

	if (lpfc_ioctl_hba_portattributes(phba, &ci, &hp))
		goto out;

	memcpy(&name[0], &hp.PortWWN, sizeof(struct lpfc_name));
	len += snprintf(info_buf + len, info_size - len,
			"Adapter DID %5x WWPN "
			"%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n\n",
			hp.PortFcId,
	...
	if (lpfc_ioctl_nodeinfo(phba, &ci, ni, sizeof(ni)))
		goto out;

	for (num = 0; num < NODE_CNT; num++) {
		if ((ni[num].a_state == 0) && (ni[num].a_flag == 0) &&
		     (ni[num].a_did == 0))
			break;
	}
	...
	for (i = 0; i < num; i++) {
		if (len > info_size - 132)
			goto out;
		if ((ni[i].a_flag & HBA_CAN_BIND_AUTOMAP) &&
		    (ni[i].a_flag & (NODE_FABRIC | NODE_FCP_TARGET)) == 0) {
			memcpy(&name[0], &ni[i].a_wwpn,
				sizeof(struct lpfc_name));
			len += snprintf(info_buf + len, info_size - len,
					"\tInitiator DID  %5x  WWPN "
					"%02x:%02x:%02x:%02x:"
					"%02x:%02x:%02x:%02x\n",
					ni[i].a_did,
					name[0], name[1], name[2], name[3],
	...
	for (i = 0; i < num; i++) {
		if (len > info_size - 132)
			goto out;
		if (ni[i].a_flag &  NODE_FCP_TARGET) {
			memcpy(&name[0], &ni[i].a_wwpn,
				sizeof(struct lpfc_name));
			len += snprintf(info_buf + len, info_size - len,
					"\n\tTarget DID  %5x  WWPN "
			...
			rplcnt = RPL_BUFFER;
			snscnt = SENSE_BUFFER;
			rc = lpfc_proc_rpl(phba, &hp.PortWWN,
					  (HBA_WWN *)&ni[i].a_wwpn,
					  rpl, &rplcnt, sense, &snscnt);
			if (rc) {
				len += snprintf(info_buf + len, info_size - len,
					"\tRPL failed, rc = x%x\n", rc);
				goto out;
			...
			if (snscnt) {
				len += snprintf(info_buf + len, info_size - len,
						"\tRPL failed: "
						"Sense Data x%x x%x x%x\n",
						sense[2], sense[12], sense[13]);
			}
			length = (unsigned char)rpl[3];
			length += (unsigned char)rpl[2] << 8;
			length += (unsigned char)rpl[1] << 16;
			length += (unsigned char)rpl[0] << 24;
			num_luns = length / 8; /* lun size is 8 bytes */

			lunp = rpl;
			/* Do Inquiry for each LUN */
			for (j = 0; j < num_luns; j++) {
				...
				evpd = 0;
				pagecode = 0;
				lunp += 8; /* Point to next LUN entry */
				savelun = lunp[1];
				fcpLun[0] = ((savelun & 0xFF) << FC_LUN_SHIFT)
					    | ((savelun >> 8) & 0xFF);
				fcpLun[1] = 0;
				memcpy((char *)&lun, (char *) &fcpLun[0],
					sizeof(lun));
				rc = lpfc_proc_inq(phba, &hp.PortWWN,
						(HBA_WWN *)&ni[i].a_wwpn,
						lun, evpd, pagecode,
						inq, &inqcnt, sense, &snscnt);
				if (rc) {
					len += snprintf(info_buf + len,
							info_size - len,
							"\tInquiry failed, "
				...
				if (j == 0) {
					strncpy(buf1, &inq[8], 16);
					strncpy(buf2, &inq[16], 4);
					len += snprintf(info_buf + len,
							info_size - len,
							"(%s %s)\n",
							buf1, buf2);
				...
				len += snprintf(info_buf + len, info_size - len,
					...
					(unsigned char)inq[8],
					(unsigned char)inq[9],
					(unsigned char)inq[10],
					(unsigned char)inq[11],
					(unsigned char)inq[12],
					(unsigned char)inq[13],
					(unsigned char)inq[14],
					(unsigned char)inq[15],
					(unsigned char)inq[16],
					(unsigned char)inq[17],
					(unsigned char)inq[18],
					(unsigned char)inq[19],
					(unsigned char)inq[20],
					(unsigned char)inq[21],
					(unsigned char)inq[22],
					(unsigned char)inq[23]);
			}
		}
	}
...
/**
 * lpfc_proc_vsaninfo - Proc Node for vport san inventory
 * @shost: pointer to Scsi_Host
 * @info_buf: pointer to buffer
 * info_len: buffer length
 *
 ...
 **/
int
lpfc_proc_vsaninfo(struct Scsi_Host *shost, char *info_buf, int info_size)
{
	struct lpfcCmdInput ci;
	struct lpfc_vport *vport = (struct lpfc_vport *) shost->hostdata;
	struct lpfc_hba *phba = vport->phba;
...
#define	INQ_BUFFER	128
#define SENSE_BUFFER	128
	uint32_t	nodecnt = NODE_CNT;

	len += snprintf(info_buf + len, info_size - len,
			"lpfc%d Vport SAN Info\n\n", phba->brd_no);

	vp = kzalloc(sizeof(DFC_VPEntryList) * NODE_CNT, GFP_KERNEL);
	ni = kzalloc(sizeof(DFC_GetNodeInfo) * NODE_CNT, GFP_KERNEL);
	rpl = kzalloc(RPL_BUFFER, GFP_KERNEL);
	inq = kzalloc(INQ_BUFFER, GFP_KERNEL);
	sense = kzalloc(SENSE_BUFFER, GFP_KERNEL);
	if (!vp || !ni || !rpl || !inq || !sense)
		goto out;

	memset((char *)&ci, 0, sizeof(ci));

	ci.lpfc_ring = 1;	/* this means kernel buffers */
	ci.lpfc_arg1 = &nodecnt;
	ci.lpfc_outsz = sizeof(vp);
	rc = lpfc_ioctl_vport_list(phba, &ci, vp);
	if (rc) {
		len += snprintf(info_buf + len, info_size - len,
				"\tVPList failed rc=x%x\n", rc);
		goto out;
	}
	for (vi = 0; vi < vp->numberOfEntries; vi++) {

		memcpy(&name[0], &vp->vpentry[vi].wwpn,
			sizeof(struct lpfc_name));
		len += snprintf(info_buf + len, info_size - len,
				"Vport DID %5x WWPN "
				"%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
				vp->vpentry[vi].PortFcId,
				name[0], name[1], name[2], name[3],
				name[4], name[5], name[6], name[7]);

		ci.lpfc_ring = 1;	/* this means kernel buffers */
		ci.lpfc_arg1 = &vp->vpentry[vi].wwpn;
		ci.lpfc_arg2 = &nodecnt;
		rc = lpfc_ioctl_vport_nodeinfo(phba, &ci, ni);
		if (rc) {
			len += snprintf(info_buf + len, info_size - len,
					"\tVPNodeList failed rc=x%x\n", rc);
			goto out;
		}
		/* Report Targets */
		for (ti = 0; ti < ni->numberOfEntries; ti++) {
			if (len > info_size - 132)
				goto out;
			memcpy(&name[0], &ni->nodeInfo[ti].fcpId.PortWWN,
				sizeof(struct lpfc_name));
			len += snprintf(info_buf + len, info_size - len,
					"\n\tTarget DID  %5x  WWPN "
					"%02x:%02x:%02x:%02x:"
					"%02x:%02x:%02x:%02x ",
					ni->nodeInfo[ti].fcpId.FcId,
					name[0], name[1], name[2], name[3],
					name[4], name[5], name[6], name[7]);
			...
			rplcnt = RPL_BUFFER;
			snscnt = SENSE_BUFFER;
			rc = lpfc_proc_rpl(phba, &vp->vpentry[vi].wwpn,
				(HBA_WWN *)&ni->nodeInfo[ti].fcpId.PortWWN,
				 rpl, &rplcnt, sense, &snscnt);
			if (rc) {
				len += snprintf(info_buf + len, info_size - len,
						"\tRPL failed, rc = x%x\n", rc);
			...
			for (li = 0; li < num_luns; li++) {
				...
				memcpy((char *)&lun, (char *) &fcpLun[0],
					sizeof(lun));
				rc = lpfc_proc_inq(phba,
				   (HBA_WWN *)&vp->vpentry[vi].wwpn,
				   (HBA_WWN *)&ni->nodeInfo[ti].fcpId.PortWWN,
				   lun, evpd, pagecode,
				   inq, &inqcnt, sense, &snscnt);
				if (rc) {
				...
				len += snprintf(info_buf + len, info_size - len,
						"\t\tTgt%02d Lun%02d",
						ni->nodeInfo[ti].scsiId.
						ScsiTargetNumber, savelun);
				len += snprintf(info_buf + len, info_size - len,
					" UID %02x%02x%02x%02x%02x%02x"
					"%02x%02x%02x%02x%02x%02x"
					"%02x%02x%02x%02x\n",

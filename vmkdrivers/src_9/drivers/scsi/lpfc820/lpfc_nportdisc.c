...
out:
	...
	/* Populate wwns in ndlp for event reporting */  
	ap = (ADISC *) lp;        
	memcpy(&ndlp->nlp_nodename, &ap->nodeName, sizeof (struct lpfc_name));  
	memcpy(&ndlp->nlp_portname, &ap->portName, sizeof (struct lpfc_name));  
			...
			void *arg, uint32_t evt)
{
	...
	lpfc_printf_vlog(vport, KERN_INFO, LOG_DISCOVERY,
			 ...
			 "state %d Data: x%x %p\n",
			 evt, ndlp->nlp_DID, cur_state, ndlp->nlp_flag, ndlp);
	...
	if (got_ndlp) {
		lpfc_printf_vlog(vport, KERN_INFO, LOG_DISCOVERY,
			 "0212 DSM out state %d on NPort x%x Data: x%x, %p\n",
			 rc, ndlp->nlp_DID, ndlp->nlp_flag, ndlp);

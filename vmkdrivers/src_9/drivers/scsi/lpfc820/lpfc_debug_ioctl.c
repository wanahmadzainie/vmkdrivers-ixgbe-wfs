...
lpfc_process_ioctl_dfc(struct lpfc_hba * phba, struct lpfcCmdInput * cip)
{
	...
	case LPFC_RESET:
		...
		break;
	case LPFC_INDUCE_DUMP:
		rc = lpfc_sli4_pdev_reg_request(phba, LPFC_FW_DUMP);
		break;
	...
	case LPFC_IOCTL_NODE_STAT:
		rc = lpfc_ioctl_node_stat(phba, cip, dataout, -1);
		break;
	}
...
lpfc_ioctl_reset (struct lpfc_hba * phba, struct lpfcCmdInput * cip)
{
	...
	/* Note: Spec says to default to Selective Reset */
	...
	case 2: /* Coordinated Reset */
		...
		break;
	case 3: /* FW Reset */
		rc = lpfc_sli4_pdev_reg_request(phba, LPFC_FW_RESET);
		if (rc == -EPERM)
			rc = lpfc_selective_reset(phba);
		break;
	default:
			  ...
			  void *dataout, int vpi)
{
	...
	struct DFC_NodeStatEntry_V2 *np = vplist->nodeStat;
	HBA_WWN vport_wwpn;
	...
	if (vpi < 0) {
		if (copy_from_user(&vport_wwpn, cip->lpfc_arg1,
				   sizeof(vport_wwpn)))
			return EIO;
	}
	...
	for (i = 0; i <= phba->max_vpi && vports[i] != NULL; i++) {
		if (vports[i]->load_flag & (FC_UNLOADING | FC_LOADING))
			continue;
		if (vports[i]->vport_flag & DIAG_VPORT)
			continue;
		if (!memcmp(&vport_wwpn, &vports[i]->fc_portname,
			    sizeof(HBA_WWN)) || (vports[i]->vpi == vpi)) {
			found = 1;
			vport = vports[i];
	...
	list_for_each_entry(ndlp, &vport->fc_nodes, nlp_listp) {
		if (entry > numberOfEntries)
			...
			if (ndlp->nlp_state == NLP_STE_MAPPED_NODE) {
				...
				np->TargetIOCnt = ndlp->tx_cnt;
				np->TargetFrameDropCnt = ndlp->frame_drop_cnt;
				np->TargetUnderrunCnt = ndlp->underrun_cnt;
				np->TargetOverrunCnt = ndlp->overrun_cnt;
				strncpy(np->vmhba,

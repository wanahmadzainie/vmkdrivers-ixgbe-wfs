 ...
 **/
void lpfc_discovery_wait(struct lpfc_vport *vport)
{
		       ...
		       uint8_t *vname, uint8_t *wwnn, uint8_t *wwpn)
{
	...
	if (!(strcmp(vname, DIAG_VPORT_NAME)))
		type = DIAG_VPORT;

#ifndef fc_host_vports
	vport = lpfc_create_port(phba, instance, &shost->shost_gendev, type);
#else
	if (fc_vport)
		vport = lpfc_create_port(phba, instance, &fc_vport->dev, type);
	else

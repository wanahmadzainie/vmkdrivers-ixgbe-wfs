...
lpfc_sli4_eq_clr_intr(struct lpfc_queue *q)
{
	...
	/* Flush PCI pipline for INTX mode only since it is level-sensitive. */
	if (q->phba->intr_type == INTx)
...
{
	struct lpfc_pgp *pgp = (phba->sli_rev == LPFC_SLI_REV3) ?
				...
				struct lpfc_sli_ring *pring, uint32_t mask)
{
	...
	IOCB_t *irsp = NULL;
	IOCB_t *entry = NULL;
	struct lpfc_iocbq *cmdiocbq = NULL, *next_iocb;
	...
	if (atomic_dec_and_test(&phba->fcp_ring_in_use))
		writel(mask, phba->HAregaddr);
	else {
		atomic_inc(&phba->fcp_ring_in_use);
		return 1;
	}

	INIT_LIST_HEAD(&lpfc_iocb_list);
	spin_lock_irqsave(&phba->hbalock, iflag);
	...
	while (pring->rspidx != portRspPut) {
		 ...
		 */
		if (unlikely(sd_test)) {
			if (likely(irsp->ulpStatus == 0))
				dfc_inject_qdepth_down(phba, irsp);
		}
	...
	spin_unlock_irqrestore(&phba->hbalock, iflag);
	list_for_each_entry_safe(cmdiocbq, next_iocb,
			&lpfc_iocb_list, list) {
		list_del(&cmdiocbq->list);
		rspiocbq.iocb.ulpStatus = 0;
		(cmdiocbq->iocb_cmpl)(phba, cmdiocbq,
				      &rspiocbq);
	}
...
lpfc_sli_set_dma_length(struct lpfc_hba * phba, uint32_t polling)
{
	...
	if ((FC_JEDEC_ID(phba->vpd.rev.biuRev) != PEGASUS_JEDEC_ID) &&
		...
		phba->hba_flag &= ~HBA_DMA_RESTRICT;
		return 0;
	}
	...
	if (ret != MBX_SUCCESS) {
		if (mbox->u.mb.mbxStatus != MBXERR_UNKNOWN_CMD)
			lpfc_printf_log(phba, KERN_ERR, LOG_INIT | LOG_LIBDFC,
				"%d:0443 Adapter failed to set maximum"
				" DMA length mbxStatus x%x \n",
				phba->brd_no, mbox->u.mb.mbxStatus);
		else
			lpfc_printf_log(phba, KERN_INFO, LOG_INIT | LOG_LIBDFC,
				"%d:0508 Adapter doesn't implement set maximum"
				" DMA length mbxStatus x%x \n",
				phba->brd_no, mbox->u.mb.mbxStatus);
...
lpfc_sli_hba_setup(struct lpfc_hba *phba)
{
	...
	/*
	 * The policy for sli modes: 0(auto), 1, or 3 is to try SLI-3
	 * first.  If this an SLI-2 HBA trying SLI-3 mode, it will
	 * fallback to SLI-2 mode.
	 */
...
#if defined(__VMKLNX__)
		/* Setting the flag thats usually set by lpfc_parse_vpd */
...
lpfc_mbox_timeout_handler(struct lpfc_hba *phba)
{
	...
	/*
	  Dump WQs and CQs for SLI4 HBAs.
	*/
	if (phba->sli_rev == LPFC_SLI_REV4) {
...
#if defined(__VMKLNX__)
		bf_set(wqe_tmo, &wqe->fcp_iwrite.wqe_com,
			iocbq->iocb.ulpTimeout);
#endif
		break;
	case CMD_FCP_IREAD64_CR:
...
#if defined(__VMKLNX__)
		bf_set(wqe_tmo, &wqe->fcp_iread.wqe_com,
			iocbq->iocb.ulpTimeout);
#endif
		break;
	case CMD_FCP_ICMND64_CR:
		...
		/* Set ulpTimeout for ESX */
#if defined(__VMKLNX__)
		bf_set(wqe_tmo, &wqe->fcp_icmd.wqe_com,
			iocbq->iocb.ulpTimeout);
#endif
		break;
	case CMD_GEN_REQUEST64_CR:
			 ...
			 struct lpfc_iocbq *piocb, uint32_t flag)
{
	...
	if ((piocb->iocb_flag & LPFC_IO_FCP) ||
		...
		/*
		 * For FCP command IOCB, round-robin the WQ index to distribute
		 * WQE across the WQs. For ABTS operations, the IOCB already
		 * has the same WQ index to the original command IOCB.  The
		 * same is true for multi-queue FCP IOCBs.
		 */
		if ((piocb->iocb_flag & LPFC_IO_FCP) &&
		    (!(piocb->iocb_flag & LPFC_USE_FCPWQIDX)))
...
lpfc_sli_setup(struct lpfc_hba *phba)
{
	...
	for (i = 0; i < psli->num_rings; i++) {
		...
		case LPFC_FCP_RING:	/* ring 0 - FCP */
			...
			pring->numCiocb += SLI2_IOCB_CMD_R3XTRA_ENTRIES;
			pring->numRiocb += SLI2_IOCB_RSP_R3XTRA_ENTRIES;
			pring->sizeCiocb = (phba->sli_rev == LPFC_SLI_REV3) ?
							SLI3_IOCB_CMD_SIZE :
							SLI2_IOCB_CMD_SIZE;
			pring->sizeRiocb = (phba->sli_rev == LPFC_SLI_REV3) ?
							SLI3_IOCB_RSP_SIZE :
							SLI2_IOCB_RSP_SIZE;
			pring->iotag_ctr = 0;
			pring->iotag_max =
...
lpfc_sli_sum_iocb(struct lpfc_vport *vport, uint16_t tgt_id, uint64_t lun_id,
		  lpfc_ctx_cmd ctx_cmd, unsigned long tstamp, uint16_t flag)
{
	struct lpfc_hba *phba = vport->phba;
	struct lpfc_iocbq *iocbq;
	int sum, i;
	...
	for (i = 1, sum = 0; i <= phba->sli.last_iotag; i++) {
		...
		if (lpfc_sli_validate_fcp_iocb (iocbq, vport, tgt_id, lun_id,
						ctx_cmd) == 0) {
			lpfc_cmd = container_of(iocbq,
						struct lpfc_scsi_buf,
						cur_iocbq);
			if (time_after(tstamp, lpfc_cmd->start_time)) {
				if (flag == 0)
					sum++;
				else if (iocbq->iocb_flag & flag)
					sum++;
			}
		}
	}
		    ...
		    uint16_t flag)
{
	...
	for (i = 1; i <= phba->sli.last_iotag; i++) {
		...
		iocbq->iocb_flag |= flag;
		lpfc_cmd = (struct lpfc_scsi_buf *) iocbq->context1;
			 ...
			 uint32_t timeout)
{
	...
	if (retval == IOCB_SUCCESS) {
		...
		piocb->iocb_cmpl = NULL;
		lpfc_iocb_wake = piocb->iocb_flag & LPFC_IO_WAKE;
		spin_unlock_irqrestore(&phba->hbalock, iflags);

		if (timeleft == 0) {
			lpfc_printf_log(phba, KERN_ERR, LOG_SLI,
					"0338 IOCB wait timeout error - no "
					"wake response Data x%x x%x\n", timeout,
					lpfc_iocb_wake);
			retval = IOCB_TIMEDOUT;
		} else if (lpfc_iocb_wake == 0) {
			lpfc_printf_log(phba, KERN_ERR, LOG_SLI,
			     ...
			     struct lpfc_wcqe_complete *wcqe)
{
	 ...
	 */
	if (unlikely(sd_test)) {
		if (likely(bf_get(lpfc_wcqe_c_status, wcqe)) == 0)
			dfc_inject_qdepth_down_s4(phba, wcqe);
	}
...
/** lpfc_sli4_check_fq_eq -- Fast-path completion look-ahead
 * @phba: Pointer to HBA context object.
 *
 * This function calls the interrupt handler looking for I/O completions.
 * By processing completions in the queuecommand code path, its intention
 * is to reduce interrupt context switching.
 **/
void
lpfc_sli4_check_fp_eq(struct lpfc_hba *phba, uint32_t scan_eqid)
{
	uint32_t fcp_eqidx;
...
#endif
{
	...
	if (unlikely(ecount == 0)) {
		if (phba->intr_type == MSIX) {
			if (!phba->look_ahead) {
				/*
				 * MSI-X treated interrupt served as no
				 * EQ share INT
				 */
				lpfc_printf_log(phba, KERN_WARNING, LOG_SLI,
						"0358 MSI-X interrupt "
						"with no EQE\n");
			}
		} else {
			atomic_inc(&fcp_eq_hdl->fcp_eq_in_use);
...
/**
 * lpfc_sli4_fcf_record_get - Get a FCF record from HBA at the FCF index
 * @phba: pointer to lpfc hba data structure.
 * @fcf_record: pointer to the buffer for getting the FCF record into.
 * @fcf_index: the index to the HBA FCF record to be obtained.
 *
 * This routine will get the FCF record from the HBA according to the @fcf_index
 * provided.  The caller should allocate a buffer for the obtained FCF record
 * and pass the pointer @fcf_record in. For the first entry of the HBA FCF table
 * the caller should pass in LPFC_FCOE_FCF_GET_FIRST to the @fcf_index. Each
 * time a FCF record is obtained from HBA, the routine will copy it into the
 * buffer pointed by the @fcf_record and return the index to the next FCF
 * record back. Once the last FCF record is returned, this routine returns
 * LPFC_FCOE_FCF_NEXT_NONE.
 *
 * This routine is primarily called by the IOCTL modules, but could
 * be called by any routine that wants a synchronous behavior to an
 * fcf_record fetch.
 *
 * Return:
 *     The index to the next FCF record in HBA.
 **/
uint16_t
lpfc_sli4_fcf_record_get(struct lpfc_hba *phba, struct fcf_record *fcf_record,
			 uint16_t fcf_index)
{
	LPFC_MBOXQ_t *mboxq;
	...
	uint32_t mbox_tmo;
	uint32_t shdr_status, shdr_add_status;
	struct fcf_record *fcf_record_ptr;
	union lpfc_sli4_cfg_shdr *shdr;
	struct lpfc_mbx_sge sge;
	dma_addr_t phys_addr;
	void *virt_addr;
	...
	/*
	 * Get the first SGE entry from the non-embedded mbox command DMA
	 * memory.  This routine only uses a single SGE.
	 */
	lpfc_sli4_mbx_sge_get(mboxq, 0, &sge);
	phys_addr = getPaddr(sge.pa_hi, sge.pa_lo);
	if (unlikely(!mboxq->sge_array)) {
		next_fcf_index = LPFC_FCOE_FCF_NEXT_NONE;
		goto out;
	}
	virt_addr = mboxq->sge_array->addr[0];
	...
	if (shdr_status || shdr_add_status) {
		lpfc_printf_log(phba, KERN_ERR, LOG_INIT,
			"2522 Synchronous READ_FCF_RECORD mailbox failed "
	...
	/* Get the next FCF record index */
	read_fcf = (struct lpfc_mbx_read_fcf_tbl *)virt_addr;
	lpfc_sli_pcimem_bcopy((void *)read_fcf, (void *)read_fcf,
			      sizeof(struct lpfc_mbx_read_fcf_tbl));
	next_fcf_index = bf_get(lpfc_mbx_read_fcf_tbl_nxt_vindx, read_fcf);

	/* Copy the FCF record to the buffer provided*/
	fcf_record_ptr = (struct fcf_record *)(virt_addr +
			  sizeof(struct lpfc_mbx_read_fcf_tbl));
	lpfc_sli_pcimem_bcopy((void *)fcf_record_ptr,
			      (void *)fcf_record, sizeof(struct fcf_record));
 ...
 */
uint32_t random32(struct lpfc_hba *phba)
{
	unsigned long r;
	struct rnd_state *state = &phba->random_state;
	r = __random32(state);
	return r;
}
 ...
 */
void srandom32(struct lpfc_hba *phba, uint32_t entropy)
{
	struct rnd_state *state = &phba->random_state;
 ...
 */
int random32_init(struct lpfc_hba *phba)
{
	struct rnd_state *state = &phba->random_state;

#define LCG(x)	((x) * 69069)	/* super-duper LCG */
	state->s1 = __seed(LCG(0 + jiffies), 1);
 ...
 *	is fully initalized.
 */
int random32_reseed(struct lpfc_hba *phba)
{
	struct rnd_state *state = &phba->random_state;
	uint32_t seeds[3];
...
/**
 * lpfc_stop_chip - Stop chip.
 * @phba: Pointer to HBA context object.
 *
 * This function stop BE SLI4 chip.
 **/
void
lpfc_sli4_stop_chip(struct lpfc_hba *phba)
{
	uint32_t if_type;
	...
	printk(KERN_ERR "lpfc%d stoping chip \n", phba->brd_no);
	writel(0xFFFFFFFF, phba->sli4_hba.ctrl_regs_memmap_p);
	writel(0xFFFFFFFF, phba->sli4_hba.conf_regs_memmap_p + 0xAC);
	writel(0xFFFFFFFF, phba->sli4_hba.conf_regs_memmap_p + 0xA8);
	writel(0, phba->sli4_hba.conf_regs_memmap_p + 0xB0);
	writel(0, phba->sli4_hba.conf_regs_memmap_p + 0xB4);
	printk(KERN_ERR "Stopped CHIP!!!!!\n");
}

/**
 * lpfc_dump_wq - This function print contents of a work queue to the kernel log.
 * @phba: Pointer to HBA context object.
 * @q: Pointer to work queue.
 *
 * This function print the pointers and the contents of the specified work queue
 * to kernel log file.
 **/
void
lpfc_dump_wq(struct lpfc_hba *phba,
	     struct lpfc_queue *q)
{
	int i;
	uint32_t *word;
	...
	printk(KERN_ERR "lpfc%d count %d id %d host_indx %d hba_indx %d \n",
	       phba->brd_no, q->entry_count, q->queue_id, q->host_index,
	       q->hba_index);

	for (i = 0; i < q->entry_count; i++) {
		word = (uint32_t *) q->qe[i].wqe;
		printk(KERN_ERR "lpfc%d:%02d:%03d:[0]: %08x %08x %08x"
		       " %08x %08x %08x %08x %08x\n",
		       phba->brd_no, q->queue_id, i,
		       word[0], word[1], word[2], word[3],
		       word[4], word[5], word[6], word[7]);
		word += 8;
		printk(KERN_ERR "lpfc%d:%02d:%03d:[8]: %08x %08x %08x "
		       "%08x %08x %08x %08x %08x\n",
		       phba->brd_no, q->queue_id, i,
...
/**
 * lpfc_dump_fcp_wq - This function print contents of FCP work queue to the kernel log.
 * @phba: Pointer to HBA context object.
 *
 * This function print the pointers and the contents of the specified work queue
 * to kernel log file.
 **/
void
lpfc_dump_fcp_wq(struct lpfc_hba *phba)
{
	int i;
	...
	printk(KERN_ERR "lpfc%d FCP WQ entries \n", phba->brd_no);
	for (i = 0; i < phba->cfg_fcp_wq_count; i++) {
		printk(KERN_ERR "lpfc%d FCP WQ index %d \n", phba->brd_no, i);
		lpfc_dump_wq(phba, phba->sli4_hba.fcp_wq[i]);
	}
}

/**
 * lpfc_dump_cq - This function print contents of a CQ to the kernel log.
 * @phba: Pointer to HBA context object.
 * @cq: Pointer to a completion queue.
 *
 * This function print the pointers and the contents of the specified
 * completion queue to kernel log file.
 **/
void
lpfc_dump_cq(struct lpfc_hba *phba, struct lpfc_queue *cq)
{
	struct lpfc_cqe *cqe1, *cqe2;
	int i;

	if (phba->sli_rev != LPFC_SLI_REV4)
		return;

	printk(KERN_ERR "CQ HBA_index 0x%x Host_index 0x%x \n",
	       cq->hba_index, cq->host_index);
	for (i = 0; i < cq->entry_count; i += 2) {
		cqe1 = cq->qe[i].cqe;
		if (i+1 < cq->entry_count)
			cqe2 = cq->qe[i+1].cqe;
		else
			cqe2 = NULL;

		printk(KERN_ERR "CQE[%d] %08x %08x %08x %08x "
		       "CQE[%d] %08x %08x %08x %08x \n",
		       i, cqe1->reserved0, cqe1->reserved1,
		       cqe1->reserved2, cqe1->word3,
		       i+1, (cqe2) ? cqe2->reserved0 : 0,
		       (cqe2) ? cqe2->reserved1 : 0,
		       (cqe2) ? cqe2->reserved2 : 0,
		       (cqe2) ? cqe2->word3 : 0);
	}
}

/**
 * lpfc_dump_fcp_cq - This function print contents of a FCP CQ to the kernel log.
 * @phba: Pointer to HBA context object.
 *
 * This function print the pointers and the contents of all FCP CQs
 * to kernel log file.
 **/
void
lpfc_dump_fcp_cq(struct lpfc_hba *phba)
{
	uint8_t fcp_eqidx;
	...
	printk(KERN_ERR "lpfc%d FCP CQ entries \n", phba->brd_no);
	for (fcp_eqidx = 0; fcp_eqidx < phba->cfg_fcp_eq_count; fcp_eqidx++) {
		printk(KERN_ERR "lpfc%d FCP CQ index %d \n",
		       phba->brd_no, fcp_eqidx);
		lpfc_dump_cq(phba, phba->sli4_hba.fcp_cq[fcp_eqidx]);
	}
}

/**
 * lpfc_dump_mb_queue - This function print contents of mailbox queue.
 * @phba: Pointer to HBA context object.
 *
 * This function print the pointers and the contents of mailbox queue
 * to kernel log file.
 **/
static void
lpfc_dump_mb_queue(struct lpfc_hba *phba)
{
	int i, j;
	uint32_t *word;
	struct lpfc_queue *q;

	q = phba->sli4_hba.mbx_wq;
	...
	for (i = 0; i < q->entry_count; i++) {
		...
		for (j = 0; j < 8; j++)
			printk(KERN_ERR "lpfc%d:%02d:%03d:[%d]: %08x %08x %08x"
			       " %08x %08x %08x %08x %08x\n",
			       phba->brd_no, q->queue_id, i, j*8,
			       word[j*8+0], word[j*8+1], word[j*8+2],
			       word[j*8+3], word[j*8+4],
			       word[j*8+5], word[j*8+6], word[j*8+7]);
	}
}


/**
 * lpfc_dump_mb_cq - This function print contents of mailbox cq.
 * @phba: Pointer to HBA context object.
 *
 * This function print the pointers and the contents of mailbox cq
 * to kernel log file.
 **/
static void
lpfc_dump_mb_cq(struct lpfc_hba *phba)
{
	struct lpfc_queue *cq = phba->sli4_hba.mbx_cq;
	uint32_t *mqe;
	int i;

	printk(KERN_ERR "MB CQ HBA_index 0x%x Host_index 0x%x \n",
	       cq->hba_index, cq->host_index);

	for (i = 0; i < cq->entry_count; i++) {
		mqe = (uint32_t *) cq->qe[i].cqe;
		printk(KERN_ERR "CQE[%d] %08x %08x %08x %08x\n",
		       i, mqe[0], mqe[1], mqe[2], mqe[3]);
	}
}

/**
 * lpfc_dump_mbox_queue - This function print contents of mailbox queue and CQ.
 * @phba: Pointer to HBA context object.
 *
 * This function print the pointers and the contents of mailbox queue and CQ
 * to kernel log file.
 **/
void
lpfc_dump_mbox_queue(struct lpfc_hba *phba)
{
	if (phba->sli_rev != LPFC_SLI_REV4)
		return;

	printk(KERN_ERR "lpfc%d mailbox work queue entries\n",
	       phba->brd_no);
	lpfc_dump_mb_queue(phba);
	msleep(1000);
	printk(KERN_ERR "lpfc%d mailbox completion queue entries\n",
	       phba->brd_no);
	lpfc_dump_mb_cq(phba);
}

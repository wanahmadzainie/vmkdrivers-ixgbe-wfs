...
lpfc_is_persistent_link_disable(struct lpfc_hba *phba)
{
	return (((phba->sli_rev == LPFC_SLI_REV4) &&
		(phba->hba_flag & HBA_FCOE_MODE)) ? false : true);
}
...
static int lpfc_ioctl_mbox_ext(struct lpfc_hba *, struct lpfcCmdInput *,
			       void *);
...
static int dfc_rsp_data_copy(struct lpfc_hba *, uint8_t *,
			     struct lpfc_dmabufext *, uint32_t, uint8_t);
...
static int lpfc_ioctl_get_fcf_list(struct lpfc_hba *, struct lpfcCmdInput *,
				   void *);
static int lpfc_ioctl_vport_create(struct lpfc_hba *, struct lpfcCmdInput *);
static int lpfc_ioctl_vport_delete(struct lpfc_hba *, struct lpfcCmdInput *);
			 ...
			 struct lpfcCmdInput * cip)
{
	...
	if (cip->lpfc_ring)
		memcpy((uint8_t *)&arg3, (uint8_t *)cip->lpfc_arg3,
			(ulong)(sizeof(IOargUn)));
	else if (copy_from_user((uint8_t *) &arg3, (uint8_t *) cip->lpfc_arg3,
			   (ulong) (sizeof (IOargUn)))) {
		rc = EIO;
		goto sndsczout;
	}
	 ...
	 */
	if (cip->lpfc_ring)
		memcpy((uint8_t *)fcpcmd, (uint8_t *)cip->lpfc_arg1,
			(ulong)(reqbfrcnt));
	else if (copy_from_user((uint8_t *) fcpcmd, (uint8_t *) cip->lpfc_arg1,
			   (ulong) (reqbfrcnt))) {
		rc = EIO;
		goto sndsczout;
	}
	...
	if (phba->sli_rev == LPFC_SLI_REV4)
		cmd->ulpContext = phba->sli4_hba.rpi_ids[pndl->nlp_rpi];
	...
	if ((cip->lpfc_cmd == LPFC_HBA_SEND_FCP) &&
		...
		if (cip->lpfc_ring)
			memcpy((uint8_t *) cip->lpfc_arg2,
				     (uint8_t *)fcprsp, count.snscnt);
		else if (copy_to_user((uint8_t *) cip->lpfc_arg2,
				     (uint8_t *) fcprsp, count.snscnt)) {
			rc = EIO;
			goto sndsczout;
		}
 ...
 cpdata:
		...
		/* Return data length */
		if (cip->lpfc_ring)
			memcpy((uint8_t *)cip->lpfc_arg3, (uint8_t *)&count,
			       (2 * sizeof(uint32_t)));
		else if (copy_to_user((uint8_t *)cip->lpfc_arg3,
				(uint8_t *)&count, (2 * sizeof(uint32_t)))) {
			rc = EIO;
			break;
		}
...
lpfc_ioctl_send_els(struct lpfc_hba * phba, struct lpfcCmdInput * cip, void *dataout)
{
	...
	/* Allow this subset of ELS commands to be issued as long as
	 * the node is logged in. */
	if (ndlp &&
	    !(lpfc_is_unmapped_node_logged_in(phba->pport, ndlp->nlp_DID)) &&
	    !(lpfc_is_mapped_node_logged_in(phba->pport, ndlp->nlp_DID))) {
		lpfc_sli_release_iocbq(phba, rspiocbq);
		return ENODEV;
	}
	...
	/* Remove ndlp reference from above.  lpfc_prep_els_iocb has taken
	 * a reference which will be released in lpfc_els_free_iocb*/
	lpfc_nlp_put(ndlp);
	...
	/*
	 * ulpContext of rpi is valid only for ECHO.  All other
	 * ELS commands use ulpContext VPI as initialized by
	 * lpfc_prep_els_iocb
	 */
			 ...
			 struct lpfcCmdInput * cip, void *dataout)
{
	...
	cip->lpfc_outsz = 0;
	rc = dfc_rsp_data_copy(phba, cip->lpfc_arg2, outdmp, outdmp->flag, 0);
	if (rc)
		rc = EIO;
...
/**
 * lpfc_ioctl_mbox_ext - Ioctl interface for sli-cfg mailbox command
 * with extended buffer.
 * @phba: Pointer to hba structure
 * @cip->lpfc_arg1: Pointer to User buffer containing Extended SLI Config Mbox
 * @cip->lpfc_arg3: Set to MAILBOX_READ or MAILBOX_WRITE
 * @cip->lpfc_arg4: Set to MAILBOX_NONEMBEDDED or MAILBOX_EMBEDDED
 * @cip->lpfc_arg5: Set to size of user buffer containing extended mbox input)
 * @cip->outsz:     Set to size of user buffer containing extended mbox (output)
 * @cip->dataout:   Pointer to User buffer containing Extended SLI Config Mbox
 *
 * This routine is the IOCTL interface for the SLI4_CFG mailbox command with
 * extended buffer.
 *
 * Return code
 *   0 - User buffer contains completed mailbox command
 *   EAGAIN - Mailbox command should be retried
 *   ENOMEM - No memory for operation
 *   EIO -  Something wrong with command arguments.
 **/

int
lpfc_ioctl_mbox_ext(struct lpfc_hba *phba,
		    struct lpfcCmdInput *cip,
		    void *dataout)
{
	int rc;
	struct lpfc_sysfs_mbox mbox_ext;

	memset(&mbox_ext, 0, sizeof(struct lpfc_sysfs_mbox));

	if (phba->sli.sli_flag & LPFC_BLOCK_MGMT_IO)
		return EAGAIN;

	/* Sanity check on buffer length */
	if (cip->lpfc_arg5 != cip->lpfc_outsz)
		return ENOMEM;
	/*
	 * Optimization: Do not copy the extended data from the user for
	 * read commands. Just copy the mailbox buffer along with the first
	 * extended buffer which has the data length.
	 */
	if ((ulong)cip->lpfc_arg3 == MAILBOX_WRITE) {
		if (copy_from_user((uint8_t *)dataout,
				   (uint8_t *)cip->lpfc_arg1,
				   cip->lpfc_arg5))
			return EFAULT;
	} else if ((ulong)cip->lpfc_arg3 == MAILBOX_READ) {
		if (copy_from_user((uint8_t *)dataout,
				   (uint8_t *)cip->lpfc_arg1,
				   min((uint32_t)cip->lpfc_arg5,
				       (uint32_t)MAILBOX_MGMT_MAX)))

			return EFAULT;
	} else
		return EINVAL;

	mbox_ext.mbext = dataout;
	rc = lpfc_mgmt_issue_sli_cfg_ext_mbox(phba, &mbox_ext);

	/* Check if success */
	if (rc == SLI_CONFIG_HANDLED) {
		/*
		 * Optimization: Do not copy the extended data to user
		 * space on write commands. Just copy the mailbox buffer.
		 */
		if ((ulong)cip->lpfc_arg3 == MAILBOX_WRITE)
			cip->lpfc_outsz = sizeof(MAILBOX_t);
		return 0;
	}
...
lpfc_ioctl_mbox(struct lpfc_hba *phba, struct lpfcCmdInput *cip, void *dataout)
{
	...
	if (copy_from_user((uint8_t *) pmbox, (uint8_t *) cip->lpfc_arg1,
		...
		if ((pmbox->mbxCommand == MBX_READ_SPARM) ||
		    ...
		    (pmbox->mbxCommand == MBX_READ_TOPOLOGY)) {
...
/**
 * lpfc_ioctl_diag_mode_enter - process preparing into device diag loopback mode
 ...
 */
static int
lpfc_ioctl_diag_mode_enter(struct lpfc_hba *phba)
{
...
/**
 * lpfc_ioctl_diag_mode_exit - exit process from device diag loopback mode
 * @phba: Pointer to HBA context object.
 *
 ...
 */
static void
lpfc_ioctl_diag_mode_exit(struct lpfc_hba *phba)
{
	struct Scsi_Host *shost;
	struct lpfc_vport **vports;
	int i;
...
static int
lpfc_sli3_ioctl_diag_loopback_mode(struct lpfc_hba *phba,
				   struct lpfcCmdInput *cip, void *dataout)
{
	uint32_t link_flags;
	uint32_t timeout;
	...
	rc = lpfc_ioctl_diag_mode_enter(phba);
	if (rc)
		goto error_out;

	link_flags = cip->lpfc_arg4;
	timeout = cip->lpfc_arg5 * 100;
...
loopback_mode_exit:
	lpfc_ioctl_diag_mode_exit(phba);
...
/**
 * lpfc_sli4_ioctl_set_link_diag_state - set sli4 link diag state
 * @phba: Pointer to HBA context object.
 ...
 */
static int
lpfc_sli4_ioctl_set_link_diag_state(struct lpfc_hba *phba, uint32_t diag)
{
	LPFC_MBOXQ_t *pmboxq;
...
/**
 * lpfc_sli4_ioctl_set_internal_loopback - set sli4 internal loopback diagnostic
 * @phba: Pointer to HBA context object.
 *
 * This function is responsible for issuing a sli4 mailbox command for setting
 ...
 */
static int
lpfc_sli4_ioctl_set_internal_loopback(struct lpfc_hba *phba)
{
	LPFC_MBOXQ_t *pmboxq;
	uint32_t req_len, alloc_len;
...
/**
 * lpfc_sli4_ioctl_diag_loopback_mode - process an sli4 ioctl vendor command
 * @phba: Pointer to HBA context object.
 * @cip: Pointer to the user command input.
 * @dataout: Pointer to the user data output.
 *
 * This function is responsible for placing an sli4 port into diagnostic
 * loopback mode in order to perform a diagnostic loopback test.
 */
static int
lpfc_sli4_ioctl_diag_loopback_mode(struct lpfc_hba *phba,
				   struct lpfcCmdInput *cip, void *dataout)
{
	...
	rc = lpfc_sli4_ioctl_set_link_diag_state(phba, 1);
	if (rc) {
		lpfc_printf_log(phba, KERN_WARNING, LOG_LIBDFC,
	...
	if (link_flags == INTERNAL_LOOP_BACK)
		rc = lpfc_sli4_ioctl_set_internal_loopback(phba);
...
loopback_mode_exit:
	/* clear loopback diagnostic mode on error out */
	if (rc) {
		spin_lock_irq(&phba->hbalock);
		phba->link_flag &= ~LS_LOOPBACK_MODE;
		spin_unlock_irq(&phba->hbalock);
...
/**
 * lpfc_ioctl_diag_loopback_mode - ioctl vendor command for diag loopback mode
 * @phba: Pointer to HBA context object.
 * @cip: Pointer to the user command input.
 * @dataout: Pointer to the user data output.
 *
 ...
 */
static int
lpfc_ioctl_diag_loopback_mode(struct lpfc_hba *phba,
			      struct lpfcCmdInput *cip, void *dataout)
{
	int rc;

	if (phba->sli_rev < LPFC_SLI_REV4)
		rc = lpfc_sli3_ioctl_diag_loopback_mode(phba, cip, dataout);
	else if (bf_get(lpfc_sli_intf_if_type, &phba->sli4_hba.sli_intf) ==
		 LPFC_SLI_INTF_IF_TYPE_2) {
		/*
		 * Mix-n-match:  conversion neeed for loopback parameter
		 */
		switch (cip->lpfc_arg4) {
		case FLAGS_LOCAL_LB:
			cip->lpfc_arg4 = INTERNAL_LOOP_BACK;
			break;
		case FLAGS_TOPOLOGY_MODE_LOOP:
			cip->lpfc_arg4 = EXTERNAL_LOOP_BACK;
			break;
		default:
			return ENODEV;
		}
		rc = lpfc_sli4_ioctl_diag_loopback_mode(phba, cip, dataout);
	}
	else
		rc = ENODEV;
...
/**
 * lpfc_sli4_ioctl_diag_mode_end - sli4 ioctl vendor cmd for ending diag mode
 * @phba: Pointer to HBA context object.
 * @cip: Pointer to the user command input.
 * @dataout: Pointer to the user data output.
 *
 ...
 */
static int
lpfc_sli4_ioctl_diag_mode_end(struct lpfc_hba *phba,
			      struct lpfcCmdInput *cip, void *dataout)
{
	uint32_t timeout;
	int i, rc = 0;
	...
	rc = lpfc_sli4_ioctl_set_link_diag_state(phba, 0);
...
/**
 * lpfc_sli4_ioctl_link_diag_test - sli4 ioctl vendor command for diag link test
 * @phba: Pointer to HBA context object.
 * @cip: Pointer to the user command input.
 * @dataout: Pointer to the user data output.
 *
 ...
 */
static int
lpfc_sli4_ioctl_link_diag_test(struct lpfc_hba *phba,
			       struct lpfcCmdInput *cip, void *dataout)
{
	LPFC_MBOXQ_t *pmboxq;
	...
	link_diag_test_cmd = &cip->diaglnk;
	timeout = cip->lpfc_arg5 * 100;
	...
	diagsta.mbox_status = mbxstatus;
	diagsta.shdr_status = shdr_status;
	diagsta.shdr_add_status = shdr_add_status;

	rc = copy_to_user((void __user *)&cip->diagsta, &diagsta,
		     ...
		     uint16_t *txxri, uint16_t * rxxri)
{
	...
	*rxxri = 0;
	...
	cmd->ulpContext = rpi;
	if (phba->sli_rev == LPFC_SLI_REV4)
		cmd->ulpContext = phba->sli4_hba.rpi_ids[rpi];
		   ...
		   struct lpfcCmdInput *cip, void *dataout)
{
	...
	if ((rc != IOCB_SUCCESS) || ((phba->sli_rev < LPFC_SLI_REV4) &&
				    (rsp->ulpStatus != IOCB_SUCCESS))) {
		lpfc_printf_log(phba, KERN_ERR, LOG_LIBDFC,
				"3142 Failed loopback test issue iocb: "
				"iocb_stat:x%x\n", rc);
		rc = EIO;
		goto err_loopback_test_exit;
	}
		...
		if ((CHECKLIST_BIT_NPIV + CHECKLIST_BIT_SLI3 + 
		    ...
		    (checklist & 
		     (CHECKLIST_BIT_NPIV + CHECKLIST_BIT_SLI3 +
		      CHECKLIST_BIT_HBA + CHECKLIST_BIT_RSRC))) {
			if (phba->fc_topology == LPFC_TOPOLOGY_PT_PT)
...
lpfc_ioctl_vport_list(struct lpfc_hba *phba, struct lpfcCmdInput *cip, void *dataout)
{
	...
	if (cip->lpfc_ring)
		memcpy(&numberOfEntries, cip->lpfc_arg1,
			sizeof(numberOfEntries));
	else if (copy_from_user(&numberOfEntries, cip->lpfc_arg1,
				sizeof(numberOfEntries)))
		return EIO;
			  ...
			  void *dataout)
{
	...
	if (cip->lpfc_ring)
		memcpy(&vport_wwpn, cip->lpfc_arg1, sizeof(vport_wwpn));
	else if (copy_from_user(&vport_wwpn, cip->lpfc_arg1,
				sizeof(vport_wwpn)))
		return EIO;
			   ...
			   void *dataout)
{
	...
	regionLen = min((size_t)cip->lpfc_outsz, (size_t)LPFC_BPL_SIZE);
	...
	/* Mix-n-match:  If flag field is populated, it contains the
	 * mailbox dump 'type'.  Otherwise, older libdfc presumed DMP_NV_PARAMS
	 */
	if (cip->lpfc_flag)
		mb->un.varDmp.type = cip->lpfc_flag;
	else
	...
	mb->un.varDmp.resp_offset = cip->lpfc_iocb;
	...
	if (rc) {
		retval = ETIMEDOUT;
		if (rc == MBX_TIMEOUT)
			/* resources will be freed by lpfc_sli_def_mbox_cmpl */
			return retval;
		else
			goto DONE;
	}
			     ...
			     void *dataout)
{
	...
	if ((regionId == 17) && (if_type == LPFC_SLI_INTF_IF_TYPE_0))
		return ENOSYS;
		  ...
		  uint32_t size, uint8_t local)
{
	...
	cnt = 0;
		...
		if (outdataptr) {
			...
			else if (copy_to_user((void __user *)
					      (outdataptr + offset),
					      (uint8_t *)mlast->dma.virt, cnt))
				return 1;
		}
		  ...
		  struct lpfc_dmabufext * inp)
{
	...
	if (phba->sli_rev == LPFC_SLI_REV4) {
		...
		icmd->ulpContext = dfchba->ct_ctx[tag].rxid;
		icmd->unsli3.rcvsli3.ox_id = dfchba->ct_ctx[tag].oxid;
		...
		icmd->un.ulpWord[3] = phba->sli4_hba.rpi_ids[ndlp->nlp_rpi];
			...
			struct lpfc_sli_ring *pring, struct lpfc_iocbq *piocbq)
{
	...
	do {
		...
		if (phba->sli_rev == LPFC_SLI_REV4) {
			...
			dfchba->ct_ctx[evtData->immed_dat].rxid =
...
lpfcdfc_do_ioctl(struct lpfc_hba *phba, struct lpfcCmdInput *cip)
{
	...
	case LPFC_GET_DFC_REV:
		/* MixNMatch version controls library/driver
		 * compatibility.
		 */
		((struct DfcRevInfo *) dataout)->a_Major = DFC_MAJOR_REV;
		((struct DfcRevInfo *) dataout)->a_Minor = DFC_MINOR_REV;
		((struct DfcRevInfo *) dataout)->a_MixNMatch = DFC_MIX_N_MATCH;
		cip->lpfc_outsz = min_value(cip->lpfc_outsz,
					  sizeof(struct DfcRevInfo));
		rc = 0;
		break;
	...
	case LPFC_MBOX_SLI_CFG_EXT:
		rc = lpfc_ioctl_mbox_ext(phba, cip, dataout);
		break;

	case LPFC_MBOX_SLI_CFG_EXT_SUP:
		break; /* This means it's supported */
	...
	case LPFC_LOOPBACK_MODE:
		rc = lpfc_ioctl_diag_loopback_mode(phba, cip, dataout);
		break;
	...
	case LPFC_LINK_DIAG_TEST: 
        	rc = lpfc_sli4_ioctl_link_diag_test(phba, cip, dataout); 
		break; 

	case LPFC_DIAG_MODE_END: 
 		rc = lpfc_sli4_ioctl_diag_mode_end(phba, cip, dataout); 
		break; 
	...
	case LPFC_VPORT_CREATE:
		rc = lpfc_ioctl_vport_create(phba, cip);
		break;

	case LPFC_VPORT_DELETE:
		rc = lpfc_ioctl_vport_delete(phba, cip);
		break;
...
/**
 * lpfc_ioctl_vport_create - create a vport for diagnostics
 * @phba: The Hba for which this call is being executed.
 * @arg1: pointer to wwpn
 * @arg2: pointer to wwnn
 *
 * This routine create a vport specified by the wwpn/wwnn in the
 * arg list.  This function has no parameter testing and is
 * tightly coupled with dfc vpdiag command.
 *
 **/
int
lpfc_ioctl_vport_create(struct lpfc_hba *phba, struct lpfcCmdInput *cip)
{
	struct Scsi_Host *shost;
	struct lpfc_vport *vport;
	uint8_t vport_wwpn[8], vport_wwnn[8];
	uint8_t vname[LPFC_VNAME_LEN];
	int rc = EIO;
	if (copy_from_user(&vport_wwpn, cip->lpfc_arg1, sizeof(vport_wwpn)))
		return rc;
	if (copy_from_user(&vport_wwnn, cip->lpfc_arg2, sizeof(vport_wwnn)))
		return rc;

	shost = lpfc_shost_from_vport(phba->pport);

	strcpy(vname, DIAG_VPORT_NAME);
	rc = __lpfc_vport_create(shost, vport_wwnn, vport_wwpn,
				 vname, &vport);
...
/**
 * lpfc_ioctl_vport_delete - delete diagnostic vport
 * @phba: The Hba for which this call is being executed.
 * @arg1: pointer to wwpn
 *
 * This routine deletes the vport specified by the wwpn in the
 * arg list.  This function has no parameter testing and is
 * tightly coupled with dfc vpdiag command.
 *
 **/
int
lpfc_ioctl_vport_delete(struct lpfc_hba *phba, struct lpfcCmdInput *cip)
{
	struct lpfc_vport *vport;
	uint8_t vport_wwpn[8];
	uint8_t match = 0;
	...
	list_for_each_entry(vport, &phba->port_list, listentry) {
		if (memcmp(&vport->fc_portname, &vport_wwpn,
	...
	return __lpfc_vport_delete(lpfc_shost_from_vport(vport), vport);
}


void
lpfc_diag_shost_remove(unsigned long ptr)
{
	struct Scsi_Host *shost = (struct Scsi_Host *)ptr;
	struct lpfc_vport *vport = shost->shost_data;

	if (shost->shost_state == SHOST_CREATED)
		mod_timer(&vport->diag_timer, jiffies + HZ);
	else if ((shost->shost_state == SHOST_DEL) &&
		 (!(vport->vpi_state & LPFC_VPI_REGISTERED))) {
		del_timer(&vport->diag_timer);
		kfree(shost);
	} else
		mod_timer(&vport->diag_timer, jiffies + HZ);
}
...
struct Scsi_Host *
lpfc_diag_shost_alloc(void)
{
	struct lpfc_vport *vport;
	struct Scsi_Host *shost = kzalloc(sizeof(struct Scsi_Host) +
					  sizeof(struct lpfc_vport),
					  GFP_KERNEL);
	if (!shost)
		return 0;

	shost->shost_data = shost + sizeof(struct Scsi_Host);

	memcpy(&shost->hostdata[0], &shost->shost_data,
	       sizeof(struct lpfc_vport *));
	...
	scsi_host_get(shost);
	vport = (struct lpfc_vport *)shost->shost_data;
	init_timer(&vport->diag_timer);
	vport->diag_timer.function = lpfc_diag_shost_remove;
	vport->diag_timer.data = (unsigned long)shost;
	mod_timer(&vport->diag_timer, jiffies + HZ*5);
	return shost;
}

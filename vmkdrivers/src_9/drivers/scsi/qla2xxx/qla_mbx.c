...
qla2x00_mailbox_command(scsi_qla_host_t *vha, mbx_cmd_t *mcp)
{
	...
	printk("\n");
	printk("%s(%ld): I/O address = %p.\n", __func__, base_vha->host_no, optr);
	...
	if ((!abort_active && io_lock_on) || IS_NOPOLLING_TYPE(ha)) {
		...
		if (IS_QLA82XX(ha)) {
			if (RD_REG_DWORD(&reg->isp82.hint) & HINT_MBX_INT_PENDING) {
				spin_unlock_irqrestore(&ha->hardware_lock, flags);
				DEBUG2_3_11(printk("%s(%ld): Pending Mailbox timeout. "
						"Exiting.\n", __func__, vha->host_no));
	...
	if (ha->flags.mbox_int) {
		...
		if (ha->mailbox_out[0] != MBS_COMMAND_COMPLETE) {
			if (ha->mailbox_out[0] == MBS_INVALID_PRODUCT_KEY) {
				/* Mailbox registers. */
				iptr = (uint16_t *)&ha->mailbox_out[0];
				printk(KERN_INFO "qla2x00_mailbox_command(%ld): "
						"Invalid Product Key!\n", vha->host_no);
				for (cnt = 0; cnt < ha->mbx_count; cnt++) {
					printk("mb%x = %x ", cnt, *iptr);
					iptr++;
				}
				printk("\n");
			}
		...
		if(IS_QLA82XX(ha)) {
			DEBUG11(qla_printk(KERN_WARNING, ha,
				"Host-Status=0x%x Host-Int=0x%x ",
				RD_REG_DWORD(&reg->isp82.host_status),
				RD_REG_DWORD(&reg->isp82.host_int)));
		}
		...
		if (!io_lock_on || (mcp->flags & IOCTL_CMD)) {
			...
			if(!test_bit(ISP_ABORT_NEEDED, &base_vha->dpc_flags) &&
				...
				qla_printk(KERN_WARNING, ha,
					"Mailbox command 0x%x timeout occurred. Scheduling ISP "
					"abort.\n", mcp->mb[0]);
				if (IS_QLA82XX(ha))
			...
			if(!test_bit(ISP_ABORT_NEEDED, &base_vha->dpc_flags) &&
					!test_bit(ABORT_ISP_ACTIVE, &base_vha->dpc_flags)) {
				qla_printk(KERN_WARNING, ha,
					"Mailbox command 0x%x timeout occurred. Issuing ISP "
					"abort.\n", mcp->mb[0]);
				...
				if (IS_QLA82XX(ha)) {
					DEBUG2_3_11(printk("%s(%ld): **** FAILED. mbx0=%x, "
						"mbx1=%x, mbx2=%x, cmd=%x ****\n", __func__,
						base_vha->host_no, mcp->mb[0], mcp->mb[1], mcp->mb[2],
						command));
					return rval;
				}
			}
...
qla2x00_execute_fw(scsi_qla_host_t *vha, uint32_t risc_addr)
{
	...
	if (IS_FWI2_CAPABLE(ha)) {
		...
		if (IS_QLA81XX(ha)) {
			...
			mcp->mb[4] = (nv->enhanced_features & EXTENDED_BB_CREDITS);
		} else
			mcp->mb[4] = 0;
...
qla2x00_get_fw_version(scsi_qla_host_t *vha, uint16_t *major, uint16_t *minor,
    ...
    uint8_t *mpi, uint32_t *mpi_caps, uint8_t *phy)
{
	int		rval;
	mbx_cmd_t	mc;
	mbx_cmd_t	*mcp = &mc;
	...
	*attributes = mcp->mb[6];
	...
	if (IS_QLA81XX(vha->hw)) {
		...
		*mpi_caps = (mcp->mb[12] << 16) | mcp->mb[13];
		...
		printk ("PHY Firmware version: %2d.%02d.%02d ",
				ha->phy_version[0], ha->phy_version[1],
				ha->phy_version[2]);

		printk ("MPI Firmware version: %2d.%02d.%02d (%d)\n",
...
qla2x00_verify_checksum(scsi_qla_host_t *vha, uint32_t risc_addr)
{
	...
	if (rval != QLA_SUCCESS) {
		DEBUG2_3_11(printk("%s(%ld): failed=%x chk sum=%x.\n", __func__,
		    vha->host_no, rval, IS_FWI2_CAPABLE(ha) ?
...
qla2x00_issue_iocb(scsi_qla_host_t *vha, void*  buffer, dma_addr_t phys_addr,
    size_t size)
{
	return qla2x00_issue_iocb_timeout(vha, buffer, phys_addr, size, MBX_TOV_SECONDS);
}
 ...
 */
int
qla2x00_issue_iocb_timeout(scsi_qla_host_t *vha, void*  buffer, dma_addr_t phys_addr,
...
int
qla82xx_get_minidump_template(scsi_qla_host_t *vha, dma_addr_t phys_addr)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	...
	mcp->mb[0] = LSW(MBC_GET_MINIDUMP);
	mcp->mb[1] = MSW(MBC_GET_MINIDUMP);
	mcp->mb[2] = LSW(0x01);
	mcp->mb[3] = MSW(0x01);
	mcp->mb[4] = LSW(phys_addr);
	mcp->mb[5] = MSW(phys_addr);
	mcp->mb[6] = MSW(MSD(phys_addr));
	mcp->mb[7] = LSW(MSD(phys_addr));
	...
	if (rval != QLA_SUCCESS) {
		DEBUG2_3_11(qla_printk(KERN_WARNING, ha,
			"%s(%ld): mailbox command rval=0x%x FAILED=0x%x subcode=0x%x.\n",
...
int
qla82xx_req_template_size(scsi_qla_host_t *vha)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	...
	if (rval != QLA_SUCCESS) {
		DEBUG2_3_11(qla_printk(KERN_ERR, ha,
			"%s(%ld): rval=0x%x mbx0=0x%04x mbx1=0x%04x.\n",
			__func__, vha->host_no, rval, mcp->mb[0], mcp->mb[1]));
	} else {
		ha->md_template_size = ((mcp->mb[3] << 16) | mcp->mb[2]);
		if (ha->md_template_size == 0)
			rval = QLA_PARAMETER_ERROR;
		DEBUG11(qla_printk(KERN_INFO, ha,
			"mb[0]=0x%04x, mb[1]=0x%04x, mb[2]=0x%04x,\n"
			"mb[3]=0x%04x, mb[4]=0x%04x, mb[5]=0x%04x,\n"
			"mb[6]=0x%04x, mb[7]=%04x\n",
			mcp->mb[0], mcp->mb[1], mcp->mb[2], mcp->mb[3],
			mcp->mb[4], mcp->mb[5], mcp->mb[6], mcp->mb[7]));
		DEBUG11(qla_printk(KERN_INFO, ha,
			"%s(%ld): done rval=0x%x.\n", __func__, vha->host_no, rval));
	}
 ...
 */
int
qla2x00_abort_target(fc_port_t *fcport, int tag)
{
	int        rval;
	mbx_cmd_t  mc;
	mbx_cmd_t  *mcp = &mc;
	...
	vha = fcport->vha;
	ha = vha->hw;
	req = ha->req_q_map[tag];
	rsp = ha->rsp_q_map[tag];
    ...
    uint8_t *area, uint8_t *domain, uint16_t *top, uint16_t *sw_cap)
{
	...
	if (IS_QLA8XXX_TYPE(vha->hw))
		mcp->in_mb |= MBX_13|MBX_12|MBX_11|MBX_10|MBX_9;
	mcp->tov = MBX_TOV_SECONDS;
	mcp->flags = 0;
	rval = qla2x00_mailbox_command(vha, mcp);
	...
	*sw_cap = mcp->mb[7];
	...
	if (rval != QLA_SUCCESS) {
		...
		printk("vlan_id: 0x%x\n", vha->fcoe_vlan_id);
		printk("vn_port_mac_address: %02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    uint8_t area, uint8_t al_pa, uint16_t *mb, uint8_t opt)
{
	...
	lg->port_id[1] = area;
	lg->port_id[2] = domain;
	lg->vp_index = cpu_to_le16(vha->vp_idx);
 ...
 */
int
qla2x00_send_rnid_mbx(scsi_qla_host_t *vha, uint16_t loop_id, uint8_t data_fmt,
...
/*
 * qla2x00_set_params_mbx
 *	Set parameters using mailbox command
 *
 * Input:
 *	ha = adapter state pointer.
 *	buffer = buffer pointer.
 ...
 */
int
qla2x00_set_params_mbx(scsi_qla_host_t *vha, dma_addr_t buf_phys_addr,
	size_t buf_size, uint16_t *mb_reg, uint16_t type)
{
	int		rval;
	mbx_cmd_t	mc;
	mbx_cmd_t	*mcp = &mc;

	DEBUG11(printk("qla2x00_set_params_mbx(%ld): entered.\n",
	    vha->host_no));

	mcp->mb[0] = MBC_SET_PARAMS;
	mcp->mb[1] = type << 8;
	mcp->mb[2] = MSW(buf_phys_addr);
	mcp->mb[3] = LSW(buf_phys_addr);
	mcp->mb[6] = MSW(MSD(buf_phys_addr));
	...
	if (rval != QLA_SUCCESS) {
		...
		DEBUG2_3_11(printk("qla2x00_set_params_mbx(%ld): "
		    "failed=%x mb[1]=%x.\n", vha->host_no, mcp->mb[0],
		    mcp->mb[1]));
	} else {
		/*EMPTY*/
		DEBUG11(printk("qla2x00_set_params_mbx(%ld): done.\n",
		    vha->host_no));
	}
...
/*
 * qla2x00_get_params_mbx
 *	Get parameters using mailbox command
 *
 * Input:
 *	ha = adapter state pointer.
 *	buffer = buffer pointer.
 ...
 */
int
qla2x00_get_params_mbx(scsi_qla_host_t *vha, dma_addr_t buf_phys_addr,
	size_t buf_size, uint16_t *mb_reg, uint16_t type)
{
	int		rval;
	mbx_cmd_t	mc;
	...
	DEBUG11(printk("qla2x00_get_params_mbx(%ld): entered.\n",
	    vha->host_no));

	mcp->mb[0] = MBC_GET_PARAMS;
	mcp->mb[1] = type << 8;
	mcp->mb[2] = MSW(buf_phys_addr);
	mcp->mb[3] = LSW(buf_phys_addr);
	mcp->mb[6] = MSW(MSD(buf_phys_addr));
	...
	if (rval != QLA_SUCCESS) {
		...
		DEBUG2_3_11(printk("qla2x00_get_params_mbx(%ld): "
		    "failed=%x mb[1]=%x.\n", vha->host_no, mcp->mb[0],
		    mcp->mb[1]));
	} else {
		/*EMPTY*/
		DEBUG11(printk("qla2x00_get_params_mbx(%ld): done.\n",
		    vha->host_no));
	}
...
int
qla24xx_abort_target(fc_port_t *fcport, int tag)
{
	int		rval;
	...
	/* Issue marker IOCB. */
	rval = qla2x00_marker(vha, req, rsp, fcport->loop_id, 0, MK_SYNC_ID);
	if (rval != QLA_SUCCESS) {
 ...
 */
int
qla2x00_get_serdes_params(scsi_qla_host_t *vha, uint16_t *sw_em_1g,
	...
	uint16_t dev, uint16_t off, uint16_t len, uint16_t opt)
{
	...
	DEBUG11(printk("%s(%ld): entered.\n", __func__, vha->host_no));
	DEBUG2_3_11(printk("%s: dev %x, off %x, len %x, opt %x\n",
		__func__, dev, off, len, opt));
	...
	if (opt & BIT_0) {
		*sfp = mcp->mb[1];
		DEBUG2_3_11(printk("%s: value read = %x\n", __func__, mcp->mb[1]));
	}
	...
	uint16_t dev, uint16_t off, uint16_t len, uint16_t opt)
{
	...
	if (opt & BIT_0) {
		len = *sfp;
		DEBUG2_3_11(printk("%s: value to be written = %x\n", __func__, len));
	}
...
int qla2xxx_set_fcp_prio(scsi_qla_host_t *vha, uint16_t loop_id,
		uint16_t priority, uint16_t *mb)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
 ...
 */
int
qla24xx_get_vp_database(scsi_qla_host_t *vha, uint16_t size)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
...
int
qla24xx_get_vp_entry(scsi_qla_host_t *vha, uint16_t size, int vp_id)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	...
	struct vp_rpt_id_entry_24xx *rptid_entry)
{
	...
	if (rptid_entry->format == 0) {
		DEBUG2(printk("%s:format 0 : scsi(%ld) number of VPs setup %d,"
			" number of VPs acquired %d\n", __func__, vha->host_no,
			MSB(rptid_entry->vp_count), LSB(rptid_entry->vp_count)));
		DEBUG2(printk("%s primary port id %02x%02x%02x\n", __func__,
			rptid_entry->port_id[2], rptid_entry->port_id[1],
			rptid_entry->port_id[0]));
	} else if (rptid_entry->format == 1) {
		vp_idx = LSB(rptid_entry->vp_idx);
		DEBUG2(printk("%s:format 1: scsi(%ld): VP[%d] enabled "
		    "- status %d - "
...
int
qla2x00_loopback_test(scsi_qla_host_t *vha, INT_LOOPBACK_REQ *req,
    uint16_t *ret_mb)
{
	int             rval;
	mbx_cmd_t       mc;
	...
	if (IS_QLA8XXX_TYPE(vha->hw)) {
		mcp->mb[2] = 0;	/* FCF index */
		mcp->out_mb |= MBX_2;
	}
	mcp->mb[10] = LSW(req->TransferCount);
	mcp->mb[11] = MSW(req->TransferCount);
	mcp->mb[14] = LSW(vha->ioctl_mem_phys); /* send data address */
	mcp->mb[15] = MSW(vha->ioctl_mem_phys);
	mcp->mb[20] = LSW(MSD(vha->ioctl_mem_phys));
	mcp->mb[21] = MSW(MSD(vha->ioctl_mem_phys));
	mcp->mb[16] = LSW(vha->ioctl_mem_phys); /* rcv data address */
	mcp->mb[17] = MSW(vha->ioctl_mem_phys);
	mcp->mb[6] = LSW(MSD(vha->ioctl_mem_phys));
	mcp->mb[7] = MSW(MSD(vha->ioctl_mem_phys));
...
int
qla2x00_echo_test(scsi_qla_host_t *vha, INT_LOOPBACK_REQ *req, uint16_t *ret_mb)
{
	int             rval;
	mbx_cmd_t       mc;
	mbx_cmd_t       *mcp = &mc;
	...
	if (IS_QLA8XXX_TYPE(vha->hw)) {
		mcp->mb[1] = BIT_15; /* send Echo ELS */
		/* mb[2] FCF index set to 0 by memset */
	} else if (IS_QLA23XX(vha->hw))
	...
	if (rval != QLA_SUCCESS) {
		/* Empty. */
		DEBUG3_11(printk(
				"%s(%ld): mailbox command FAILED=%x/%x. mbx[3]=0x%x\n",
				__func__, vha->host_no, mcp->mb[0], mcp->mb[1], mcp->mb[3]));
	} else {
		/* Empty. */
		DEBUG11(printk("%s(%ld): done.\n", __func__, vha->host_no));
	}
 ...
 */
int
__qla2x00_lun_reset(scsi_qla_host_t *vha, fc_port_t *fcport, uint16_t lun, int tag)
{
	int		rval;
	mbx_cmd_t	mc;
	mbx_cmd_t	*mcp = &mc;
	...
	struct qla_hw_data *ha = vha->hw;
	struct req_que *req = ha->req_q_map[tag];
	struct rsp_que *rsp = ha->rsp_q_map[tag];

	if (IS_QLA24XX_TYPE(ha) || IS_QLA54XX(ha) || IS_QLA25XX(ha)
			|| IS_QLA8XXX_TYPE(ha)) {
		return qla24xx_lun_reset(vha, fcport, lun, tag);
	}
	...
	if (rval != QLA_SUCCESS) {
		...
		printk(KERN_WARNING "qla2x00_lun_reset(%d): failed = %d",
		    (int)vha->instance, rval);
	} else {
		printk(KERN_INFO "qla2x00_lun_reset(%d): issue marker 0x%x lun %d",
		    (int)vha->instance, loop_id, lun);
		rval = qla2x00_marker(vha, req, rsp, loop_id, lun, MK_SYNC_ID_LUN);
		if (rval != QLA_SUCCESS) {
...
int
qla2x00_lun_reset(scsi_qla_host_t *vha, fc_port_t *fcport, uint16_t lun, int tag)
{
	int		rval;

	DEBUG(printk("%s(%ld): lun %d\n", __func__, vha->host_no, lun));
	rval = 	__qla2x00_lun_reset(vha, fcport, lun, tag);

	return rval;
}

int
qla24xx_lun_reset(scsi_qla_host_t *vha, fc_port_t *fcport, uint16_t lun, int tag)
{
	int		rval;
	...
	/* Issue marker IOCB. */
	rval = qla2x00_marker(vha, req, rsp, fcport->loop_id, lun, MK_SYNC_ID_LUN);
	if (rval != QLA_SUCCESS) {
		DEBUG2_3_11(printk("%s(%ld): failed to issue Marker IOCB "
		    "(%x).\n", __func__, vha->host_no, rval));
	} else {
...
int qla2xxx_read_serdes_word(scsi_qla_host_t *vha, uint16_t addr,
		uint16_t *data)
{
	int rval;
	mbx_cmd_t mc;
...
int qla8xxx_get_xgmac_stats(scsi_qla_host_t *vha, dma_addr_t stats_dma,
...
int
qla8xxx_get_dcbx_params(scsi_qla_host_t *vha, dma_addr_t tlv_dma,
    uint16_t size)
{
	int rval;
	mbx_cmd_t mc;
	...
	if (!IS_QLA81XX(vha->hw) && !IS_QLA82XX(vha->hw))
		return QLA_FUNCTION_FAILED;
...
int
qla81xx_restart_mpi_firmware(scsi_qla_host_t *vha, uint16_t *mb)
{
	int rval = 0;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
...
int
qla82xx_config_led(scsi_qla_host_t *vha, uint16_t beacon_ops)
{
	int rval;
	mbx_cmd_t mc;
	mbx_cmd_t *mcp = &mc;
	...
	if (rval != QLA_SUCCESS) {
		DEBUG2_3_11(printk("%s(%ld): failed=%x mb[0]=%x.\n", __func__,
		    vha->host_no, rval, mcp->mb[0]));
		if (mcp->mb[0] == 0x4005)
			DEBUG2_11(printk("scsi(%ld): Beaconing coult not be "
				"enabled/disabled.\n", vha->host_no));
		else if (mcp->mb[0] == 0x4006)
			DEBUG2_11(printk("scsi(%ld): Beaconing parameter error "
				"(invalid option in mb7).\n", vha->host_no));
	} else {
		DEBUG11(printk("%s(%ld): done.\n", __func__, vha->host_no));
	}

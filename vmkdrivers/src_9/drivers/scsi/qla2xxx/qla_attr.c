...
qla24xx_vport_create(struct fc_vport *fc_vport, bool disable)
{
	...
	if (ha->flags.cpu_affinity_enabled) {
		req = ha->req_q_map[1];
		rsp_que_id = RSP_QUE_NONE;
	}
	if (!ha->flags.qos_enabled)
		goto vport_queue;
	/* Create a request queue in QoS mode for the vport */
	options |= (BIT_7 | BIT_8);
	ret = qla25xx_create_req_que(ha, options, vha->vp_idx,
			RID_DEFAULT, rsp_que_id, QLA_DEFAULT_QUE_QOS_BW);
	if (!ret)
		qla_printk(KERN_WARNING, ha,
		...
		DEBUG2(qla_printk(KERN_INFO, ha,
			...
			ret, QLA_DEFAULT_QUE_QOS_BW, vha->vp_idx));
...
qla24xx_vport_delete(struct fc_vport *fc_vport)
{
	...
	scsi_remove_host(vha->host);
		...
		if (vha->req->id > 1 && ha->flags.qos_enabled) {
			if (qla25xx_delete_req_que(vha, vha->req) != QLA_SUCCESS) {
				qla_printk(KERN_WARNING, ha, "Couldn't delete req que %d\n",
						vha->req->id);
				qla_printk(KERN_WARNING, ha, "Queue delete failed.\n");
			}
		}

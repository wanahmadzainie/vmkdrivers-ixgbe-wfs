...
int qla25xx_update_req_que(struct scsi_qla_host *vha, uint8_t qos)
{
	int ret = 0;
...
qla25xx_delete_queues(struct scsi_qla_host *vha)
{
	...
	for (cnt = 1; queue_count < ha->num_req_queues; cnt++) {
	...
	for (cnt = 1; cnt < ha->num_rsp_queues; cnt++) {
 	...
 	uint8_t vp_idx, uint16_t rid, int req)
{
	...
	spin_lock_irqsave(&ha->vport_lock, flags);
	que_id = find_first_zero_bit(ha->rsp_qid_map, ha->num_rsp_queues);
	if (que_id >= ha->num_rsp_queues) {
		spin_unlock_irqrestore(&ha->vport_lock, flags);
		qla_printk(KERN_INFO, ha, "No resources to create "
...
qla2xxx_initialize_boothost(scsi_qla_host_t *vha)
{
	...
	if (qla2xxx_flash_npiv_conf(vha, 0))
		return;

	/* Initialize control block for boot vports. */
	spin_lock_irqsave(&ha->vport_lock, flags);
	list_for_each_entry(vp, &ha->vp_list, list) {
		if (vp->vp_idx && !vp->fc_vport) {

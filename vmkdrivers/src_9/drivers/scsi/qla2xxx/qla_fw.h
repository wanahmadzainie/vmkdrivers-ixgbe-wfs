...
#define FA_VPD_SIZE_82	0x400
	uint32_t req_q_out[64];		/* Request Queue out-Pointer. (64 * 4) */
	uint32_t rsp_q_in[64];		/* Response Queue In-Pointer. (64 * 4) */
...
#define ISRX_QLA82XX_RISC_INT	BIT_0	/* RISC interrupt. */
};
...
struct qla_fcp_prio_entry {
	...
	/* priority entries that are valid	*/
...
#define FCP_PRIO_ENTRY_DWWN_VALID	0x80
	...
	uint32_t src_pid;	/* Src port id. high order byte unused;
						   -1 (wild card) */
	uint32_t dst_pid;	/* Src port id. high order byte unused;
						   -1 (wild card) */
	uint16_t lun_beg;	/* 1st lun num of lun range. -1 (wild card) */
	uint16_t lun_end;	/* 2nd lun num of lun range. -1 (wild card) */
...
#define MPI_SCODE_FW_IMAGE_ERROR			0x27
#define MPI_SCODE_FW_AUTO_LOAD_CFG_ERROR	0x28
...
#define BRDCFG_START	0x4000		/* board config */
#define BOOTLD_START	0x10000		/* boot loader */
#define IMAGE_START		0x43000		/* compressed image */
#define MS_IMAGE_START	0x100000	/* compressed image (MS) */
#define USER_START		0x3E8000	/* Firmware info */

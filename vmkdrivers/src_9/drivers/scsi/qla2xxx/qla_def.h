...
/* MQ related defines */
#define MSIX_CNTS	/* code for testing purposes, shows ints/vector in proc */
...
#define SRB_IOCTL		BIT_1	/* IOCTL command. */
#define SRB_TAPE		BIT_2	/* FCP2 (Tape) command. */
#define SRB_NO_TIMER	BIT_3  /* Do not attach timer to cmd */
#define SRB_FCP_CMND_DMA_VALID	BIT_4	/* FCP command in IOCB */
...
#define MBA_AUTO_DMA_ERROR	0x8070	/* Unrecoverable error with Auto DMA detected */
...
#define MBC_SET_PARAMS			0x59	/* Set Parameters */
#define MBC_GET_PARAMS			0x5a	/* Get Paramters */
 ...
 */
#define MBC_READ_SERDES			0x04	/* Read serdes word */
...
/*
 * ISP82xx specific mailbox commands
 */
#define MBC_SET_LED_CONFIG     0x125    /* Enable/Disable 82XX Beaconing */
#define MBC_GET_MINIDUMP       0x129    /* Get 82XX minidump */

/* Firmware return data sizes */
#define FCAL_MAP_SIZE	128

/* MB Cmd 59h (Set Paramters) type definitions */
#define SET_PARAM_TYPE_RNID		0x0
#define SET_PARAM_TYPE_STOP_AUTO_DMA	0x3
#define SET_PARAM_TYPE_START_AUTO_DMA	0x4

/* MB Cmd 5Ah (Get Paramters) type definitions */
#define GET_PARAM_TYPE_RNID		0x0
#define GET_PARAM_TYPE_OCSD_INFO	0x3
#define GET_PARAM_TYPE_TEMP_SENSOR_BUF	0x4
...
typedef struct qla82xx_minidump_template_hdr {
	uint32_t entry_type;
	...
	uint32_t num_of_free_entries;	/* For f/w internal use */
	uint32_t free_entry_offset;	/* For f/w internal use */
	uint32_t total_table_size;	/*  For f/w internal use */
	uint32_t bkup_table_offset;	/*  For f/w internal use */
} __attribute__((packed)) qla82xx_minidump_template_hdr_t;
...
#define FCF_PERSISTENT_BOUND	BIT_2
#define FCF_TAPE_PRESENT	BIT_3
#define FCF_RLC_SUPPORT		BIT_4
#define FCF_FCP2_DEVICE		BIT_5
...
struct isp_operations {
	...
	int (*abort_target) (struct fc_port *, int);
	...
	int (*alloc_sp_pool) (struct qla_hw_data *);
	void (*free_sp_pool) (struct qla_hw_data *);
};

struct mdump_size_table
{
	uint32_t size;
...
#define QLA_MAX_QUEUES 255
#define ISP_QUE_REG(ha, id) \
	((ha->mqenable) ? \
	((void *)(ha->mqiobase) +\
	(QLA_QUE_PAGE * id)) :\
	((void *)(ha->iobase)))
#define QLA_DEFAULT_QUE_QOS_PRI 5
#define QLA_DEFAULT_QUE_QOS_BW 100
#define QLA_PRECONFIG_VPORTS 32
#define QLA_MAX_VPORTS_QLA24XX	128
#define QLA_MAX_VPORTS_QLA25XX	256
#define VP_IDX_0 0
#define RID_DEFAULT 0
#define RSP_QUE_NONE -1
...
struct qla_hw_data {
	...
	volatile struct {
		...
		uint32_t	qos_enabled				:1;
		uint32_t	qos_ioctl_enabled		:1;
...
/* Multi queue data structs */
	...
	uint8_t		max_qos_req_queues;
	uint8_t		num_req_queues;
	uint8_t		num_rsp_queues;
...
#define IS_NOCACHE_VPD_TYPE(ha)       (IS_QLA81XX(ha)) /* Do not cache VPD */


#define IS_AUTO_DMA(ha)		((ha)->device_type & DT_AUTO_DMA)
...
#define MPI_CAP_LIST_PROTOTYPE	0x01
#define MPI_CAP_LIST_QLE8142	0x02
#define MPI_CAP_LIST_QLE8152	0x03
#define MPI_CAP_LIST_QMI8142	0x04
	...
	dma_addr_t	eft_dma;
	void		*eft;
	/* 82XX specific f/w mindump */
	void *fw_minidump;
	int fw_dump_size;
	int capture_mask;
	void *md_template_hdr;
...
typedef struct scsi_qla_host {
	struct list_head list;
	struct list_head hostlist;	/* for qla_hostlist */
...
#define VPORT_CLEANUP_NEEDED  8
#define RELOGIN_NEEDED        9
#define LOGIN_RETRY_NEEDED    10      /* Initiate required fabric logins. */
#define REGISTER_FC4_NEEDED   11      /* SNS FC4 registration required. */
#define ISP_ABORT_RETRY       12      /* ISP aborted. */
#define FCPORT_RESCAN_NEEDED  13      /* IO descriptor processing needed */
#define LOOP_RESET_NEEDED     14
#define BEACON_BLINK_NEEDED   15
#define REGISTER_FDMI_NEEDED  16
#define FCPORT_UPDATE_NEEDED  17
#define VP_DPC_NEEDED         18      /* wake up for VP dpc handling */
#define VP_DELETE_ACTIVE      19      /* VP deletion in progress */
#define ISP_UNRECOVERABLE     20      /* Device in un-recoverable state (82XX) */
#define ISP_QUIESCE_NEEDED    21      /* Device needs some quiesce time (82XX) */
#define FCOE_CTX_RESET_NEEDED 22      /* Initiate quick (FCoE ctx) reset (82XX) */
#define STOP_AUTO_DMA	      23      /* Stop Auto DMA of temperature sensor data */
#define MPI_RESET_NEEDED      24      /* Initiate MPI FW reset */
#define IOCTL_ERROR_RECOVERY  25      /* Perform error recovery for IOCTL cmds */

	uint32_t	device_flags;
 #define       SWITCH_FOUND            BIT_0
 #define       DFLG_NO_CABLE           BIT_1
 #define       DFLG_DEV_FAILED         BIT_2
...
typedef struct serdes_params {
	struct { uint8_t dat; uint8_t en; } amp_fdr;
	struct { uint8_t dat; uint8_t en; } amp_hdr;
	struct { uint8_t dat; uint8_t en; } amp_qdr;

	struct { uint8_t dat; uint8_t en; } imain_fdr;
	struct { uint8_t dat; uint8_t en; } imain_hdr;
	struct { uint8_t dat; uint8_t en; } imain_qdr;

	struct { uint8_t dat; uint8_t en; } ipst_fdr;
	struct { uint8_t dat; uint8_t en; } ipst_hdr;
	struct { uint8_t dat; uint8_t en; } ipst_qdr;

	struct { uint8_t dat; uint8_t en; } ipre_fdr;
	struct { uint8_t dat; uint8_t en; } ipre_hdr;
	struct { uint8_t dat; uint8_t en; } ipre_qdr;

	struct { uint8_t dat; } z1cnt_fdr;
	struct { uint8_t dat; } z1cnt_hdr;

	struct { uint8_t dat; } g1cnt_fdr;
	struct { uint8_t dat; } g1cnt_hdr;

	struct { uint8_t dat; } zcnt_fdr;
	struct { uint8_t dat; } zcnt_hdr;

	struct { uint8_t dat; } tlth_fdr;
	struct { uint8_t dat; } tlth_hdr;

	struct { uint8_t dat; } dfelth_fdr;
	struct { uint8_t dat; } dfelth_hdr;
}serdes_params_t;

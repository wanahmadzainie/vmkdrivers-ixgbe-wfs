...
extern int qla82xx_diagnostic_command(scsi_qla_host_t *, EXT_IOCTL *, int);
extern int qla2xxx_get_board_temp(scsi_qla_host_t *, EXT_IOCTL *, int);
extern int qla25xx_get_serdes_params(scsi_qla_host_t *, EXT_IOCTL *, int);
extern int qla2xxx_host_side_qos_cmd(scsi_qla_host_t *, EXT_IOCTL *, int);
extern int qla2xxx_get_vport_list(scsi_qla_host_t *, EXT_IOCTL *, int);
extern int qla2x00_fru_vpd_op(scsi_qla_host_t *, EXT_IOCTL *, int);
...
qla2x00_cmd_timeout(srb_t *sp)
{
	...
	base_vha = pci_get_drvdata(vis_ha->hw->pdev);
	req = vis_ha->req;
...
#define MAX_RETRIES 6	
	...
	case INT_CC_HOST_SIDE_QOS:
		ret = qla2xxx_host_side_qos_cmd(vha, pext, mode);
		break;

	case INT_CC_GET_VPORT_LIST:
		ret = qla2xxx_get_vport_list(vha, pext, mode);
		break;

	case INT_CC_DIAG_MODE:
		ret = qla82xx_diagnostic_command(vha, pext, mode);
		break;
	...
	case INT_CC_FRU_VPD_OP:
		ret = qla2x00_fru_vpd_op(vha, pext, mode);
		break;

	case INT_CC_GET_SERDES_PARAMS:
		ret = qla25xx_get_serdes_params(vha, pext, mode);
		break;
...
qla2x00_query_hba_node(scsi_qla_host_t *vha, EXT_IOCTL *pext, int mode)
{
	...
	if (IS_QLA24XX(ha) || IS_QLA54XX(ha) || IS_QLA25XX(ha) ||
		IS_QLA2322(ha) || IS_QLA8XXX_TYPE(ha)) {
...
qla2x00_query_hba_port(scsi_qla_host_t *vha, EXT_IOCTL *pext, int mode)
{
	...
	ptmp_hba_port->Topology = ha->current_topology;
...
qla2x00_get_statistics(scsi_qla_host_t *vha, EXT_IOCTL *pext, int mode)
{
	...
	ptmp_stat->ControllerErrorCount   =  vha->qla_stats.total_isp_aborts;
	ptmp_stat->DeviceErrorCount       =  vha->total_dev_errs;
	ptmp_stat->TotalIoCount           =  vha->total_ios;
	ptmp_stat->TotalMBytes            =  (vha->qla_stats.input_bytes +
						vha->qla_stats.output_bytes) >> 20;
	ptmp_stat->TotalLipResets         =  vha->total_lip_cnt;
...
qla2x00_query_driver(scsi_qla_host_t *vha, EXT_IOCTL *pext, int mode)
{
	...
	pdriver_prop->LunsPerTarget = ql2xmaxlun;
...
qla2x00_get_rnid_params(scsi_qla_host_t *vha, EXT_IOCTL *pext, int mode)
{
	...
	/* Send command */
	tmp_rval = qla2x00_get_params_mbx(vha, vha->ioctl_mem_phys,
			sizeof(EXT_RNID_DATA), &mb[0], GET_PARAM_TYPE_RNID);
...
#define	DCBX_PARAM_BUFFER_SIZE	0x1000
	...
	ret = qla8xxx_get_dcbx_params(vha, vha->ioctl_mem_phys, transfer_size);
	if (ret != QLA_SUCCESS) {
		/* error */
		pext->Status = EXT_STATUS_ERR;
    ...
    fc_port_t *pfcport, fc_lun_t *pfclun)
{
	...
	pscsi_cmd->device->hostdata	= sp->fcport;
...
qla2x00_enqueue_aen(scsi_qla_host_t *vha, uint16_t event_code, void *payload)
{
    ...
    switch (event_code) {
    case MBA_LOOP_UP:
        vmklnx_iodm_event(vha->host, IODM_LINKUP, NULL, 0);
        break;
    case MBA_LOOP_DOWN:
        vmklnx_iodm_event(vha->host, IODM_LINKDOWN, NULL, 0);
        break;
    case MBA_RSCN_UPDATE:
        if (payload != NULL) {
                mbx = (uint16_t *)payload;
		vmklnx_iodm_event(vha->host, IODM_RSCN, NULL, mbx[1] << 16 | mbx[2]);
        }    
        break;
    }
#endif

    /* Skip enqueuing aens for pre-boot hosts. */
    if (vha->vp_idx && !vha->fc_vport)
        return;
...
qla2x00_set_rnid_params(scsi_qla_host_t *vha, EXT_IOCTL *pext, int mode)
{
	...
	tmp_rval = qla2x00_set_params_mbx(vha, vha->ioctl_mem_phys,
			sizeof(EXT_RNID_DATA), &mb[0], SET_PARAM_TYPE_RNID);
    ...
    uint32_t *pext_stat, uint32_t *pext_dstat)
{
	...
	case EXT_DEF_GRN_BLINK_ON:
		...
		if(ha->isp_ops->beacon_on(vha) != QLA_SUCCESS) {
			*pext_stat  = EXT_STATUS_ERR;
			*pext_dstat = EXT_STATUS_ERR;
			DEBUG9_10(printk("%s(%ld): inst=%ld beacon on failed.\n",
				__func__, vha->host_no, vha->instance));
			break;
		}	
	...
	case EXT_DEF_GRN_BLINK_OFF:
		...
		if(ha->isp_ops->beacon_off(vha) != QLA_SUCCESS) {
			*pext_stat  = EXT_STATUS_ERR;
			*pext_dstat = EXT_STATUS_ERR;
			DEBUG9_10(printk("%s(%ld): inst=%ld beacon off failed.\n",
				__func__, vha->host_no, vha->instance));
			break;
		}
